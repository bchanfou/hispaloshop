...
export default function FeedRecipeCard({
  ...
  const DIFF_LABELS = {
    easy: 'Fácil',
    medium: 'Media',
    hard: i18n.t('recipes.hard', 'Difícil')
  };
  return <div className="mx-3 my-3 rounded-2xl shadow-sm bg-white overflow-hidden lg:hover:shadow-md lg:hover:-translate-y-0.5 transition-all duration-200">
      <div className="cursor-pointer" onClick={() => setShowOverlay(true)} role="button" tabIndex={0} onKeyDown={e => {
      if (e.key === 'Enter') setShowOverlay(true);
    }}>
        {/* Recipe image */}
        {image ? <div className="aspect-[3/4] w-full overflow-hidden relative">
            <img src={image} alt={title} className="h-full w-full object-cover" loading="lazy" />
            <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/50 to-transparent" />
            {prepTime && <span className="absolute bottom-3 left-3 inline-flex items-center gap-1 rounded-full bg-white/90 backdrop-blur-sm px-2.5 py-1 text-[11px] font-medium text-stone-700">
                <Clock className="h-3 w-3" />
                {prepTime} min
              </span>}
          </div> : <div className="aspect-[3/4] w-full bg-stone-50 flex items-center justify-center">
        ...
        <div className="p-3.5">
          ...
          {(authorName || difficulty) && <div className="mt-1 flex items-center gap-2 flex-wrap">
              {authorName && <span className="text-xs text-stone-500 truncate max-w-[140px]">{authorName}</span>}
              {difficulty && DIFF_LABELS[difficulty] && <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${difficulty === 'hard' ? 'bg-stone-950 text-stone-50' : 'bg-stone-100 text-stone-600'}`}>
      ...
      {/* R-10: Recipe overlay */}
      {showOverlay && <Suspense fallback={null}>
          <RecipeOverlay recipe={recipe} onClose={() => setShowOverlay(false)} />
        </Suspense>}

      {/* P-09: Ingredient shopping section */}
      {ingredients.length > 0 && <div className="border-t border-stone-100">
        ...
        setShowIngredients(prev => !prev);
          ...
          duration: 0.2
              ...
              if (!name) return null;
              return <div key={idx} className="flex items-center justify-between gap-2">
                        <span className="text-[13px] text-stone-700 flex-1 min-w-0 truncate">{name}</span>
                        <button type="button" onClick={() => navigate(`/search?q=${encodeURIComponent(name)}`)} className="flex items-center gap-1 shrink-0 rounded-full bg-stone-100 px-2.5 py-1 text-[11px] font-medium text-stone-600 border-none cursor-pointer hover:bg-stone-200 transition-colors">
                          <Search size={10} />
                          Buscar
                        </button>
                      </div>;
