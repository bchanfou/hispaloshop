...
export default function StoriesBar({
  ...
  const selfStory = currentUser ? stories.find(s => s.user_id === currentUser.user_id || s.user_id === currentUser.id || s.user_id === currentUser._id) : null;
  return <div className="scrollbar-hide flex gap-2.5 overflow-x-auto overscroll-x-contain snap-x snap-mandatory px-3 py-2" role="region" aria-label="Historias" tabIndex={0}>
      {/* Self ring — only when authenticated */}
      {currentUser && <div className="relative shrink-0 snap-center">
          <StoryRing user={currentUser} isSelf hasUnseenStory={hasActiveStory} onClick={hasActiveStory ? async () => {
        ...
        try {
          ...
          {!hasActiveStory && <motion.div animate={{
        scale: [1, 1.15, 1]
      }} transition={{
        duration: 2,
        repeat: Infinity,
      ...
      {featuredRecipe && <div className="shrink-0 snap-center">
          <button type="button" onClick={() => navigate(`/recipes/${featuredRecipe.recipe_id || featuredRecipe.id}`)} className="flex flex-col items-center gap-1 w-[58px] bg-transparent border-none cursor-pointer p-0">
            <div className="relative h-[58px] w-[58px] rounded-full bg-gradient-to-br from-stone-200 to-stone-400 p-[2px]">
              <div className="h-full w-full rounded-full bg-white p-[2px]">
                {featuredRecipe.image_url ? <img src={featuredRecipe.image_url} alt={i18n.t('stories_bar.recetaDelDia', 'Receta del día')} className="h-full w-full rounded-full object-cover" /> : <div className="h-full w-full rounded-full bg-stone-100 flex items-center justify-center">
      ...
      {loading ? Array.from({
      length: 4
    }).map((_, i) => <div key={i} className="flex shrink-0 snap-center flex-col items-center gap-1 w-[58px]" aria-hidden="true">
              <div className="animate-pulse h-[58px] w-[58px] rounded-full bg-stone-100" />
            </div>) : error ? <button onClick={() => refetch()} className="flex shrink-0 snap-center flex-col items-center gap-1 w-[58px] bg-transparent border-none cursor-pointer" aria-label="Reintentar cargar historias">
            ...
            </button> : stories.length === 0 ? <div className="flex items-center px-2">
              <span className="text-xs text-stone-400">No hay historias recientes</span>
            </div> : stories.filter(s => !currentUser || s.user_id !== currentUser.user_id && s.user_id !== currentUser.id && s.user_id !== currentUser._id).map((story, idx) => <StoryRing key={story.user_id || idx} user={story.user} isSelf={false} hasUnseenStory={story.has_unseen ?? true} itemsCount={story.stories_count || 1} isLoading={loadingUserId === story.user_id} onClick={async () => {
      if (!onStoryClick || loadingUserId) return;
