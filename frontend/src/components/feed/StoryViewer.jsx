...
const EmojiBurst = ({
  ...
  duration: 0.8,
  ease: 'easeOut'
}} onAnimationComplete={onComplete} className="absolute bottom-0 left-1/2 -translate-x-1/2 text-3xl pointer-events-none">
    {emoji}
  </motion.span>;
...
export default function StoryViewer({
  ...
  const [tapHintShown, setTapHintShown] = useState(() => {
    try {
      ...
      /* storage unavailable */return false;
    }
  });
  ...
  const effectiveDuration = currentItem?.video_url ? videoDuration && isFinite(videoDuration) ? Math.min(videoDuration, MAX_VIDEO_DURATION) : STORY_DURATION : STORY_DURATION;
  ...
  useEffect(() => {
    ...
    queryClient.fetchQuery({
      ...
      if (fullItems.length > 0) {
        ...
        setLocalStories(prev => prev.map(s => s.user_id === targetUserId ? {
          ...s,
  ...
  const handleSendReply = useCallback(async () => {
    ...
    try {
      ...
      setReplySent(true);
      setTimeout(() => {
        if (mountedRef.current) setReplySent(false);
      }, 200);
      if (conversationId) {
        setTimeout(() => {
      ...
      try {
        await apiClient.post(`/stories/${currentItem?.story_id || currentItem?.id}/reply`, {
          text
        });
        setReplyText('');
        replyInputRef.current?.blur();
  ...
  const handleShareOpen = useCallback(async () => {
    ...
    try {
      ...
      /* non-critical: conversations fetch failed, show empty */
  ...
  const fetchViewers = useCallback(async () => {
    ...
    try {
      ...
      /* viewers fetch failed, show empty */
      setViewers([]);
    } finally {
  ...
  const overlayProductStickers = (currentItem?.overlays?.stickers || []).filter(s => s.type === 'product').map(s => ({
  ...
  return <motion.div className="fixed inset-0 z-[9999] flex flex-col bg-black" role="dialog" aria-label={`Historia de ${user?.name || user?.username || 'usuario'}`} aria-modal="true" initial={{
    scale: 0.85,
    opacity: 0
  }} animate={{
    scale: 1,
    ...
    overflow: 'hidden'
      ...
      {/* Progress bars */}
      <div className="flex gap-1 px-2 pt-[calc(env(safe-area-inset-top,8px)+8px)]" role="progressbar" aria-valuenow={Math.round(progress * 100)} aria-valuemin={0} aria-valuemax={100}>
        {items.map((item, i) => {
        let fillWidth;
        if (i < currentItemIndex) fillWidth = '100%';else if (i === currentItemIndex) fillWidth = `${progress * 100}%`;else fillWidth = '0%';
        return <div key={item.id || item._id || `story-seg-${i}`} className="flex-1 h-[3px] rounded-full overflow-hidden bg-white/30">
      ...
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        e.stopPropagation();
        handleClose();
        navigate(`/${user?.username || user?.id || user?.user_id}`);
      }} className="flex items-center gap-2 cursor-pointer" role="link" aria-label={`Ver perfil de ${user?.name || user?.username}`}>
          {user?.avatar_url || user?.avatar || user?.profile_image ? <img src={user.avatar_url || user.avatar || user.profile_image} alt={`Avatar de ${user?.name || user?.username || 'usuario'}`} className="w-8 h-8 rounded-full object-cover" /> : <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-semibold">
        ...
        {/* Paused indicator */}
        {paused && <span className="text-[10px] text-white/40 font-sans mr-1">En pausa</span>}
        <button onClick={handleClose} className="w-11 h-11 bg-transparent border-none cursor-pointer flex items-center justify-center" aria-label="Cerrar historia">
          <X size={24} className="text-white" />
        </button>
      </div>

      {/* Story content + tap/swipe zones */}
      <div className="flex-1 relative flex items-center justify-center overflow-hidden" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerCancel}>
        ...
        <AnimatePresence mode="wait" initial={false} custom={transitionCtx}>
          <motion.div key={`${currentUserIndex}-${currentItemIndex}`} custom={transitionCtx} variants={prefersReducedMotion ? undefined : storyVariants} initial={prefersReducedMotion ? false : 'enter'} animate="center" exit={prefersReducedMotion ? undefined : 'exit'} transition={{
          duration: 0.15
        }} className="absolute inset-0 flex items-center justify-center">
            {currentItem?.video_url ? <>
                <video ref={videoRef} key={currentItem.video_url} src={currentItem.video_url} autoPlay muted={muted} playsInline onLoadedMetadata={handleVideoLoaded} className="w-full h-full object-cover" />
                <button onClick={e => {
              e.stopPropagation();
              setMuted(m => !m);
            }} aria-label={muted ? 'Activar sonido' : 'Silenciar'} className="absolute top-16 right-4 z-10 w-11 h-11 rounded-full bg-black/40 flex items-center justify-center">
                ...
                {currentItem?.overlays?.texts?.map((t, i) => <div key={i} className="absolute pointer-events-none z-[3]" style={{
              left: `${t.x}%`,
              top: `${t.y}%`,
              ...
              borderRadius: t.style === 'box' ? 6 : undefined
                ...
                {Array.isArray(currentItem?.overlays?.draws) && currentItem.overlays.draws.length > 0 && <svg className="absolute inset-0 w-full h-full z-[2] pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                ...
                return <path key={pi} d={d} stroke={path.color || '#fff'} strokeWidth={path.width || 3} strokeLinecap="round" strokeLinejoin="round" fill="none" vectorEffect="non-scaling-stroke" />;
              })}
                  </svg>}
              ...
              if (s.type === 'emoji') {
                return <div key={i} className="text-4xl" style={pos}>
                        {s.content}
                      </div>;
              }
              if (s.type === 'poll') {
                  ...
                  pointerEvents: 'none'
                        ...
                        <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-3 shadow-lg w-[180px] text-center">
                          ...
                          <div className="flex gap-1">
                            {(s.options || []).map((opt, oi) => <div key={oi} className="flex-1 bg-stone-100 rounded-full py-1 px-1.5 text-[10px] font-semibold text-stone-950 text-center truncate">{opt}</div>)}
                          </div>
                        </div>
                      </div>;
              }
              if (s.type === 'question') {
                return <div key={i} style={pos}>
                        <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-3 shadow-lg w-[180px] text-center">
              ...
              </> : <img key={currentItem?.image_url} src={currentItem?.image_url} alt={currentItem?.caption || i18n.t('story_viewer.contenidoDeLaHistoria', 'Contenido de la historia')} className="w-full h-full object-cover" draggable={false} />}
            ...
            {isOwnStory && !readOnly && <>
            ...
            }} className="absolute bottom-4 left-4 z-[2] flex items-center gap-1 bg-transparent border-none cursor-pointer" aria-label="Ver quien ha visto esta historia">
              ...
              isPaused.current = true;
              setPaused(true);
            }} className="absolute bottom-4 right-4 z-[2] flex items-center gap-1 bg-transparent border-none cursor-pointer" aria-label="Eliminar story">
            ...
            <AnimatePresence>
              {showSeenBy && isOwnStory && <motion.div initial={{
              y: '100%'
            }} animate={{
              y: 0
            }} exit={{
              ...
              damping: 28,
              stiffness: 300
            }} className="absolute bottom-0 left-0 right-0 z-[3] bg-black/80 rounded-t-2xl max-h-[50%] flex flex-col" onClick={e => e.stopPropagation()}>
                  ...
                  isPaused.current = false;
                  setPaused(false);
                }} className="text-white/60 bg-transparent border-none cursor-pointer p-1" aria-label="Cerrar lista de vistas">
                      <X size={18} />
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto px-4 py-2">
                    {viewersLoading ? <div className="flex items-center justify-center py-6">
                        <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                      </div> : viewers.length > 0 ? viewers.map((v, vi) => <div key={v.user_id || v.id || vi} className="flex items-center gap-3 py-2">
                          {v.avatar_url || v.profile_image || v.avatar ? <img src={v.avatar_url || v.profile_image || v.avatar} alt="" className="w-8 h-8 rounded-full object-cover" /> : <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-semibold shrink-0">
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            {v.name && v.username && <span className="text-xs text-white/50 font-sans truncate">{v.name}</span>}
                          </div>
                        </div>) : <p className="text-center text-sm text-white/40 font-sans py-6">{i18n.t('story_viewer.sinDatosDeVistasDisponibles', 'Sin datos de vistas disponibles')}</p>}
                  </div>
                </motion.div>}
            </AnimatePresence>

            {/* 7.1: Product stickers + Link stickers — unified stack above reply bar */}
            {(effectiveProducts.length > 0 || currentItem?.links?.length > 0) && <div className="absolute bottom-16 left-4 right-4 z-[2] flex flex-col gap-2">
                {effectiveProducts.map((product, idx) => <div key={product.id || product.product_id || idx} onClick={e => {
              e.stopPropagation();
              handleClose();
              ...
              if (e.key === 'Enter' || e.key === ' ') {
            ...
            }} className="flex items-center gap-2 px-3 py-2.5 min-h-[44px] rounded-full bg-white/15 backdrop-blur-xl cursor-pointer" role="link" tabIndex={0} aria-label={`Ver producto: ${product?.name}`}>
                    {(product?.thumbnail || product?.image || product?.product_image) && <img src={product.thumbnail || product.image || product.product_image} alt="" className="w-8 h-8 rounded-2xl object-cover shrink-0" />}
                    <div className="flex flex-col flex-1 min-w-0">
                      <span className="text-[13px] text-white font-sans font-medium overflow-hidden text-ellipsis whitespace-nowrap">
                        {product?.name || product?.product_name}
                      </span>
                      {(product?.price ?? product?.product_price) != null && <span className="text-[11px] text-white/70 font-semibold font-sans">
                ...
                {currentItem?.links?.map((link, idx) => <div key={link.url || idx} onClick={e => {
              e.stopPropagation();
              if (!link?.url) return;
              ...
              if (e.key === 'Enter' || e.key === ' ') {
                ...
                if (isInternalUrl(link.url)) {
                  ...
                  try {
                    ...
                    /* non-critical: invalid URL, navigate raw */
                    navigate(link.url);
                  }
                } else {
                  window.open(link.url, '_blank', 'noopener');
            ...
            }} className="flex items-center gap-2 px-3 py-2.5 min-h-[44px] rounded-full bg-white/15 backdrop-blur-xl cursor-pointer" role="link" tabIndex={0} aria-label={link.label || link.url}>
        ...
        <AnimatePresence>
          {tapHintSide === 'left' && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          ...
          {tapHintSide === 'right' && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
      ...
      {!isOwnStory && <div className="flex items-center justify-center gap-1.5 px-3 py-1">
        {QUICK_REACTIONS.map(emoji => <div key={emoji} className="relative">
            <button onClick={() => handleQuickReaction(emoji)} className="w-10 h-10 rounded-full flex items-center justify-center text-xl bg-transparent border-none cursor-pointer hover:bg-white/10 transition-colors" aria-label={`Reaccionar con ${emoji}`}>
              {emoji}
            </button>
            {/* A-1: Burst animations above the emoji button */}
            <AnimatePresence>
              {emojiBursts.filter(b => b.emoji === emoji).map(b => <EmojiBurst key={b.id} id={b.id} emoji={b.emoji} onComplete={() => removeEmojiBurst(b.id)} />)}
            </AnimatePresence>
          </div>)}
      </div>}

      {/* Bottom bar: like + reply input + send + share — hidden for own stories */}
      {!isOwnStory && <div className="flex items-center gap-2 px-3 py-2 pb-[calc(env(safe-area-inset-bottom,8px)+8px)]">
      ...
      }} className="shrink-0 w-10 h-10 flex items-center justify-center bg-transparent border-none cursor-pointer" aria-label={liked ? 'Quitar me gusta' : 'Me gusta'}>
          <Heart size={24} className={liked ? 'text-white fill-white' : 'text-white'} strokeWidth={liked ? 0 : 1.5} />
        </button>
        {/* A-2: Taller reply input with @username placeholder */}
        <input ref={replyInputRef} type="text" value={replyText} onChange={e => setReplyText(e.target.value)} onFocus={() => {
        isPaused.current = true;
        setPaused(true);
      }} onBlur={() => {
        if (!shareSheetOpen) {
      ...
      }} placeholder={`Responder a @${user?.username || user?.name || 'usuario'}...`} className="flex-1 min-h-[44px] rounded-full bg-white/10 border border-white/20 px-4 text-sm text-white placeholder-white/40 outline-none focus:border-white/40 font-sans" />
        {/* J3: Send button — checkmark briefly on success, arrow when text present */}
        {replyText.trim() ? <button onClick={handleSendReply} disabled={sendingReply} className="shrink-0 w-10 h-10 flex items-center justify-center bg-white rounded-full border-none cursor-pointer disabled:opacity-50" aria-label="Enviar respuesta como mensaje directo">
            {replySent ? <Check size={18} className="text-stone-950" /> : <ArrowRight size={18} className="text-stone-950" />}
          </button> : (/* A-3: Share button (when no reply text) */
      <button onClick={handleShareOpen} className="shrink-0 w-10 h-10 flex items-center justify-center bg-transparent border-none cursor-pointer" aria-label="Compartir historia">
      ...
      <AnimatePresence>
        {shareSheetOpen && <motion.div initial={{
        y: '100%'
      }} animate={{
        y: 0
      }} exit={{
        ...
        stiffness: 300
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-2">
              ...
              <button onClick={handleShareClose} className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border-none cursor-pointer" aria-label="Cerrar panel de compartir">
            ...
            <div className="px-4 pb-2">
              <div className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-2">
                ...
                <input value={shareSearch} onChange={e => setShareSearch(e.target.value)} placeholder={i18n.t('story_viewer.buscarConversacion', 'Buscar conversación...')} className="flex-1 bg-transparent text-white border-none outline-none text-sm placeholder:text-white/30 font-sans" autoFocus />
              </div>
            </div>
            {/* Conversations list */}
            <div className="flex-1 overflow-y-auto px-4 pb-[calc(env(safe-area-inset-bottom,8px)+8px)]">
              {shareLoading ? <div className="flex justify-center py-6">
                  <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                </div> : filteredShareConversations.length === 0 ? <p className="text-center text-white/40 text-sm py-6">
                  {shareSearch ? 'Sin resultados' : 'No hay conversaciones recientes'}
                </p> : filteredShareConversations.slice(0, 20).map(conv => {
            ...
            return <button key={convId} onClick={() => handleShareToUser(conv)} disabled={shareSending === convId} className="flex items-center gap-3 w-full px-2 py-3 bg-transparent border-none cursor-pointer rounded-2xl hover:bg-white/10 transition-colors text-left disabled:opacity-50">
                      {avatar ? <img src={avatar} alt="" className="w-10 h-10 rounded-full object-cover shrink-0" /> : <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white text-sm font-semibold shrink-0">
                          {name.charAt(0).toUpperCase()}
                        </div>}
                      <span className="flex-1 text-sm text-white font-medium truncate">{name}</span>
                      {shareSending === convId ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin shrink-0" /> : <Send size={16} className="text-white/40 shrink-0" />}
                    </button>;
          })}
            </div>
          </motion.div>}
      ...
      <AnimatePresence>
        {showDeleteConfirm && <>
            <motion.div key="del-overlay" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          ...
          setPaused(false);
            ...
            <motion.div key="del-modal" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          duration: 0.15
        }} className="absolute left-4 right-4 top-1/2 -translate-y-1/2 z-[111] bg-white rounded-2xl p-4 shadow-xl mx-auto max-w-[320px]" onClick={e => e.stopPropagation()}>
              <p className="mb-1 text-center text-[15px] font-semibold text-stone-950">¿Eliminar esta historia?</p>
              <p className="mb-4 text-center text-sm text-stone-500">{i18n.t('contentManagement.deleteModal.warning', 'Esta acción no se puede deshacer.')}</p>
