...
export const Hero = ({
  ...
  visual
    ...
    <FadeUp>
      {eyebrow && <p className="text-[13px] font-semibold tracking-widest uppercase text-stone-500 mb-4">
          {eyebrow}
        </p>}
      ...
      <a href={ctaHref || '/registro'} className="inline-flex items-center gap-2 py-[15px] px-8 rounded-full text-base font-semibold no-underline shadow-lg transition-all duration-300 [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-[1.03] hover:shadow-xl" style={{
      background: ctaColor,
...
export const FeatureGrid = ({
  features
    ...
    <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-4 max-w-[1080px] mx-auto">
      {features.map((f, i) => <FadeUp key={f.title} delay={i * 0.07}>
...
export const PricingCard = ({
  ...
  isDark
    ...
    <div className={`rounded-[22px] p-8 relative h-full flex flex-col transition-all duration-300 ${isDark ? 'bg-stone-950' : 'bg-white'} ${isPopular ? 'scale-[1.03]' : 'scale-100'}`} style={{
    border: isPopular ? `2px solid ${accentColor}` : `0.5px solid rgba(0,0,0,${isDark ? 0.0 : 0.08})`,
    boxShadow: isPopular ? `0 8px 40px ${accentColor}28` : '0 2px 16px rgba(0,0,0,0.07)'
  }}>
      {isPopular && <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 text-white text-[11px] font-bold py-[5px] px-4 rounded-full tracking-wide uppercase whitespace-nowrap" style={{
      ...
      <div className="mb-6">
        ...
        {price > 0 && <span className={`text-sm ml-1 ${isDark ? 'text-white/50' : 'text-stone-400'}`}>
            + IVA{period}
          </span>}
      </div>
      ...
      <ul className="list-none p-0 flex-1 mb-6">
        {features.map(f => <li key={f} className={`flex items-start gap-2.5 py-[7px] text-sm leading-snug ${isDark ? 'border-b border-white/[0.07] text-white/85' : 'border-b border-black/[0.06] text-stone-950'}`}>
      ...
      {/* CTA */}
      <a href={ctaHref || '/registro'} className="block text-center py-3.5 px-6 rounded-full text-[15px] font-semibold no-underline transition-opacity duration-200 hover:opacity-90" style={{
...
export const PricingSection = ({
  ...
  plans
    ...
    <FadeUp>
      ...
      {sub && <p className="text-[17px] text-stone-500 text-center max-w-[480px] mx-auto mb-14">
          {sub}
        </p>}
    </FadeUp>
...
export const FooterCTA = ({
  ...
  ctaColor = '#0A0A0A'
    ...
    <FadeUp>
      ...
      <a href={ctaHref || '/registro'} className="inline-flex items-center gap-2 py-[15px] px-9 rounded-full bg-white text-stone-950 text-base font-bold no-underline transition-all duration-300">
        {cta} →
      </a>
    </FadeUp>
  </section>;
...
export const InfoNav = ({
  activePage
    ...
    <div className="flex gap-1">
      {[{
      href: '/que-es-hispaloshop',
      label: i18n.t('shared.queEs', 'Qué es')
    }, {
      href: '/productor',
      label: 'Productor'
    }, {
      ...
      href,
      label
    }) => <a key={href} href={href} className={`py-1.5 px-3 rounded-full text-[13px] font-medium no-underline transition-all duration-200 whitespace-nowrap ${activePage === href ? 'bg-stone-950 text-white' : 'bg-transparent text-stone-500'}`}>
          {label}
        </a>)}
    </div>
