...
export default function RecipeShoppingListOverlay({
  recipeId,
  defaultServings = 1,
  onClose
}) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    apiClient.get(`/recipes/${recipeId}/shopping-list-preview`, {
      params: {
        servings
      }
    }).then(data => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await apiClient.post(`/recipes/${recipeId}/shopping-list`, {
        items,
        servings
      });
      ...
      toast.error(error.message || i18n.t('errors.unauthorized', 'Inicia sesión para continuar'));
    } finally {
      setSubmitting(false);
    }
  };
  const total = (preview?.items || []).reduce((sum, item) => sum + (Number(item.price) || 0) * (quantities[item.product_id] || item.quantity || 1), 0);
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-[32px] bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-stone-200 px-5 py-4">
          ...
          <button type="button" onClick={onClose} className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-200 text-stone-500 transition-colors duration-150 hover:bg-stone-50 hover:text-stone-950" aria-label="Cerrar">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto px-5 py-5">
          <div className="mb-4 flex items-center justify-between rounded-2xl border border-stone-200 bg-stone-50 p-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => setServings(value => value + 1)} aria-label={i18n.t('recipe_shopping_list_overlay.masRaciones', 'Más raciones')} className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-700 cursor-pointer">
          ...
          {loading ? <div className="flex items-center justify-center py-14">
              ...
              {(preview?.items || []).map(item => <div key={item.product_id} className="flex items-center gap-3 rounded-2xl border border-stone-200 bg-stone-50 p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => setQuantities(current => ({
                  ...current,
                  [item.product_id]: Math.max(1, (current[item.product_id] || item.quantity || 1) - 1)
                }))} aria-label={`Menos ${item.name}`} className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-700 cursor-pointer">
                      <Minus className="h-3.5 w-3.5" />
                    </button>
                    <span className="w-6 text-center text-sm font-medium text-stone-950">{quantities[item.product_id] || item.quantity || 1}</span>
                    <button type="button" onClick={() => setQuantities(current => ({
                  ...current,
                  [item.product_id]: (current[item.product_id] || item.quantity || 1) + 1
                }))} aria-label={`Más ${item.name}`} className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-700 cursor-pointer">
        ...
        <div className="border-t border-stone-200 px-5 py-4">
          <div className="mb-3 flex items-center justify-between text-sm">
            ...
            <span className="font-semibold text-stone-950">{total.toLocaleString('es-ES', {
                style: 'currency',
                currency: 'EUR'
              })}</span>
          </div>
          <button type="button" onClick={handleSubmit} disabled={submitting || loading || !(preview?.items || []).length} className="h-11 w-full rounded-full border-none bg-stone-950 text-sm font-semibold text-white cursor-pointer hover:bg-stone-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors">
