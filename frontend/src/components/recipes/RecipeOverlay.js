...
function normalizeStep(step) {
  if (typeof step === 'string') {
    return { text: step, image_url: null };
  }
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
        ...
        {showNavigation && hasPrev ? (
          <button
            type="button"
            onClick={() => onNavigate?.('prev')}
            className="absolute left-3 top-1/2 z-[60] flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-stone-700 shadow-sm transition-all duration-150 hover:-translate-y-[52%] hover:bg-white"
        ...
        {showNavigation && hasNext ? (
          <button
            ...
            className="absolute right-3 top-1/2 z-[60] flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-stone-700 shadow-sm transition-all duration-150 hover:-translate-y-[52%] hover:bg-white"
                  ...
                  <div className="overflow-hidden rounded-3xl bg-stone-100">
                    <div className="aspect-square">
                      {recipeDetail?.image_url ? (
                        <img
                          src={resolveUserImage(recipeDetail.image_url)}
                  ...
                  <div className="rounded-3xl border border-stone-100 bg-stone-50 p-5">
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {recipeDetail?.author_id ? (
                          <Link to={`/user/${recipeDetail.author_id}`} onClick={onClose} className="truncate text-sm font-medium text-stone-950 hover:underline">
                            {recipeDetail.author_name || 'Usuario'}
                          </Link>
                        ) : (
                          <p className="truncate text-sm font-medium text-stone-950">{recipeDetail?.author_name || 'Usuario'}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <section className="rounded-3xl border border-stone-100 p-5">
                    ...
                    <div className="mt-4 space-y-4">
                      {steps.map((step, index) => (
                        <div key={`step-${index}`} className="rounded-2xl border border-stone-100 bg-white p-4">
