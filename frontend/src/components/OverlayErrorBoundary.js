  ...
  render() {
    ...
    return <FocusTrap focusTrapOptions={{
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <button type="button" className="absolute inset-0 bg-black/75" onClick={onClose} aria-label="Cerrar" />
        <div className="relative z-10 flex w-full max-w-sm flex-col items-center gap-4 rounded-[28px] bg-white p-8 text-center shadow-xl">
          <button type="button" onClick={onClose} className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full border border-stone-200 text-stone-500 hover:bg-stone-50" aria-label="Cerrar">
            <X className="h-4 w-4" />
          </button>
          ...
          <div>
            <p className="text-base font-semibold text-stone-950">{i18n.t('overlay_error_boundary.algoSalioMal', 'Algo salió mal')}</p>
            <p className="mt-1 text-sm text-stone-500">{i18n.t('overlay_error_boundary.noSePudoCargarEsteContenidoIntent', 'No se pudo cargar este contenido. Inténtalo de nuevo.')}</p>
          </div>
          <button type="button" onClick={onClose} className="mt-1 rounded-full bg-stone-950 px-6 py-2.5 text-sm font-medium text-white hover:bg-stone-800">
            Cerrar
          </button>
        </div>
      </div>
