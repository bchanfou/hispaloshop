    ...
    <div className="flex items-center gap-3 px-5 py-2.5">
      ...
      <Link to={`/user/${user.id}`} className="relative shrink-0">
        <div className="h-11 w-11 overflow-hidden rounded-full bg-stone-100">
          {avatarSrc ? (
            <img
              src={avatarSrc}
              srcSet={getCloudinarySrcSet(avatarSrc, [44, 88, 132])}
              sizes="44px"
              alt={user.username}
              loading="lazy"
      ...
      <Link to={`/user/${user.id}`} className="min-w-0 flex-1">
        ...
        {user.full_name && user.username ? (
          <p className="truncate text-[12px] text-stone-500">{user.full_name}</p>
        ) : null}
      </Link>
...
export default function FollowersModal({
  ...
  const handleToggleFollow = useCallback(
    (targetId, isFollowing) => {
      followMutation.mutate({ userId: targetId, isFollowing });
    },
    [followMutation],
  );
          ...
          >
            ...
            <div className="flex shrink-0 items-center px-5 pt-3">
              <div className="flex flex-1 gap-0">
                {['followers', 'following'].map((t) => (
                  <button
                    ...
                    className={`flex-1 border-b-2 pb-3 pt-1 text-[14px] transition-colors ${
                      tab === t
                        ? 'border-stone-950 font-semibold text-stone-950'
                        : 'border-transparent font-normal text-stone-400'
                    }`}
                  >
                    {t === 'followers'
                      ? `${(followersCount ?? 0).toLocaleString('es-ES')} seguidores`
                      : `${(followingCount ?? 0).toLocaleString('es-ES')} siguiendo`}
                  </button>
                ))}
              </div>
              ...
              <button
                type="button"
                onClick={onClose}
                className="ml-3 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-stone-100 text-stone-600 transition-colors hover:bg-stone-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="shrink-0 px-4 py-3">
              <div className="flex items-center gap-2 rounded-2xl bg-stone-100 px-3.5 py-2.5">
                ...
                <input
                  ...
                  placeholder="Buscar"
                  className="flex-1 border-none bg-transparent text-[14px] text-stone-950 outline-none placeholder:text-stone-400"
                />
                {search ? (
                  <button
                    type="button"
