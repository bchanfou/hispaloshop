    ...
    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
      {suggestions.slice(0, 3).map((suggestion, index) => (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => onSelect(suggestion)}
          className="flex flex-shrink-0 items-center gap-1.5 rounded-full border border-stone-200/80 bg-[rgba(255,255,255,0.88)] px-4 py-2.5 text-sm font-medium text-stone-700 shadow-[0_6px_14px_rgba(30,25,20,0.04)] transition-colors hover:border-stone-300 hover:bg-white"
        >
