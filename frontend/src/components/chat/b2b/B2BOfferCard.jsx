...
const STATUS_CONFIG = {
  draft:            { bg: 'bg-stone-50',  text: 'text-stone-500', label: 'Borrador' },
  sent:             { bg: 'bg-blue-50',   text: 'text-blue-700',  label: 'Enviada' },
  accepted:         { bg: 'bg-stone-100', text: 'text-stone-950', label: 'Aceptada' },
  rejected:         { bg: 'bg-red-50',    text: 'text-red-600',   label: 'Rechazada' },
  offer_sent:       { bg: 'bg-blue-50',   text: 'text-blue-700',  label: 'Enviada' },
  ...
  contract_pending: { bg: 'bg-blue-50',   text: 'text-blue-700',  label: 'Contrato pendiente' },
  contract_signed:  { bg: 'bg-stone-100', text: 'text-stone-950', label: 'Contrato firmado' },
  payment_pending:  { bg: 'bg-stone-50',  text: 'text-stone-500', label: 'Pago pendiente' },
  payment_confirmed:{ bg: 'bg-stone-100', text: 'text-stone-950', label: 'Pago confirmado' },
  in_transit:       { bg: 'bg-blue-50',   text: 'text-blue-700',  label: t('influencer_dashboard.enTransito', 'En tránsito') },
...
function InfoRow({ label, value, highlighted }) {
  return (
    <div className={`mb-2 ${highlighted ? 'px-1.5 py-1 bg-stone-50 rounded-md' : ''}`}>
  ...
  return (
    <div className="max-w-[280px] rounded-2xl overflow-hidden border border-stone-200">
      ...
      <div className={`bg-white p-3.5 ${hasFooter ? '' : 'rounded-b-2xl'}`}>
        ...
        {pricePerUnitStr && (
          <InfoRow label="Precio unitario" value={pricePerUnitStr} highlighted={isModified('price_per_unit')} />
        )}
        ...
        {offer.payment_terms && (
          <InfoRow label="Condiciones de pago" value={offer.payment_terms} highlighted={isModified('payment_terms')} />
        )}
        {deliveryStr && (
          <InfoRow label="Plazo de entrega" value={deliveryStr} highlighted={isModified('delivery_days')} />
        )}
      </div>

      {/* Footer */}
      {hasFooter && (
        <div className="flex gap-2 bg-white p-3 rounded-b-2xl border-t border-stone-200">
          {showReceiverActions && (
            <>
              <button
                onClick={handleAccept}
                disabled={actionLoading}
                className={`flex-1 flex items-center justify-center h-9 rounded-full text-[13px] font-medium border-none text-white ${
                  actionLoading ? 'bg-stone-500 cursor-not-allowed opacity-70' : 'bg-stone-950 cursor-pointer'
                }`}
              >
                {actionLoading ? (
              ...
              <button
                ...
                disabled={actionLoading}
                className={`flex-1 flex items-center justify-center h-9 rounded-full text-[13px] font-medium bg-white text-stone-950 border border-stone-200 ${
          ...
          {showSenderActions && (
            <button
              ...
              className="flex items-center justify-center h-9 bg-transparent text-red-600 border-none text-[13px] font-medium cursor-pointer p-0"
            >
              Cancelar oferta
            </button>
          )}
          ...
          {showAcceptedActions && (
            <button
              ...
              className="w-full flex items-center justify-center h-9 bg-stone-950 text-white border-none rounded-full text-[13px] font-medium cursor-pointer"
            >
              Ver contrato →
            </button>
          )}
