...
export default function ProductCardMessage({ product }) {
  ...
  return (
    <div className="border rounded-xl p-3 max-w-sm">
      ...
      <p className="text-sm text-stone-600">€ {((product.price_cents || 0) / 100).toFixed(2)}</p>
      {productId ? <Link to={`/products/${productId}`} className="inline-block mt-2 text-stone-950 text-sm font-medium">Comprar</Link> : null}
    </div>
  );
}
