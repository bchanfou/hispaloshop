...
function WelcomeScreen({
  ...
  return <motion.div initial={{
    ...
    duration: 0.3
      ...
      {suggestions?.length ? <SuggestionChips suggestions={suggestions} onSelect={onSuggestionClick} isEmpty /> : null}
    </motion.div>;
}
function ClearConfirmModal({
  ...
  return <motion.div initial={{
    ...
    opacity: 0
  }} className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 backdrop-blur-[2px]" onClick={onCancel}>
      <motion.div initial={{
      y: 60,
      opacity: 0
    }} animate={{
      ...
      damping: 28,
      stiffness: 320
    }} onClick={e => e.stopPropagation()} className="mx-4 mb-6 w-full max-w-sm rounded-full border border-stone-200/80 bg-[rgba(255,255,255,0.98)] p-6 shadow-[0_25px_60px_rgba(15,15,15,0.18)]">
        <p className="mb-1 text-base font-semibold text-stone-950">¿Borrar conversación?</p>
        <p className="mb-6 text-sm text-stone-500">{i18n.t('chat_container.seEliminaranTodosLosMensajesDeEsta', 'Se eliminarán todos los mensajes de esta sesión.')}</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 rounded-full border border-stone-200 py-3 text-sm font-medium text-stone-600 transition-colors hover:bg-stone-50">
            Cancelar
          </button>
          <button onClick={onConfirm} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-medium text-white transition-colors hover:bg-stone-800">
            Borrar
          </button>
        </div>
      </motion.div>
...
function ChatContainer() {
  ...
  return <div className="relative flex h-[100dvh] flex-col overflow-hidden bg-[linear-gradient(180deg,#f8f5f0_0%,#f3efe8_48%,#f1ede6_100%)]">
      ...
      <div className="sticky top-0 z-20 border-b border-stone-200/70 bg-[rgba(248,245,240,0.78)] backdrop-blur-2xl">
        <div className={`mx-auto flex max-w-3xl items-center justify-between px-4 ${isKeyboardOpen ? 'pb-2.5 pt-2.5' : 'pb-3.5 pt-3.5'}`} style={{
        paddingTop: `max(env(safe-area-inset-top), ${isKeyboardOpen ? '8px' : '14px'})`
      }}>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} aria-label="Volver" className="-ml-2 rounded-full p-2 transition-colors hover:bg-white/70">
          ...
          <div className="relative">
            <button onClick={() => setShowActions(value => !value)} aria-label="Opciones del chat" className="rounded-full p-2 transition-colors hover:bg-white/70">
            ...
            <AnimatePresence>
              {showActions ? <>
                ...
                opacity: 0
              }} className="fixed inset-0 z-10" onClick={() => setShowActions(false)} />
                  <motion.div initial={{
                opacity: 0,
                y: -6,
                scale: 0.98
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto">
        <div className={`mx-auto max-w-3xl px-4 ${isKeyboardOpen ? 'pb-2 pt-2' : 'pb-8 pt-3'}`}>
          <AnimatePresence mode="wait">
            {isWelcomeState ? <WelcomeScreen key="welcome" user={user} roleConfig={roleConfig} suggestions={suggestions} onSuggestionClick={handleSuggestionClick} /> : <motion.div key="conversation" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }}>
                ...
                {enrichedMessages.map(message => <MessageBubble key={message.id} message={message} isFirstInGroup={message.isFirstInGroup} onSwipeRight={handleSwipeRight} />)}
              </motion.div>}
          </AnimatePresence>
      ...
      }} className="absolute bottom-28 right-4 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-[radial-gradient(circle_at_30%_30%,#232323_0%,#0d0d0d_72%)] shadow-[0_18px_32px_rgba(15,15,15,0.22)]" aria-label="Ir al final">
            <ChevronDown className="h-5 w-5 text-white" />
          </motion.button> : null}
      </AnimatePresence>

      <AnimatePresence>
        {suggestions.length > 0 && !isLoading && !isWelcomeState && !isKeyboardOpen ? <motion.div initial={{
        opacity: 0,
        y: 8
      }} animate={{
        opacity: 1,
        ...
        y: 8
            ...
            <div className="mx-auto max-w-3xl px-4">
              <SuggestionChips suggestions={suggestions} onSelect={handleSuggestionClick} isEmpty={false} />
            </div>
          </motion.div> : null}
      </AnimatePresence>
      ...
      <AnimatePresence>
        ...
        y: 8
            ...
            <button type="button" onClick={() => setReplyingTo(null)} className="flex-shrink-0 rounded-full p-1 text-stone-400 hover:text-stone-700 transition-colors" aria-label="Cancelar respuesta">
              <X size={14} />
            </button>
          </motion.div> : null}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showRoleSelector ? <RoleSelector activeRole={activeRole} onSwitch={handleSwitchRole} isOpen={showRoleSelector} onClose={() => setShowRoleSelector(false)} availableRoles={availableRoles} /> : null}
      </AnimatePresence>

      <AnimatePresence>
        {showClearConfirm ? <ClearConfirmModal onConfirm={() => {
        clearChat();
        setShowClearConfirm(false);
      }} onCancel={() => setShowClearConfirm(false)} /> : null}
      </AnimatePresence>
    </div>;
}
