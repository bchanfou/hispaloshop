...
export default function EditProfileSheet({
  ...
  const hasChanges = useMemo(() => {
    ...
    const sl = profile.social_links || {};
    return draft.name !== (profile.name || '') || draft.username !== (profile.username || '') || draft.bio !== (profile.bio || '') || draft.website !== (profile.website || '') || draft.location !== (profile.location || profile.city || '') || draft.instagram !== (sl.instagram || profile.instagram || '') || draft.tiktok !== (sl.tiktok || profile.tiktok || '') || draft.youtube !== (sl.youtube || profile.youtube || '') || draft.company_name !== (profile.company_name || '') || draft.store_description !== (profile.store_description || '');
  }, [draft, profile]);
  const set = useCallback(key => e => setDraft(d => ({
    ...d,
    [key]: e.target.value
  })), []);
  const handleAvatarChange = useCallback(async e => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast.error(i18n.t('edit_profile.maximo5Mb', 'Máximo 5 MB'));
      return;
    }
    try {
  ...
  const handleSave = useCallback(() => {
    if (usernameStatus === 'taken') {
      toast.error(i18n.t('edit_profile.eseNombreDeUsuarioYaEstaEnUso', 'Ese nombre de usuario ya está en uso'));
      return;
    }
    if (usernameStatus === 'invalid') {
      toast.error(i18n.t('edit_profile.elNombreDeUsuarioDebeTenerAlMenos', 'El nombre de usuario debe tener al menos 3 caracteres'));
      return;
    }
  ...
  return <AnimatePresence>
      {isOpen ? <>
        ...
        damping: 38,
        mass: 0.8
      }} role="dialog" aria-modal="true" aria-label="Editar perfil" className="fixed bottom-0 left-0 right-0 z-[101] flex h-[95vh] flex-col overflow-hidden rounded-t-[20px] bg-white md:bottom-auto md:left-1/2 md:right-auto md:top-1/2 md:h-[85vh] md:max-h-[700px] md:w-[480px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[20px]">
            {/* Handle */}
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-stone-100 px-4 py-3.5">
              <button type="button" onClick={onClose} className="text-[14px] text-stone-500 active:opacity-50">
                Cancelar
              </button>
              <span className="text-[15px] font-semibold text-stone-950">Editar perfil</span>
              <button type="button" onClick={handleSave} disabled={isPending || usernameStatus === 'checking' || !hasChanges} className="flex items-center gap-1 text-[14px] font-semibold text-stone-950 disabled:opacity-50 active:opacity-50">
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              ...
              <div className="flex flex-col items-center py-6">
                <div className="relative">
                  <div className="h-[90px] w-[90px] overflow-hidden rounded-full bg-stone-100 ring-[2px] ring-stone-950 ring-offset-[3px] ring-offset-white">
                    {avatarSrc ? <img src={avatarSrc} srcSet={getCloudinarySrcSet(avatarSrc, [90, 180, 270])} sizes="90px" alt="Avatar" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-stone-300">
                        <Camera className="h-8 w-8" />
                      </div>}
                    {uploadingAvatar && <div className="absolute inset-0 flex items-center justify-center rounded-full bg-black/35">
              ...
              <div className="divide-y divide-stone-100">
                ...
                <div className="px-5 py-3.5">
                  ...
                  <input type="text" value={draft.name} onChange={set('name')} placeholder="Tu nombre" maxLength={50} className="mt-1 w-full bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>
                ...
                <div className="px-5 py-3.5">
                  <div className="flex items-center justify-between">
                    ...
                    {usernameStatus === 'checking' && <Loader2 className="h-3 w-3 animate-spin text-stone-400" />}
                    {usernameStatus === 'available' && <span className="flex items-center gap-1 text-[11px] font-medium text-stone-600">
                        <Check className="h-3 w-3" /> Disponible
                      </span>}
                    {usernameStatus === 'taken' && <span className="flex items-center gap-1 text-[11px] font-medium text-stone-950">
                        <X className="h-3 w-3" /> No disponible
                      </span>}
                    {usernameStatus === 'invalid' && <span className="text-[11px] font-medium text-stone-400">
                        Mín. 3 caracteres
                      </span>}
                  </div>
                  <div className="mt-1 flex items-center gap-1">
                    <span className="text-[15px] text-stone-400">@</span>
                    <input type="text" value={draft.username} onChange={set('username')} placeholder="nombre_de_usuario" maxLength={20} className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                  </div>
                </div>
                ...
                <div className="px-5 py-3.5">
                  ...
                  <textarea value={draft.bio} onChange={set('bio')} placeholder={i18n.t('edit_profile.cuentanosAlgoSobreTi…', 'Cuéntanos algo sobre ti…')} rows={3} maxLength={bioMax} className="mt-1 w-full resize-none bg-transparent text-[15px] leading-relaxed text-stone-950 outline-none placeholder:text-stone-400" />
                </div>

                {/* Enlace web */}
                <div className="flex items-center gap-2 px-5 py-3.5">
                  <Globe className="h-4 w-4 shrink-0 text-stone-400" />
                  <input type="url" value={draft.website} onChange={set('website')} placeholder="Enlace web" className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>

                {/* Ubicación */}
                <div className="flex items-center gap-2 px-5 py-3.5">
                  <MapPin className="h-4 w-4 shrink-0 text-stone-400" />
                  <input type="text" value={draft.location} onChange={set('location')} placeholder={i18n.t('store.location', 'Ubicación')} maxLength={60} className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>
              </div>
              ...
              <div className="divide-y divide-stone-100">
                ...
                <div className="flex items-center gap-2 px-5 py-3.5">
                  ...
                  <input type="text" value={draft.instagram} onChange={set('instagram')} placeholder="Usuario de Instagram" maxLength={30} className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>
                {/* TikTok */}
                <div className="flex items-center gap-2 px-5 py-3.5">
                  ...
                  <input type="text" value={draft.tiktok} onChange={set('tiktok')} placeholder="Usuario de TikTok" maxLength={30} className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>
                {/* YouTube */}
                <div className="flex items-center gap-2 px-5 py-3.5">
                  <Youtube className="h-4 w-4 shrink-0 text-stone-400" />
                  <input type="text" value={draft.youtube} onChange={set('youtube')} placeholder="Canal de YouTube" maxLength={50} className="flex-1 bg-transparent text-[15px] text-stone-950 outline-none placeholder:text-stone-400" />
                </div>
              </div>
              ...
              {isProducer && <>
                  ...
                  <div className="divide-y divide-stone-100">
                    ...
                    <div className="px-5 py-3.5">
                      <div className="flex items-center justify-between">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-stone-400">{i18n.t('edit_profile.descripcionDeTienda', 'Descripción de tienda')}</p>
