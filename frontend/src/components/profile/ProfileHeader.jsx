...
function LinkifiedBio({
  ...
  return <span>
      ...
      if (part.startsWith('@')) {
        ...
        return <a key={i} href={`/${encodeURIComponent(username)}`} className="font-medium text-stone-600 no-underline">
              {part}
            </a>;
      }
      if (part.startsWith('#')) {
        const tag = part.slice(1);
        return <a key={i} href={`/search?q=${encodeURIComponent('#' + tag)}`} className="font-medium text-stone-600 no-underline">
              {part}
            </a>;
      }
...
function SocialIcon({
  ...
  return <a href={url} target="_blank" rel="noopener noreferrer" aria-label={label} className="flex h-8 w-8 items-center justify-center rounded-full bg-stone-100 text-stone-600 transition-colors hover:bg-stone-200">
      {children}
    </a>;
}
...
function VerifiedBadge({
  ...
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className="ml-1 inline-block align-middle" aria-label="Cuenta verificada">
...
export default function ProfileHeader({
  ...
  const handleCloseAccount = useCallback(async acc => {
    ...
    try {
      ...
      toast.error(i18n.t('profile_header.noSePudoCerrarLaCuentaIntentaloD', 'No se pudo cerrar la cuenta. Intentalo de nuevo.'));
    } finally {
  ...
  const handleHighlightSaveName = useCallback(async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ['user', 'highlights', user?.user_id || user?.id]
      });
  ...
  const handleOpenCreateHighlight = useCallback(async () => {
    ...
    try {
      ...
      setArchivedStories([]);
      toast.error(i18n.t('profile_header.noSePudieronCargarLasHistorias', 'No se pudieron cargar las historias'));
    } finally {
  ...
  const toggleStorySelection = useCallback(storyId => {
    setCreateHighlightSelectedIds(prev => prev.includes(storyId) ? prev.filter(id => id !== storyId) : [...prev, storyId]);
  }, []);
  ...
  const handleSaveHighlight = useCallback(async () => {
    ...
    try {
      const firstStory = archivedStories.find(s => (s.story_id || s.id || s._id) === createHighlightSelectedIds[0]);
      ...
      toast.error(i18n.t('profile_header.errorAlCrearElDestacado', 'Error al crear el destacado'));
    } finally {
  ...
  const bioText = user?.bio && user.bio.length > 150 && !bioExpanded ? user.bio.slice(0, 150) + '...' : user?.bio;
  ...
  return <div>
      ...
      <div className="sticky top-0 z-40 flex h-[52px] items-center justify-between border-b border-stone-200 bg-white px-3">
        {isOwn ? <>
            <div className="w-10" />
            <button onClick={() => setShowAccountSwitcher(true)} aria-label="Cambiar cuenta" aria-expanded={showAccountSwitcher} aria-haspopup="dialog" className="flex items-center gap-1 text-[15px] font-semibold text-stone-950">
              @{user?.username || ''}
              <ChevronDown size={14} />
            </button>
            <div className="w-10" />
          </> : <>
            <button onClick={() => navigate(-1)} aria-label="Volver" className="flex items-center justify-center p-2.5">
              <ChevronLeft size={22} className="text-stone-950" />
            </button>
            <span className="text-[15px] font-semibold text-stone-950">
              @{user?.username || ''}
            </span>
            <button onClick={() => setShowOptionsSheet(true)} aria-label={i18n.t('common.moreOptions', 'Más opciones')} className="flex items-center justify-center p-2.5">
      ...
      <AnimatePresence>
        {showAccountSwitcher && <>
            <motion.div key="as-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setShowAccountSwitcher(false)} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="as-sheet" variants={sheetVariants} initial="hidden" animate="visible" exit="exit" role="dialog" aria-modal="true" aria-label="Cambiar cuenta" className="fixed bottom-0 left-0 right-0 z-[9999] rounded-t-2xl bg-white shadow-modal px-5 pb-8 pt-4">
              <div className="mx-auto mb-5 h-1 w-9 rounded-full bg-stone-200" />
              <div className="mb-4 text-base font-semibold">Cuentas</div>

              {accounts.map((acc, idx) => {
            const isActive = String(acc.user_id || acc.id || '') === currentUserId;
            return <div key={acc.user_id || acc.id || acc.username || idx} className="flex items-center gap-2 py-1.5">
                ...
                setShowAccountSwitcher(false);
                  ...
                  display: 'none'
                      ...
                      {acc.role && <span className="rounded-full bg-stone-100 text-stone-700 border border-stone-200 text-[10px] font-medium uppercase px-2 py-0.5">
              ...
              }} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-stone-100 text-stone-600" aria-label={isActive ? 'Cerrar sesion de esta cuenta' : 'Eliminar cuenta de este dispositivo'} title={isActive ? 'Cerrar sesion' : 'Eliminar cuenta'}>
                      <LogOut size={16} />
                    </button>
                  </div>;
          })}
            ...
            if (currentToken && user) {
              ...
              try {
                existingAccounts = JSON.parse(localStorage.getItem('hsp_accounts') || '[]');
              } catch {
                existingAccounts = [];
              }
              ...
              if (idx >= 0) existingAccounts[idx] = currentAccObj;else existingAccounts.push(currentAccObj);
      ...
      <AnimatePresence>
        {accountToClose && <>
            <motion.div key="acc-close-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => {
          if (!closingAccount) setAccountToClose(null);
        }} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="acc-close-modal" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          duration: 0.15
              ...
              <p className="mb-4 text-center text-sm text-stone-500">
                {String(accountToClose.user_id || accountToClose.id || '') === currentUserId ? hasAlternativeAccount ? 'Se cerrara la sesion y se cambiara automaticamente a otra cuenta guardada.' : i18n.t('profile_header.seCerraraLaSesionDeEstaCuentaYTe', 'Se cerrara la sesion de esta cuenta y tendras que iniciar sesion de nuevo.') : i18n.t('profile_header.soloSeEliminaDelSelectorDeCuentas', 'Solo se elimina del selector de cuentas en este dispositivo.')}
              </p>
              <div className="flex gap-3">
                <button onClick={() => setAccountToClose(null)} disabled={closingAccount} className="flex-1 rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950 disabled:opacity-60">
                  Cancelar
                </button>
                <button onClick={() => handleCloseAccount(accountToClose)} disabled={closingAccount} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-60">
      ...
      <div className="flex items-center gap-5 px-4 pt-4 pb-2 lg:items-start lg:gap-8 lg:pt-8 lg:pb-4">
        ...
        <div className="relative h-[86px] w-[86px] shrink-0 lg:h-[150px] lg:w-[150px]">
          {user?.has_active_story && <div className="absolute -inset-[3px] rounded-full" style={{
        ...
        }].map(stat => <div key={stat.label} onClick={stat.link ? () => navigate(stat.link) : undefined} role={stat.link ? 'button' : undefined} tabIndex={stat.link ? 0 : undefined} onKeyDown={stat.link ? e => {
          if (e.key === 'Enter') navigate(stat.link);
        } : undefined} className={stat.link ? 'cursor-pointer' : ''}>
      ...
      <div className="px-4 pb-3">
        ...
        <div className="mb-0.5 flex flex-wrap items-center">
          ...
          {roleLabel && <span className="ml-1.5 rounded-full bg-stone-100 text-stone-700 border border-stone-200 text-[10px] font-medium uppercase px-2 py-0.5">
              {roleLabel}
            </span>}
        </div>

        {/* bio (Q4: line breaks + linkify) */}
        {user?.bio && <motion.div layout transition={{
        duration: 0.2,
        ease: 'easeOut'
      }} className="mt-1 whitespace-pre-line text-sm leading-relaxed text-stone-950">
            <LinkifiedBio text={bioText} />
            {user.bio.length > 150 && !bioExpanded && <button onClick={() => setBioExpanded(true)} className="ml-0.5 inline-flex items-center min-h-[44px] text-[14px] font-semibold text-stone-600">
                más
              </button>}
            {user.bio.length > 150 && bioExpanded && <button onClick={() => setBioExpanded(false)} className="ml-0.5 inline-flex items-center min-h-[44px] text-[14px] font-semibold text-stone-600">
                menos
              </button>}
          </motion.div>}
        ...
        {/* producer stats (Q2) */}
        {user?.role === 'producer' && user?.seller_stats && <div className="mt-2 flex flex-wrap items-center gap-3">
            {user.seller_stats.avg_rating > 0 && <span className="flex items-center gap-1 text-xs text-stone-600">
                <Star size={12} className="fill-stone-950 text-stone-950" />
                {user.seller_stats.avg_rating} ({user.seller_stats.review_count})
              </span>}
            {user.seller_stats.total_products > 0 && <span className="flex items-center gap-1 text-xs text-stone-600">
                <Package size={12} /> {user.seller_stats.total_products} productos
              </span>}
            {user.seller_stats.total_orders > 0 && <span className="flex items-center gap-1 text-xs text-stone-600">
        ...
        {/* website */}
        {user?.website && /^https?:\/\//i.test(user.website) && <div className="mt-1 flex items-center gap-1">
        ...
        {/* location */}
        {user?.location && <div className="mt-0.5 flex items-center gap-1">
      ...
      <div className="flex flex-wrap gap-1.5 px-4 pb-3 lg:flex-nowrap lg:w-auto lg:max-w-[480px]">
        {isOwn ? <>
          ...
          damping: 20,
          stiffness: 400
        }} onClick={onEditProfile} className="min-h-[44px] flex-1 rounded-full bg-stone-100 px-2 py-1.5 text-[13px] font-semibold text-stone-950">
              Editar perfil
            </motion.button>
            <motion.button whileTap={{
          scale: 0.96
          ...
          damping: 20,
          stiffness: 400
        }} onClick={shareProfile} className="min-h-[44px] flex-1 rounded-full bg-stone-100 px-2 py-1.5 text-[13px] font-semibold text-stone-950">
              Compartir perfil
            </motion.button>
            <motion.button whileTap={{
          scale: 0.96
          ...
          damping: 20,
          stiffness: 400
        }} onClick={() => navigate('/discover/people')} aria-label="Descubrir personas" className="flex min-h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full bg-stone-100">
              <UserPlus size={16} className="text-stone-950" />
            </motion.button>
          </> : <>
            <motion.button whileTap={{
          scale: 0.95
        }} animate={user?.is_following ? {
          scale: [1, 1.1, 1]
        } : {
          scale: 1
        }} transition={{
          ...
          onFollowToggle?.();
          trigger('medium');
        }} aria-label={user?.follow_request_pending ? 'Cancelar solicitud' : user?.is_following ? `Dejar de seguir a ${user?.name}` : user?.is_private ? `Solicitar seguir a ${user?.name}` : `Seguir a ${user?.name}`} className={`min-h-[44px] flex-1 rounded-full px-3 py-1.5 text-[13px] font-semibold overflow-hidden ${user?.is_following ? 'bg-stone-100 text-stone-950' : user?.follow_request_pending ? 'bg-stone-100 text-stone-500' : 'bg-stone-950 text-white hover:bg-stone-800'}`}>
              <AnimatePresence mode="wait" initial={false}>
                <motion.span key={user?.follow_request_pending ? 'requested' : user?.is_following ? 'following' : 'follow'} initial={{
              opacity: 0,
              y: 8
            }} animate={{
              opacity: 1,
              ...
              duration: 0.15
                  ...
                  {user?.follow_request_pending ? 'Solicitado' : user?.is_following ? 'Siguiendo' : user?.is_private ? 'Solicitar' : 'Seguir'}
                </motion.span>
              </AnimatePresence>
            </motion.button>
            <motion.button whileTap={{
          ...
          damping: 20,
          stiffness: 400
        }} onClick={onMessage} aria-label="Enviar mensaje" disabled={user?.is_private && !user?.is_following} className={`flex min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-full bg-stone-100 px-3 py-1.5 text-[13px] font-semibold text-stone-950 ${user?.is_private && !user?.is_following ? 'opacity-40 cursor-not-allowed' : ''}`}>
              <MessageCircle size={15} />
              {!showStoreButton && 'Mensaje'}
            </motion.button>
            {showStoreButton && <motion.button whileTap={{
          scale: 0.96
        }} transition={{
          type: 'spring',
          damping: 20,
          stiffness: 400
        }} onClick={() => navigate(`/store/${user?.store_slug || user?.username}`)} className="flex min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-full bg-stone-100 px-3 py-1.5 text-[13px] font-semibold text-stone-950">
                <Store size={15} />
                Tienda
              </motion.button>}
            <motion.button whileTap={{
      ...
      {!isOwn && user?.mutual_followers?.length > 0 && <div className="flex items-center gap-2 px-4 pb-2">
          <div className="flex -space-x-1.5">
            {user.mutual_followers.slice(0, 3).map(mf => mf.profile_image ? <img key={mf.user_id} src={mf.profile_image} alt={mf.username} className="h-4 w-4 rounded-full border-[1.5px] border-white object-cover" onError={e => {
          e.target.style.display = 'none';
      ...
      {(isOwn || highlights.length > 0) && <div className="flex gap-4 overflow-x-auto px-4 pb-3 pt-1 scrollbar-none scrollbar-hide snap-x">
          ...
          {highlights.map((hl, i) => <motion.div key={hl.highlight_id || hl.id || i} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
      ...
      <AnimatePresence>
        {highlightMenu && isOwn && <>
            <motion.div key="hl-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => {
          setHighlightMenu(null);
          setHighlightEditMode(null);
        }} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="hl-sheet" variants={sheetVariants} initial="hidden" animate="visible" exit="exit" role="dialog" aria-modal="true" aria-label="Opciones de destacado" className="fixed bottom-0 left-0 right-0 z-[9999] rounded-t-2xl bg-white shadow-modal pb-8 pt-4" onClick={e => e.stopPropagation()}>
              ...
              {!highlightEditMode && <>
                  ...
                  <OptionRow icon={<Pencil size={20} />} label="Editar nombre" onClick={() => {
              setHighlightEditName(highlightMenu.title || '');
              setHighlightEditMode('name');
            }} />
                  <OptionRow icon={<Camera size={20} />} label="Editar portada" onClick={async () => {
              ...
              try {
                ...
                setHighlightCoverLoading(false);
                  ...
                  <OptionRow icon={<Trash2 size={20} />} label="Eliminar destacado" muted onClick={() => setHighlightEditMode('delete')} />
                </>}

              {/* Edit name inline */}
              {highlightEditMode === 'name' && <div className="px-5">
                  <p className="mb-3 text-center text-[15px] font-semibold text-stone-950">Editar nombre</p>
                  <input type="text" value={highlightEditName} onChange={e => setHighlightEditName(e.target.value.slice(0, 30))} maxLength={30} autoFocus className="w-full rounded-2xl border border-stone-200 px-4 py-3 text-sm text-stone-950 outline-none focus:border-stone-950" placeholder="Nombre del destacado" />
                  <div className="mt-4 flex gap-3">
                    <button onClick={() => setHighlightEditMode(null)} className="flex-1 rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950">
                      Cancelar
                    </button>
                    <button onClick={handleHighlightSaveName} disabled={highlightSavingName || !highlightEditName.trim()} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">
                ...
                return <button key={story.id || story.story_id || si} onClick={async () => {
                  try {
                    ...
                    toast.error('Error al actualizar portada');
                            ...
                            {storyImg ? <img src={storyImg} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center bg-stone-100 text-stone-400 text-xs">Sin imagen</div>}
                          </button>;
              })}
                    </div> : <p className="py-6 text-center text-sm text-stone-500">{i18n.t('profile_header.noHayHistoriasEnEsteDestacado', 'No hay historias en este destacado')}</p>}
                  <button onClick={() => setHighlightEditMode(null)} className="mt-4 w-full rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950">
                    Volver
                  </button>
                </div>}

              {/* Delete confirmation */}
              {highlightEditMode === 'delete' && <div className="px-5 text-center">
              ...
              }} disabled={highlightDeleting} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">
      ...
      <AnimatePresence>
        {createHighlightOpen && isOwn && <>
            <motion.div key="ch-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setCreateHighlightOpen(false)} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="ch-sheet" variants={sheetVariants} initial="hidden" animate="visible" exit="exit" role="dialog" aria-modal="true" aria-label="Crear historia destacada" className="fixed bottom-0 left-0 right-0 z-[9999] rounded-t-2xl bg-white shadow-modal pb-8 pt-4 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="mx-auto mb-4 h-1 w-9 rounded-full bg-stone-200" />

              {/* Step 1: Select stories from archive */}
              {createHighlightStep === 1 && <div className="flex flex-col flex-1 min-h-0 px-5">
                  <p className="mb-3 text-center text-[15px] font-semibold text-stone-950">
                    Seleccionar historias
                  </p>
                  {archivedStoriesLoading ? <div className="flex items-center justify-center py-10">
                      <div className="w-6 h-6 border-2 border-stone-200 border-t-stone-950 rounded-full animate-spin" />
                    </div> : archivedStories.length === 0 ? <div className="py-10 flex flex-col items-center gap-3">
                      <p className="text-sm text-stone-500 text-center">{i18n.t('profile_header.noHayHistoriasEnTuArchivo', 'No hay historias en tu archivo')}</p>
                      <p className="text-xs text-stone-400 text-center">{i18n.t('profile_header.publicaUnaHistoriaPrimeroParaPoder', 'Publica una historia primero para poder crear un destacado')}</p>
                      <button onClick={() => {
                setCreateHighlightOpen(false);
                window.dispatchEvent(new CustomEvent('open-creator', {
                  detail: {
                    mode: 'story'
                  }
                }));
                    ...
                    </div> : <div className="grid grid-cols-4 gap-2 overflow-y-auto flex-1 min-h-0 pb-3">
                ...
                return <button key={storyId || si} onClick={() => toggleStorySelection(storyId)} className={`relative aspect-square overflow-hidden rounded-2xl border-2 ${selected ? 'border-stone-950' : 'border-stone-200'}`}>
                            {storyImg ? <img src={storyImg} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center bg-stone-100 text-stone-400 text-xs">
                                Sin imagen
                              </div>}
                            {selected && <div className="absolute top-1 right-1 w-5 h-5 rounded-full bg-stone-950 flex items-center justify-center">
                  ...
                  <div className="mt-3 flex gap-3">
                    <button onClick={() => setCreateHighlightOpen(false)} className="flex-1 rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950">
                      Cancelar
                    </button>
                    <button onClick={() => setCreateHighlightStep(2)} disabled={createHighlightSelectedIds.length === 0} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">
              ...
              {createHighlightStep === 2 && <div className="px-5">
                  <p className="mb-3 text-center text-[15px] font-semibold text-stone-950">
                    Nombre del destacado
                  </p>
                  <input type="text" value={createHighlightName} onChange={e => setCreateHighlightName(e.target.value.slice(0, 30))} maxLength={30} autoFocus placeholder="Ej: Viajes, Recetas, Favoritos..." className="w-full rounded-2xl border border-stone-200 px-4 py-3 text-sm text-stone-950 outline-none focus:border-stone-950" />
                  <div className="mt-4 flex gap-3">
                    <button onClick={() => setCreateHighlightStep(1)} className="flex-1 rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950">
                      Atras
                    </button>
                    <button onClick={handleSaveHighlight} disabled={createHighlightSaving || !createHighlightName.trim()} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">
      ...
      <AnimatePresence>
        {deleteConfirmHighlight && <>
            <motion.div key="dc-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setDeleteConfirmHighlight(null)} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="dc-modal" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          duration: 0.15
              ...
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirmHighlight(null)} className="flex-1 rounded-full bg-stone-100 py-3 text-sm font-semibold text-stone-950">
                  Cancelar
                </button>
                <button onClick={confirmDeleteHighlight} disabled={highlightDeleting} className="flex-1 rounded-full bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">
                  {highlightDeleting ? 'Eliminando...' : 'Eliminar'}
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showOptionsSheet && <>
            <motion.div key="opt-overlay" variants={overlayVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setShowOptionsSheet(false)} className="fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm" />
            <motion.div key="opt-sheet" variants={sheetVariants} initial="hidden" animate="visible" exit="exit" role="dialog" aria-modal="true" aria-label="Opciones de perfil" className="fixed bottom-0 left-0 right-0 z-[9999] rounded-t-2xl bg-white shadow-modal pb-8 pt-4">
              <div className="mx-auto mb-5 h-1 w-9 rounded-full bg-stone-200" />

              {showStoreButton && <>
                  <OptionRow icon={<Store size={20} />} label="Visitar tienda" onClick={() => {
              setShowOptionsSheet(false);
              navigate(`/store/${user?.store_slug || user?.username}`);
            }} />
                  <OptionRow icon={<Package size={20} />} label="Ver productos" onClick={() => {
              setShowOptionsSheet(false);
              onSwitchTab?.('products');
            }} />
                </>}

              <OptionRow icon={<Copy size={20} />} label="Copiar enlace del perfil" onClick={() => {
            copyProfileLink();
            setShowOptionsSheet(false);
          }} />
              <OptionRow icon={<Send size={20} />} label="Compartir perfil" onClick={() => {
            shareProfile();
            setShowOptionsSheet(false);
          }} />
            ...
            try {
              ...
              queryClient.invalidateQueries({
                queryKey: ['userProfile', String(user?.user_id)]
              });
              navigate(-1);
            } catch {
            ...
            try {
              await apiClient.post(`/users/${user?.user_id}/report`, {
...
function OptionRow({
  ...
  return <button type="button" onClick={onClick} aria-label={typeof label === 'string' ? label : undefined} className={`flex w-full items-center gap-3 px-5 py-3.5 min-h-[44px] text-left text-[15px] font-medium transition-all duration-150 hover:bg-stone-50 ${muted ? 'text-stone-500' : 'text-stone-950'}`}>
      {icon}
      {label}
    </button>;
}