...
import { X, Bookmark, Activity, Package, HelpCircle, FileText, Globe as GlobeIcon, LayoutDashboard, Settings, LogOut, ChevronDown, Check, User, Store, Users, ChefHat, MessageCircle, Info, Megaphone, Truck, Globe2, ShieldCheck } from 'lucide-react';
...
export default function HamburgerMenu({
  ...
  const LANGUAGES = Object.entries(locale.languages || {}).map(([code, data]) => ({
    code,
    name: data.native || data.name || code
  }));
  ...
  return createPortal(<AnimatePresence>
      {isOpen && <>
          {/* Overlay */}
          <motion.div key="menu-overlay" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        ...
        duration: 0.2
      }} onClick={onClose} className="fixed inset-0 z-[9998] bg-stone-950/50 backdrop-blur-sm" />

          {/* Drawer — enters from RIGHT */}
          <motion.aside key="menu-drawer" initial={{
        x: '100%'
      }} animate={{
        x: 0
      }} exit={{
        ...
        duration: 0.3
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-stone-200">
              ...
              <button onClick={onClose} aria-label={i18n.t('hamburger_menu.cerrarMenu', 'Cerrar menú')} className="flex items-center justify-center w-8 h-8 rounded-full border-none bg-stone-100 cursor-pointer">
            ...
            <div className="flex-1 py-2">
              ...
              {user ? <>
                  ...
                  <Link to={profileUsername ? `/${profileUsername}` : profileUserId ? `/profile/${profileUserId}` : '/profile'} onClick={onClose} className="flex items-center gap-3 px-5 py-3 no-underline transition-colors hover:bg-stone-50 active:scale-[0.98]">
                    {profileImage ? <img src={profileImage} alt="" className="w-10 h-10 rounded-full object-cover border border-stone-200" /> : <div className="w-10 h-10 rounded-full bg-stone-950 text-white flex items-center justify-center text-base font-bold">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {username && <p className="text-xs text-stone-500 mt-px mb-0">
                          @{username}
                        </p>}
                    </div>
                    {user.plan && user.plan !== 'free' && <span className={`text-[9px] font-bold text-white rounded-full py-0.5 px-2 uppercase ${user.plan === 'elite' ? 'bg-stone-950' : 'bg-stone-500'}`}>
                        {user.plan}
                      </span>}
                    <ChevronDown size={16} className="text-stone-400 -rotate-90" />
                  </Link>

                  <MenuItem to="/settings" icon={<Settings size={20} />} label={i18n.t('community.configuracion', 'Configuración')} onClose={onClose} />
              ...
              <MenuItem to="/saved" icon={<Bookmark size={20} />} label="Guardados" onClose={onClose}>
                {wishlistCount > 0 && <span className="rounded-full bg-stone-950 text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center px-1 font-semibold ml-auto">
                    {wishlistCount}
                  </span>}
              </MenuItem>
              ...
              {/* País */}
              <AccordionRow icon={<GlobeIcon size={20} />} label="País" value={`${currentCountry.flag} ${currentCountry.name}`} isOpen={openAccordion === 'country'} onToggle={() => {
            setLocaleSearch('');
            setOpenAccordion(openAccordion === 'country' ? null : 'country');
          }}>
                <input type="text" placeholder={i18n.t('fiscal_setup.buscarPais', 'Buscar país...')} value={localeSearch} onChange={e => setLocaleSearch(e.target.value)} className="w-full px-3 py-2 mb-2 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-stone-400" />
                ...
                return <AccordionOption key={c.code} label={`${c.flag} ${c.name}`} isActive={isActive} onClick={() => {
              ...
              {/* Idioma */}
              <AccordionRow icon={<span className="text-lg w-5 text-center">🗣️</span>} label="Idioma" value={currentLang.name} isOpen={openAccordion === 'language'} onToggle={() => {
            setLocaleSearch('');
            setOpenAccordion(openAccordion === 'language' ? null : 'language');
          }}>
                <input type="text" placeholder="Buscar idioma..." value={localeSearch} onChange={e => setLocaleSearch(e.target.value)} className="w-full px-3 py-2 mb-2 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-stone-400" />
                <div className="max-h-60 overflow-y-auto">
                ...
                return <AccordionOption key={l.code} label={`${l.code.toUpperCase()} — ${l.name}`} isActive={isActive} onClick={() => {
              ...
              {/* Divisa */}
              <AccordionRow icon={<span className="text-lg w-5 text-center">💱</span>} label="Divisa" value={currentCurrency.code} isOpen={openAccordion === 'currency'} onToggle={() => {
            setLocaleSearch('');
            setOpenAccordion(openAccordion === 'currency' ? null : 'currency');
          }}>
                <input type="text" placeholder="Buscar divisa..." value={localeSearch} onChange={e => setLocaleSearch(e.target.value)} className="w-full px-3 py-2 mb-2 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-stone-400" />
                <div className="max-h-60 overflow-y-auto">
                ...
                return <AccordionOption key={c.code} label={`${c.symbol} ${c.code} — ${c.name}`} isActive={isActive} onClick={() => {
              ...
              <MenuItem to="/terms" icon={<FileText size={20} />} label={i18n.t('register.terminosYCondiciones', 'Términos y condiciones')} onClose={onClose} />

              <Divider />

              {/* ── HISPALOSHOP ── */}
              <SectionLabel>HISPALOSHOP</SectionLabel>
              <MenuItem to="/que-es" icon={<Info size={20} />} label={i18n.t('hamburger_menu.queEsHispaloshop', '¿Qué es HispaloShop?')} onClose={onClose} />
              ...
              {user && <>
                  <Divider />
                  <button onClick={handleLogout} className="flex items-center gap-3 w-full px-5 py-3.5 border-none bg-transparent text-[15px] font-medium text-stone-500 cursor-pointer text-left hover:text-stone-950 transition-colors">
                    <LogOut size={20} />
                    Cerrar sesión
                  </button>
                </>}
...
function SectionLabel({
  ...
  return <p className="px-5 pt-3 pb-1 text-[11px] font-semibold text-stone-500 uppercase tracking-wide m-0">
      {children}
    </p>;
}
function MenuItem({
  ...
  return <Link to={to} onClick={onClose} className="flex items-center gap-3 px-5 py-3.5 no-underline text-[15px] font-medium text-stone-950 transition-colors hover:bg-stone-100 active:bg-stone-200">
...
function AccordionRow({
  ...
  return <div>
      <button type="button" onClick={onToggle} className="flex items-center gap-3 w-full px-5 py-3.5 border-none bg-transparent cursor-pointer text-left">
...
function AccordionOption({
  ...
  return <button type="button" onClick={disabled ? undefined : onClick} disabled={disabled} className={`flex items-center gap-2 w-full px-4 py-2.5 border-none bg-transparent text-left text-sm ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer opacity-100'} ${isActive ? 'text-stone-950 font-semibold' : 'text-stone-500 font-normal'}`}>
      <span className="flex-1">{label}</span>
      {badge && <span className={`text-[9px] font-semibold py-px px-1.5 rounded-full ${badgeVariant === 'dark' ? 'bg-stone-950 text-white' : 'bg-stone-200 text-stone-500'}`}>
          {badge}
        </span>}
