...
function SearchPanel({
  ...
  const doSearch = useCallback(async q => {
    ...
    try {
      ...
      const [productsRes, usersRes] = await Promise.allSettled([apiClient.get('/products', {
        params: {
          search: q,
          limit: 5
        }
      ...
      const products = (productsRes.status === 'fulfilled' ? productsRes.value?.products || productsRes.value?.data || [] : []).map(p => ({
        ...p,
        _type: 'product'
      }));
  ...
  return <>
      ...
      {/* Panel */}
      <div className="fixed top-0 z-[46] flex flex-col bg-white border-r border-stone-200 shadow-xl transition-transform duration-200 ease-out" style={{
      left: '220px',
      width: 380,
      height: '100vh',
      transform: open ? 'translateX(0)' : 'translateX(-100%)'
        ...
        <div className="px-4 py-3">
          <div className="relative">
            ...
            <input ref={inputRef} value={query} onChange={handleInput} placeholder="Buscar productos, personas, tiendas..." data-search-input className="w-full h-10 pl-9 pr-3 bg-stone-100 border-none rounded-2xl text-sm text-stone-950 outline-none focus:ring-2 focus:ring-stone-300 font-sans placeholder:text-stone-400" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2">
          ...
          {!loading && query && results.length === 0 && <p className="text-center text-sm text-stone-400 py-8">Sin resultados para "{query}"</p>}

          {!loading && results.map((item, i) => {
          if (item._type === 'user') {
            const username = item.username || item.user_id;
            return <button key={`u-${item.user_id || i}`} onClick={() => goTo(`/${username}`, item.name || username)} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-2xl hover:bg-stone-50 text-left bg-transparent border-none cursor-pointer transition-colors">
                  {item.profile_image ? <img src={item.profile_image} alt="" className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full bg-stone-200 flex items-center justify-center text-xs font-bold text-stone-600">
          ...
          // Product
          return <button key={`p-${item.product_id || i}`} onClick={() => goTo(`/products/${item.product_id}`, item.name)} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-2xl hover:bg-stone-50 text-left bg-transparent border-none cursor-pointer transition-colors">
                {item.images?.[0] ? <img src={item.images[0]} alt="" className="h-10 w-10 rounded-2xl object-cover" /> : <div className="h-10 w-10 rounded-2xl bg-stone-100 flex items-center justify-center">
...
function MoreDropdown({
  ...
  const items = [{
    icon: Settings,
    label: i18n.t('community.configuracion', 'Configuración'),
    ...
    icon: LogOut,
    label: i18n.t('common.logout', 'Cerrar sesión'),
    action: onLogout,
    danger: true
  }];
  return <div ref={dropdownRef} className="absolute bottom-full left-3 right-3 mb-2 bg-white rounded-2xl border border-stone-200 shadow-lg overflow-hidden z-50">
      {items.map((item, i) => {
      if (item.divider) {
      ...
      return <button key={item.label} onClick={() => {
        item.action();
        onClose();
      }} className={`flex items-center gap-3 w-full px-4 py-3 text-left text-sm bg-transparent border-none cursor-pointer font-sans transition-colors hover:bg-stone-50 ${item.danger ? 'text-stone-950' : 'text-stone-700'}`}>
...
export default function SideNav() {
  ...
  return <>
      <aside className="fixed left-0 top-0 bottom-0 z-40 hidden lg:flex w-[220px] flex-col border-r bg-white" style={{
      borderColor: '#e7e5e4'
    }}>
        {/* Logo */}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-1 space-y-0.5">
          ...
          const Icon = item.icon;
          if (isSearch) {
            return <button key={item.key} type="button" onClick={() => setSearchOpen(v => !v)} className={`group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-[14px] transition-colors bg-transparent border-none cursor-pointer font-sans ${active ? 'font-semibold text-stone-950' : 'text-stone-700 hover:bg-stone-50'}`}>
                  <Icon className="h-5 w-5 flex-shrink-0" strokeWidth={active ? 2.2 : 1.6} />
                  <span>{t(item.labelKey, item.fallback)}</span>
                </button>;
          }
          return <Link key={item.key} to={item.to} className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-[14px] transition-colors ${active ? 'font-semibold text-stone-950' : 'text-stone-700 hover:bg-stone-50'}`}>
                <div className="relative flex-shrink-0">
                  <Icon className="h-5 w-5" strokeWidth={active ? 2.2 : 1.6} />
                  {item.key === 'notifications' && unreadCount > 0 && <span className="absolute -right-1 -top-1 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-stone-950 px-0.5 text-[8px] font-bold text-white">
          ...
          {/* Cart */}
          <Link to="/cart" className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-[14px] transition-colors ${isActive('/cart') ? 'font-semibold text-stone-950' : 'text-stone-700 hover:bg-stone-50'}`}>
            <div className="relative flex-shrink-0">
              <ShoppingCart className="h-5 w-5" strokeWidth={isActive('/cart') ? 2.2 : 1.6} />
              {totalCartItems > 0 && <span className="absolute -right-1 -top-1 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-stone-950 px-0.5 text-[8px] font-bold text-white">
          ...
          <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('open-creator'))} className="flex w-full items-center justify-center gap-2 mt-3 bg-stone-950 text-white rounded-full py-2.5 px-4 text-[14px] font-semibold border-none cursor-pointer transition-colors hover:bg-stone-800 font-sans">
        ...
        <div className="px-3 pb-1">
          <Link to={profileUrl} className={`flex items-center gap-3 rounded-xl px-3 py-2 text-[14px] transition-colors ${isActive(profileUrl) ? 'font-semibold text-stone-950' : 'text-stone-700 hover:bg-stone-50'}`}>
            {profileImage ? <img src={profileImage} alt="" className="h-7 w-7 rounded-full object-cover ring-1 ring-stone-200 flex-shrink-0" /> : <div className="h-7 w-7 rounded-full bg-stone-200 flex items-center justify-center flex-shrink-0">
        ...
        <div className="relative px-3 pb-3">
          <button type="button" onClick={() => setMoreOpen(v => !v)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-[14px] transition-colors bg-transparent border-none cursor-pointer font-sans ${moreOpen ? 'bg-stone-100 text-stone-950' : 'text-stone-700 hover:bg-stone-50'}`}>
      ...
      {searchOpen && <div className="hidden lg:block">
