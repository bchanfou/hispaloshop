  ...
  return (
    <div className={`media-uploader ${className}`}>
      ...
      <div
        ...
        onDrop={handleDrop}
        style={{
          border: `2px dashed ${dragging ? 'var(--color-blue)' : error ? 'var(--color-red)' : 'var(--color-border)'}`,
          borderRadius: 'var(--radius-md)',
          padding: '24px',
          textAlign: 'center',
          cursor: disabled ? 'not-allowed' : 'pointer',
          background: dragging ? 'var(--color-blue-light)' : 'var(--color-cream)',
      ...
      >
        ...
        {uploading && (
          <div style={{ width: '100%' }}>
            <div style={{ height: '6px', background: 'var(--color-surface)', borderRadius: '3px', overflow: 'hidden', marginBottom: '8px' }}>
              <div
                style={{
                  height: '100%',
                  width: `${progress}%`,
            ...
            <p style={{ fontSize: '13px', color: 'var(--color-stone)', margin: 0 }}>Subiendo... {progress}%</p>
          </div>
        )}
        ...
        {!(preview && previewSrc) && !uploading && (
          <>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={dragging ? 'var(--color-blue)' : 'var(--color-stone)'} strokeWidth="1.5">
              <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" strokeLinecap="round" strokeLinejoin="round"/>
              <polyline points="17 8 12 3 7 8" strokeLinecap="round" strokeLinejoin="round"/>
              <line x1="12" y1="3" x2="12" y2="15" strokeLinecap="round"/>
            </svg>
            <p style={{ fontSize: '14px', color: 'var(--color-stone)', margin: 0 }}>
              {label || 'Arrastra un archivo o haz clic para seleccionar'}
            </p>
            <p style={{ fontSize: '12px', color: 'var(--color-stone)', margin: 0 }}>
      ...
      {/* Error message */}
      {error && (
        <p style={{ fontSize: '13px', color: 'var(--color-red)', marginTop: '6px', marginBottom: 0 }}>
          {error}
        </p>
      )}
      ...
      {progress === 100 && !uploading && (
        <button
          ...
          style={{
            ...
            fontSize: '12px', color: 'var(--color-stone)', marginTop: '4px', padding: 0,
          }}
        >
          Cambiar archivo
        </button>
