...
export default function FollowStep({
  ...
  const handleFinish = async () => {
    if (suggestions.length > 0 && selected.length < 3) {
      onError?.(i18n.t('follow.sigueAlMenos3CuentasParaCompletar', 'Sigue al menos 3 cuentas para completar el onboarding.'));
      return;
    }
    setSaving(true);
    try {
      ...
      onError?.(error?.response?.data?.detail || i18n.t('follow.noHemosPodidoGuardarLasCuentasSegu', 'No hemos podido guardar las cuentas seguidas todavía.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return <div className="space-y-6">
      ...
      {suggestions.length === 0 ? <div className="rounded-2xl border border-stone-200 bg-stone-50 p-6 text-center">
          <p className="text-stone-700">No hay sugerencias disponibles ahora mismo.</p>
          <p className="mt-2 text-sm text-stone-500">{i18n.t('follow.puedesFinalizarYEmpezarADescubrirC', 'Puedes finalizar y empezar a descubrir cuentas más tarde.')}</p>
        </div> : <div className="space-y-3">
          {suggestions.map(user => {
        const active = selected.includes(user.user_id);
        return <button key={user.user_id} type="button" onClick={() => toggleUser(user.user_id)} className={`flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition-colors ${active ? 'border-stone-950 bg-stone-100' : 'border-stone-200 bg-white hover:border-stone-200'}`}>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <button type="button" onClick={handleFinish} disabled={saving} className="rounded-full bg-stone-950 px-6 py-3 font-medium text-white transition-colors hover:bg-black disabled:cursor-not-allowed disabled:bg-stone-300">
