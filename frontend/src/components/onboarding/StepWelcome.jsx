...
export default function StepWelcome({
  ...
  const options = [{
    ...
    description: i18n.t('step_welcome.anadeMasInformacionSobreTi', 'Añade más información sobre ti'),
  ...
  return <div className="space-y-6 text-center">
      ...
      <div className="space-y-3">
        {options.map((option, index) => <motion.button key={option.title} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        delay: index * 0.1
      }} onClick={option.action} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-stone-200 hover:border-stone-950 transition-colors text-left">
