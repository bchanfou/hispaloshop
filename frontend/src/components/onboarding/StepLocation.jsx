    ...
    navigator.geolocation.getCurrentPosition(async position => {
      try {
        ...
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json&accept-language=es`, {
          headers: {
            'User-Agent': 'HispaloShop/1.0 (https://hispaloshop.com)'
          }
        });
        ...
        try {
          localStorage.setItem('hsp_user_coords', JSON.stringify({
            lat: latitude,
            lng: longitude
          }));
        } catch {/* ignore */}
        ...
        setZipCode('28001');
        setCity('Madrid');
        toast('No pudimos detectar tu ubicación. Puedes introducirla manualmente.', {
          icon: '📍'
        });
      } finally {
      ...
      if (err.code === 1) {
        ...
        setZipCode('28001');
        setCity('Madrid');
        toast('No pudimos obtener tu ubicación. Hemos puesto Madrid por defecto.', {
          icon: '📍'
        });
      }
    }, {
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="relative">
            ...
            <input type="text" value={zipCode} onChange={handleZipChange} placeholder="41001" maxLength={5} className="w-full pl-10 pr-4 py-3 border border-stone-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-stone-950" />
          </div>
        </div>

        {city && <div className="p-3 bg-stone-100 rounded-2xl">
      ...
      <div className="flex justify-between pt-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => onNext()} className="px-6 py-3 text-stone-500 hover:text-stone-950 font-medium">
            Omitir
          </button>
          <button onClick={handleNext} className="px-6 py-3 bg-stone-950 text-white rounded-2xl font-medium hover:bg-stone-800 transition-colors">
            Siguiente
          </button>
        </div>
      </div>
