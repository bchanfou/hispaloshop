...
const quoteSchema = z.object({
  ...
  target_country: z.string().length(2, "Código de país debe tener 2 letras").transform(v => v.toUpperCase()),
...
export default function QuoteBuilder({
  ...
  const onSubmit = async data => {
    const requestedLines = data.rows.map(row => `- ${row.product_id}: ${row.qty_requested} uds`).join('\n');
    try {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 rounded-2xl border border-stone-200 bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input {...register('producer_id')} placeholder="prod_abc123" className={`w-full px-3 py-2 rounded-2xl border text-sm focus:outline-none focus:ring-2 focus:ring-stone-500 ${errors.producer_id ? 'border-stone-400' : 'border-stone-200'}`} />
        ...
        <div>
          ...
          <Controller control={control} name="target_country" render={({
          field
        }) => <select {...field} className={`w-full px-3 py-2 rounded-2xl border text-sm bg-white focus:outline-none focus:ring-2 focus:ring-stone-500 ${errors.target_country ? 'border-stone-400' : 'border-stone-200'}`}>
      ...
      <div>
        ...
        <div className="space-y-2">
          {fields.map((field, idx) => <div key={field.id} className="flex gap-2 items-start">
              <div className="flex-1">
                <input {...register(`rows.${idx}.product_id`)} placeholder="ID del producto" className={`w-full px-3 py-2 rounded-2xl border text-sm focus:outline-none focus:ring-2 focus:ring-stone-500 ${errors.rows?.[idx]?.product_id ? 'border-stone-400' : 'border-stone-200'}`} />
                <FieldError message={errors.rows?.[idx]?.product_id?.message} />
              </div>
              <div className="w-28 shrink-0">
                <input {...register(`rows.${idx}.qty_requested`)} type="number" min="1" placeholder="Cant." className={`w-full px-3 py-2 rounded-2xl border text-sm focus:outline-none focus:ring-2 focus:ring-stone-500 ${errors.rows?.[idx]?.qty_requested ? 'border-stone-400' : 'border-stone-200'}`} />
                <FieldError message={errors.rows?.[idx]?.qty_requested?.message} />
              </div>
              {fields.length > 1 && <button type="button" onClick={() => remove(idx)} className="mt-1 p-2 text-stone-600 hover:bg-stone-50 rounded-2xl transition-colors">
      ...
      {/* Submit */}
      <div className="pt-1">
        <button type="submit" disabled={isSubmitting || createInquiry.isPending} className="flex items-center gap-2 px-5 py-2.5 bg-stone-950 text-white rounded-2xl text-sm font-medium hover:bg-stone-800 disabled:opacity-60 transition-colors">
          {isSubmitting || createInquiry.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
