...
export default function ImporterProfileForm({ onSubmit }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <input className="w-full border p-2 rounded" placeholder={t('importer_profile_form.paisOrigenEs', 'País origen (ES)')} value={form.country_origin} onChange={(e) => setForm({ ...form, country_origin: e.target.value.toUpperCase() })} />
