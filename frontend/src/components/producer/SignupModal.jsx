...
export default function SignupModal({
  ...
  const validateStep = targetStep => {
    ...
    if (targetStep === 3) {
      ...
      if (isPaidPlan(selectedPlan)) {
        if (stripeLoading) nextErrors.card = 'Estamos preparando Stripe. Espera un segundo.';else if (stripeError) nextErrors.card = stripeError;else if (!cardReady || !cardComplete) nextErrors.card = t('signup.completaLosDatosDeLaTarjetaParaCo', 'Completa los datos de la tarjeta para continuar.');
      }
    }
    setErrors(current => ({
      ...current,
  ...
  const renderStepOne = () => <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="relative">
        ...
        ...current,
        password: true
      }))} placeholder={t('common.minCharacters', 'Mínimo 6 caracteres')} className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-12 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.password), touched.password && validity.password)}`} />
  ...
  const renderStepTwo = () => <div className="space-y-6">
      <div className="relative">
        ...
        ...current,
        brandName: true
      }))} placeholder={t('signup.nombreDeTuObradorMarcaOCooperativ', 'Nombre de tu obrador, marca o cooperativa')} className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-10 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.brandName), touched.brandName && validity.brandName)}`} />
            ...
            <div className="mx-auto flex h-full max-w-7xl flex-col gap-8 px-4 py-6 sm:px-6 lg:flex-row lg:gap-12 lg:px-10 lg:py-10">
              <aside className="w-full rounded-[28px] bg-hs-black p-6 text-white shadow-[0_24px_70px_-36px_rgba(44,36,27,0.6)] lg:max-w-[360px]">
                ...
                <p className="mt-4 text-sm leading-7 text-white/78">
                  {successState ? t('signup.tuCuentaYaExisteYElSiguientePaso', 'Tu cuenta ya existe y el siguiente paso es verificar el email y terminar de preparar tu perfil.') : activePlan.summary}
                </p>
                  ...
                  ease: 'easeOut'
                      ...
                      <div className="mt-10 flex flex-col gap-4 border-t border-stone-200 pt-6 lg:flex-row lg:items-center lg:justify-between">
                        <div className="text-sm leading-7 text-stone-500">
                          {step === 1 ? t('signup.empezamosPorLoMinimoParaNoPonerte', 'Empezamos por lo mínimo para no ponerte otra barrera más.') : step === 2 ? 'Esto nos ayuda a entender tu realidad y orientarte mejor.' : t('signup.estamosAUnPasoDeAbrirTuTiendaYD', 'Estamos a un paso de abrir tu tienda y dejar la infraestructura preparada.')}
                        </div>
