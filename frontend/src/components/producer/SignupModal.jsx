...
import { ArrowLeft, ArrowRight, CheckCircle2, CircleCheckBig, CreditCard, Eye, EyeOff, Loader2, LockKeyhole, Mail, MapPinned, Phone, ShieldCheck, Sparkles, Store, UserRound, Wheat } from 'lucide-react';
...
export const PRODUCER_PLANS = {
  free: {
    ...
    features: ["Tienda virtual personalizada con tu historia y origen", "Hasta 30 productos en catálogo", 'Acceso a comunidad de productores', "Visibilidad nacional en España", 'Comisión del 20% solo sobre ventas realizadas']
  },
  pro: {
    key: 'pro',
    name: 'PRO',
  ...
  elite: {
    ...
    features: ['Todo lo del PRO', 'Pedro AI — Tu socio comercial internacional', "Predicción de demanda por país y temporada", "Análisis de mercados internacionales (precio, competencia, oportunidad)", "Detección preventiva de riesgo de desabastecimiento", "Matching directo con importadores verificados de otros países", "Generación automática de contratos B2B en PDF", "Análisis de regulaciones y aranceles por mercado destino", "Creación de dossieres de exportación", "Prioridad absoluta de visibilidad en la plataforma", 'Comisión reducida al 17%', "Soporte telefónico directo"]
  }
};
...
const PRODUCT_TYPES = ['Conservas', 'Aceites', 'Embutidos', 'Quesos', "Panadería", 'Bebidas', "Orgánico", 'Otros'];
const EXPORT_OPTIONS = ["Solo vendo en España", "Ya exporto a algún país", 'Quiero empezar a exportar'];
const REFERENCE_OPTIONS = ['1-5 productos', '6-20', "Más de 20"];
const REGIONS = ["Andalucía", "Aragón", 'Asturias', 'Baleares', 'Canarias', 'Cantabria', 'Castilla-La Mancha', "Castilla y León", "Cataluña", 'Ceuta', 'Comunidad Valenciana', 'Extremadura', 'Galicia', 'La Rioja', 'Madrid', 'Melilla', 'Murcia', 'Navarra', "País Vasco"];
let stripeLoader;
...
function loadStripeJs() {
  if (typeof window === 'undefined') {
    return Promise.reject(new Error(i18n.t('signup.stripeNoEstaDisponible', 'Stripe no está disponible.')));
  }
  if (window.Stripe) {
...
function PlanPill({
  ...
  return <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${selected ? accent : 'bg-stone-100 text-stone-600'}`}>
      {label}
    </span>;
}
  ...
  return <div>
      ...
      <div className="relative mt-4">
        ...
        <div className="relative flex items-center justify-between">
          ...
          const label = point === 1 ? i18n.t('signup.quienEres', 'Quién eres') : point === 2 ? 'Tu producción' : 'Confirma tu plan';
          return <div key={label} className="flex flex-col items-center gap-2">
...
export default function SignupModal({
    ...
    const mountCard = async () => {
      try {
        ...
        if (!key) {
          ...
          if (!key) {
            throw new Error(i18n.t('signup.stripeNoEstaConfiguradoTodavia', 'Stripe no está configurado todavía.'));
          }
          if (active) {
            setStripeKey(key);
          }
        ...
        if (active) {
          setStripeError(error.message || i18n.t('signup.noPudimosPrepararElPagoSeguro', 'No pudimos preparar el pago seguro.'));
        }
      } finally {
        if (active) {
  ...
  const validateStep = targetStep => {
    const nextErrors = {};
    if (targetStep === 1) {
      if (!validity.fullName) nextErrors.fullName = i18n.t('signup.necesitamosTuNombreParaDarteDeAlta', 'Necesitamos tu nombre para darte de alta.');
      if (!validity.email) nextErrors.email = i18n.t('signup.introduceUnEmailValido', 'Introduce un email válido.');
      if (!validity.phoneNumber) nextErrors.phoneNumber = i18n.t('signup.necesitamosUnTelefonoOperativo', 'Necesitamos un teléfono operativo.');
      if (!validity.password) nextErrors.password = i18n.t('signup.tuContrasenaDebeTenerAlMenos6Cara', 'Tu contraseña debe tener al menos 6 caracteres.');
      if (!formData.acceptLegal) nextErrors.acceptLegal = i18n.t('signup.debesAceptarLaPoliticaDePrivacidad', 'Debes aceptar la política de privacidad y los términos.');
      if (Object.keys(nextErrors).length > 0) {
        markTouched(['fullName', 'email', 'phoneNumber', 'password', 'acceptLegal']);
      }
    }
    if (targetStep === 2) {
      if (!validity.brandName) nextErrors.brandName = i18n.t('signup.cuentanosComoSeLlamaTuMarcaOCoope', 'Cuéntanos cómo se llama tu marca o cooperativa.');
      if (!validity.productTypes) nextErrors.productTypes = i18n.t('signup.seleccionaAlMenosUnTipoDeProducto', 'Selecciona al menos un tipo de producto.');
      if (!validity.exportStage) nextErrors.exportStage = i18n.t('signup.indicanosSiYaExportasOQuieresEmpez', 'Indícanos si ya exportas o quieres empezar.');
      if (!validity.region) nextErrors.region = i18n.t('signup.seleccionaTuRegionDeProduccion', 'Selecciona tu región de producción.');
      if (!validity.references) nextErrors.references = i18n.t('signup.necesitamosUnRangoAproximadoDeRefer', 'Necesitamos un rango aproximado de referencias.');
      if (Object.keys(nextErrors).length > 0) {
    ...
    if (targetStep === 3) {
      ...
      if (isPaidPlan(selectedPlan)) {
        if (stripeLoading) nextErrors.card = 'Estamos preparando Stripe. Espera un segundo.';else if (stripeError) nextErrors.card = stripeError;else if (!cardReady || !cardComplete) nextErrors.card = i18n.t('signup.completaLosDatosDeLaTarjetaParaCo', 'Completa los datos de la tarjeta para continuar.');
      }
    }
    setErrors(current => ({
      ...current,
  ...
  const subscribeInline = async () => {
    ...
    if (!stripe || !card) {
      throw new Error(i18n.t('signup.stripeNoEstaListoTodavia', 'Stripe no está listo todavía.'));
    }
    ...
    if (paymentMethodResult.error) {
      throw new Error(paymentMethodResult.error.message || i18n.t('signup.noPudimosValidarLaTarjeta', 'No pudimos validar la tarjeta.'));
    }
    ...
    if (subscribeData?.requires_action && subscribeData?.client_secret) {
      ...
      if (confirmation.error) {
        if (confirmation.error.type === 'card_error' || confirmation.error.type === 'validation_error') {
          throw new Error(confirmation.error.message || i18n.t('signup.tuBancoRechazoLaOperacion', 'Tu banco rechazó la operación.'));
        }
        throw new Error(i18n.t('signup.haOcurridoUnErrorConElPagoPorFa', 'Ha ocurrido un error con el pago. Por favor inténtalo de nuevo.'));
      }
      if (confirmation.paymentIntent?.status === 'requires_payment_method') {
        throw new Error(i18n.t('signup.elMetodoDePagoFueRechazadoPorFav', 'El método de pago fue rechazado. Por favor usa otra tarjeta.'));
      }
  ...
  const submitFlow = async () => {
    if (!validateStep(3)) {
      toast.error(i18n.t('signup.revisaLosDatosAntesDeContinuar', 'Revisa los datos antes de continuar.'));
      return;
    }
    setSubmitting(true);
    setPaymentNotice('');
    try {
      ...
      const message = error.message || i18n.t('signup.noPudimosCompletarElAltaDelProduct', 'No pudimos completar el alta del productor.');
      if (message.toLowerCase().includes('email already registered')) {
        setPaymentNotice(i18n.t('signup.tuCuentaYaExisteSiYaSeCreoSolo', 'Tu cuenta ya existe. Si ya se creo, solo falta terminar el plan o revisar tu acceso.'));
      }
      toast.error(message);
    } finally {
      setSubmitting(false);
  ...
  const proceed = () => {
    if (!validateStep(step)) {
      toast.error(i18n.t('signup.completaLosCamposObligatoriosParaAv', 'Completa los campos obligatorios para avanzar.'));
      return;
    }
  ...
  const renderStepOne = () => <div className="grid gap-5 lg:grid-cols-2">
      <div className="relative lg:col-span-2">
        ...
        <input id="producer-full-name" ref={firstRef} value={formData.fullName} onChange={event => updateField('fullName', event.target.value)} onBlur={() => setTouched(current => ({
        ...current,
        fullName: true
      }))} placeholder="Tu nombre y apellidos" className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-10 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.fullName), touched.fullName && validity.fullName)}`} />
      ...
      <div className="relative">
        ...
        <input id="producer-email" type="email" value={formData.email} onChange={event => updateField('email', event.target.value)} onBlur={() => setTouched(current => ({
        ...current,
        email: true
      }))} placeholder="tu@obrador.com" className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-10 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.email), touched.email && validity.email)}`} />
      ...
      <div>
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-[128px_1fr]">
          <select value={formData.phonePrefix} onChange={event => updateField('phonePrefix', event.target.value)} className="h-12 rounded-[10px] border border-stone-200 bg-white px-3 text-sm font-semibold text-stone-700 shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] focus:outline-none focus:border-stone-950">
            {PHONE_PREFIXES.map(prefix => <option key={prefix.value} value={prefix.value}>
                {prefix.label}
              </option>)}
          </select>
          <div className="relative">
            <Phone className="pointer-events-none absolute left-4 top-[15px] h-4 w-4 text-stone-400" />
            <input id="producer-phone" value={formData.phoneNumber} onChange={event => updateField('phoneNumber', event.target.value)} onBlur={() => setTouched(current => ({
            ...current,
            phoneNumber: true
          }))} placeholder="600 000 000" className={`h-12 w-full rounded-[10px] border bg-white pl-11 pr-10 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.phoneNumber), touched.phoneNumber && validity.phoneNumber)}`} />
      ...
      <div className="relative">
        ...
        <input id="producer-password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={event => updateField('password', event.target.value)} onBlur={() => setTouched(current => ({
        ...current,
        password: true
      }))} placeholder={i18n.t('common.minCharacters', 'Mínimo 6 caracteres')} className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-12 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.password), touched.password && validity.password)}`} />
        <button type="button" onClick={() => setShowPassword(current => !current)} className="absolute right-3 top-[38px] rounded-full p-1 text-stone-500 transition hover:bg-stone-100 hover:text-stone-800">
        ...
        <div className="flex items-start gap-3">
          <input type="checkbox" id="producer-legal" checked={formData.acceptLegal} onChange={e => updateField('acceptLegal', e.target.checked)} className="mt-1 h-5 w-5 rounded accent-stone-950 cursor-pointer" />
          <div>
  ...
  const renderStepTwo = () => <div className="space-y-6">
      <div className="relative">
        ...
        <input id="producer-brand" ref={secondRef} value={formData.brandName} onChange={event => updateField('brandName', event.target.value)} onBlur={() => setTouched(current => ({
        ...current,
        brandName: true
      }))} placeholder={i18n.t('signup.nombreDeTuObradorMarcaOCooperativ', 'Nombre de tu obrador, marca o cooperativa')} className={`mt-2 h-12 w-full rounded-[10px] border bg-white pl-11 pr-10 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.brandName), touched.brandName && validity.brandName)}`} />
      ...
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">{i18n.t('signup.exportacionActual', 'Exportación actual')}</p>
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          {EXPORT_OPTIONS.map(option => <button key={option} type="button" onClick={() => updateField('exportStage', option)} className={`rounded-[18px] border px-4 py-4 text-left text-sm font-semibold transition ${formData.exportStage === option ? 'border-stone-950 bg-stone-100 text-stone-900' : 'border-stone-200 bg-white text-stone-700 hover:border-stone-200'}`}>
              {option}
            </button>)}
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <label htmlFor="producer-region" className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">
          ...
          <div className="relative">
            ...
            <select id="producer-region" value={formData.region} onChange={event => updateField('region', event.target.value)} className={`mt-2 h-12 w-full rounded-[10px] bg-white pl-11 pr-4 text-base shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] ${fieldClass(Boolean(errors.region), touched.region && validity.region)}`}>
              <option value="">{i18n.t('signup.seleccionaTuRegion', 'Selecciona tu región')}</option>
        ...
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">{i18n.t('signup.numeroDeReferencias', 'Número de referencias')}</p>
          <div className="mt-3 space-y-3">
            {REFERENCE_OPTIONS.map(option => <button key={option} type="button" onClick={() => updateField('references', option)} className={`flex w-full items-center justify-between rounded-[14px] border px-4 py-3 text-sm font-semibold transition ${formData.references === option ? 'border-stone-950 bg-stone-100 text-stone-900' : 'border-stone-200 bg-white text-stone-700 hover:border-stone-200'}`}>
                <span>{option}</span>
  ...
  const renderStepThree = () => <div className="space-y-7">
        ...
        {Object.values(PRODUCER_PLANS).map(plan => selectedPlan === plan.key ? <div key={plan.key} className="mt-6">
              <div className={`rounded-[26px] border p-6 transition ${plan.accentClass}`}>
                <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
                  <div>
                    <PlanPill label={plan.badge} selected={true} accent={plan.key === 'pro' ? 'bg-stone-200 text-stone-700' : plan.key === 'elite' ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-800'} />
                    ...
                    <div className="mt-5 rounded-[18px] border border-stone-200 bg-stone-50 p-4">
                      <p className="text-sm font-semibold text-stone-500">{i18n.t('signup.cargoOComision', 'Cargo o comisión')}</p>
                  ...
                  <div className="rounded-[18px] border border-stone-200 bg-white p-5">
                    ...
                    <ul className="mt-4 space-y-3">
                      {plan.features.map(feature => <li key={feature} className="flex items-start gap-3 text-sm leading-7 text-stone-700">
      ...
      {selectedPlan === 'free' ? <div className="rounded-[22px] border border-stone-200 bg-stone-50 p-5">
          <div className="flex items-start gap-3">
            <input type="checkbox" id="producer-commission" checked={formData.acceptCommission} onChange={e => updateField('acceptCommission', e.target.checked)} className="mt-1 h-5 w-5 rounded accent-stone-950 cursor-pointer" />
            <div>
        ...
        </div> : <div className="rounded-[22px] border border-stone-200 bg-white p-5 shadow-[0_22px_50px_-34px_rgba(0,0,0,0.18)]">
          ...
          <div className="mt-5 rounded-[18px] border border-stone-200 bg-stone-50 px-4 py-4">
            {stripeLoading ? <div className="flex items-center gap-3 text-sm text-stone-500">
  ...
  const renderSuccess = () => {
    ...
    return <motion.div key="producer-success" initial={{
      opacity: 0,
      y: 24
    }} animate={{
      opacity: 1,
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-extrabold tracking-[-0.03em] text-stone-900">
                  {successState ? 'Alta completada' : step === 1 ? i18n.t('signup.quienEres', 'Quién eres') : step === 2 ? 'Tu producción' : 'Confirma tu plan'}
                </h2>
              </div>
              <button type="button" onClick={closeModal} className="rounded-full border border-stone-200 px-4 py-2 text-sm font-semibold text-stone-600 transition hover:border-stone-200 hover:text-stone-900">
                Cerrar
              </button>
            </div>
            <div className="mt-5">
            ...
            <div className="mx-auto flex h-full max-w-7xl flex-col gap-8 px-4 py-6 sm:px-6 lg:flex-row lg:gap-12 lg:px-10 lg:py-10">
              <aside className="w-full rounded-[28px] bg-stone-950 p-6 text-white shadow-[0_24px_70px_-36px_rgba(44,36,27,0.6)] lg:max-w-[360px]">
                ...
                <p className="mt-4 text-sm leading-7 text-white/78">
                  {successState ? i18n.t('signup.tuCuentaYaExisteYElSiguientePaso', 'Tu cuenta ya existe y el siguiente paso es verificar el email y terminar de preparar tu perfil.') : activePlan.summary}
                </p>
                ...
                <div className="mt-6 space-y-4 text-sm text-white/74">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p>{i18n.t('signup.laContrasenaNoSeGuardaEnLocalstora', 'La contraseña no se guarda en localStorage, así evitas dejar un secreto expuesto.')}</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <CreditCard className="mt-1 h-4 w-4 shrink-0 text-stone-400" />
                    <p>{i18n.t('signup.freeCreaLaCuentaAlInstanteProYE', 'FREE crea la cuenta al instante. PRO y ELITE activan Stripe inline sin salir del flujo.')}</p>
                  </div>
                </div>
              </aside>
              ...
              <section className="flex-1">
                <AnimatePresence mode="wait">
                  {successState ? renderSuccess() : <motion.div key={`producer-step-${step}`} initial={{
                  opacity: 0,
                  x: 28
                }} animate={{
                  opacity: 1,
                  ...
                  ease: 'easeOut'
                      ...
                      {paymentNotice ? <div className="mb-6 rounded-[18px] border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-700">
                          {paymentNotice}
                        </div> : null}
                      ...
                      <div className="mt-10 flex flex-col gap-4 border-t border-stone-200 pt-6 lg:flex-row lg:items-center lg:justify-between">
                        <div className="text-sm leading-7 text-stone-500">
                          {step === 1 ? i18n.t('signup.empezamosPorLoMinimoParaNoPonerte', 'Empezamos por lo mínimo para no ponerte otra barrera más.') : step === 2 ? 'Esto nos ayuda a entender tu realidad y orientarte mejor.' : i18n.t('signup.estamosAUnPasoDeAbrirTuTiendaYD', 'Estamos a un paso de abrir tu tienda y dejar la infraestructura preparada.')}
                        </div>

                        <div className="flex flex-col gap-3 sm:flex-row">
                          {step > 1 ? <button type="button" onClick={() => setStep(current => Math.max(current - 1, 1))} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-stone-200 px-5 py-3 text-sm font-semibold text-stone-700 transition hover:border-stone-900 hover:text-stone-900">
                              <ArrowLeft className="h-4 w-4" />
                              Atrás
                            </button> : null}

                          {step < 3 ? <button type="button" onClick={proceed} disabled={step === 1 && !formData.acceptLegal} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-stone-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-300">
                              Continuar
                              ...
                              {submitting ? <>
                                ...
                                </> : selectedPlan === 'free' ? 'Crear mi cuenta y empezar' : <>
