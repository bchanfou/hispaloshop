...
import { Euro, Users, MousePointerClick, Link as LinkIcon, TrendingUp, Gift, Loader2 } from 'lucide-react';
...
function InfluencerDashboard() {
  ...
  const dashboardData = useMemo(() => {
    ...
    const totalEarnings = commissions.reduce((sum, commission) => sum + (commission.amount || 0), 0) || dashboard?.earnings?.total || 0;
  ...
  return <div className="min-h-screen bg-stone-50 p-4 pb-24">
      <DashboardHeader userName={firstToken(user?.name, 'Influencer')} subtitle={`Nivel: ${dashboardData.tier}`} notificationCount={0} />

      <div className="grid grid-cols-2 gap-3 mb-4">
        <KPICard icon={Euro} value={`EUR ${dashboardData.kpis.earnings.toFixed(0)}`} label="Ganancias" subtext="este mes" accentColor="#0c0a09" />
        <KPICard icon={MousePointerClick} value={dashboardData.kpis.clicks} label="Clicks" subtext="en tus enlaces" accentColor="#78716c" />
        <KPICard icon={Users} value={dashboardData.kpis.conversions} label="Ventas" subtext="generadas" accentColor="#78716c" />
        <KPICard icon={Users} value={dashboardData.kpis.followers} label="Seguidores" subtext="en tu perfil" accentColor="#0c0a09" />
      </div>

      <div className="bg-white rounded-2xl p-4 mb-6">
        <div className="mb-4">
          <h3 className="font-semibold text-stone-950">Ganancias semanales</h3>
          <p className="text-xs text-stone-500">{i18n.t('influencer_dashboard.evolucionDeTusComisiones', 'Evolución de tus comisiones')}</p>
        </div>
      ...
      {dashboardData.recentConversions.length > 0 && <div className="mb-6">
