...
import { Euro, Package, Globe, Plus, ShoppingCart, Users, TrendingUp, Loader2 } from 'lucide-react';
...
function ImporterDashboard() {
  ...
  const dashboardData = useMemo(() => {
    ...
    return {
      ...
      suggestions: [{
        id: 1,
        title: i18n.t('importer_dashboard.expandeTuCatalogo', 'Expande tu catálogo'),
        ...
        actionLabel: i18n.t('importer_dashboard.verAnalisis', 'Ver análisis'),
  ...
  const quickActions = [{
    id: 'add',
    icon: Plus,
    label: i18n.t('importer_dashboard.anadirProducto', 'Añadir producto'),
    color: '#0c0a09',
    ...
    id: 'analytics',
    icon: TrendingUp,
    label: i18n.t('importer_dashboard.analisis', 'Análisis'),
    color: '#0c0a09',
  ...
  return <div className="min-h-screen bg-stone-50 p-4 pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <KPICard icon={Euro} value={`EUR ${dashboardData.kpis.revenue.toFixed(0)}`} label="Ingresos" subtext="este mes" accentColor="#1c1917" />
        <KPICard icon={Package} value={dashboardData.kpis.orders} label="Pedidos" subtext="totales" accentColor="#44403c" />
        <KPICard icon={Globe} value={dashboardData.kpis.countries} label={i18n.t('breadcrumbs.countries', 'Países')} subtext="de origen" accentColor="#78716c" />
        <KPICard icon={Package} value={dashboardData.kpis.products} label="Productos" subtext="activos" accentColor="#1c1917" />
      </div>

      <div className="bg-white rounded-2xl p-4 mb-6">
        <div className="mb-4">
          <h3 className="font-semibold text-stone-950">{i18n.t('importer_dashboard.evolucionDeVentas', 'Evolución de ventas')}</h3>
      ...
      {dashboardData.pendingOrders.length > 0 && <div className="mb-6">
