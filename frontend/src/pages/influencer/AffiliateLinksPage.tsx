...
function AffiliateLinkCard({
  ...
  return <div className="bg-white rounded-2xl shadow-sm p-4">
      <div className="flex gap-3 mb-3">
        {link.product_image ? <img src={link.product_image} alt={link.product_name} className="w-12 h-12 rounded-2xl object-cover shrink-0" /> : <div className="w-12 h-12 rounded-2xl bg-stone-100 flex items-center justify-center shrink-0">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        label: i18n.t('influencer.commissionRate', 'Comisión'),
        value: convertAndFormatPrice(Number(link.commission_eur || 0))
      }].map(stat => <div key={stat.label} className="bg-stone-50 rounded-2xl p-2 text-center">
      ...
      {/* Conversion rate */}
      {(link.clicks || 0) > 0 && <p className="text-xs text-stone-500 mb-2 text-center">
          Conversión: <span className="font-semibold text-stone-950">{((link.conversions || 0) / link.clicks * 100).toFixed(1)}%</span>
        </p>}

      <button onClick={copyLink} className={`w-full py-2.5 rounded-2xl text-sm font-medium transition-colors flex items-center justify-center gap-2 ${copied ? 'bg-stone-950 text-white' : 'bg-stone-950 text-white hover:bg-stone-800'}`}>
        {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
        {copied ? i18n.t('affiliate_links.copiado', '¡Copiado!') : 'Copiar link'}
      </button>
    </div>;
}
          ...
          <div className="mt-6 p-5 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {(blocked.reason || blocked.detail) && <p className="text-sm text-stone-500 mb-4">
                    Motivo: {blocked.reason || blocked.detail}
                  </p>}
                <RouterLink to="/influencer/fiscal-setup" className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold transition-colors bg-stone-950 text-white rounded-2xl">
                  Activar afiliados
                </RouterLink>
              </div>
            </div>
  ...
  return <div className="min-h-screen bg-stone-50">
      <div className="max-w-[975px] mx-auto px-4 py-6 pb-28">
        ...
        <div className="relative mb-6">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar producto para generar link..." className="w-full pl-10 pr-4 py-3 rounded-2xl border border-stone-200 bg-white text-sm text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-400" />
          ...
          {searchResults.length > 0 && <>
              ...
              <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white rounded-2xl border border-stone-200 shadow-lg max-h-64 overflow-y-auto">
                {searchResults.map(product => <button key={product.product_id || product.id} type="button" onClick={() => generateLink(product)} disabled={generating} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-stone-50 transition-colors border-b border-stone-100 last:border-0">
                    {product.images?.[0] ? <img src={product.images[0]} alt="" className="w-9 h-9 rounded-2xl object-cover shrink-0" /> : <div className="w-9 h-9 rounded-2xl bg-stone-100 shrink-0" />}
                    <div className="flex-1 min-w-0">
        ...
        {/* Generated link */}
        {generatedLink && selectedProduct && <motion.div initial={{
        opacity: 0,
        y: -8
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="flex gap-3 mb-3">
              {selectedProduct.images?.[0] && <img src={selectedProduct.images[0]} alt="" className="w-12 h-12 rounded-2xl object-cover shrink-0" />}
              <div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {myLinks.length > 1 && <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="rounded-xl border border-stone-200 bg-white text-sm text-stone-950 px-3 py-1.5 focus:outline-none focus:border-stone-400">
              <option value="revenue">{i18n.t('affiliate_links.masIngresos', 'Más ingresos')}</option>
              <option value="clicks">{i18n.t('affiliate_links.masClics', 'Más clics')}</option>
              <option value="recent">{i18n.t('products.newest', 'Más recientes')}</option>
            </select>}
        </div>
        {loadingLinks ? <div className="space-y-3">
            {[1, 2, 3].map(i => <div key={i} className="bg-white rounded-2xl shadow-sm p-4 animate-pulse space-y-3">
          ...
          </div> : myLinks.length === 0 ? <div className="text-center py-10">
          ...
          return bTime - aTime;
        }).map(link => <AffiliateLinkCard key={link.link_id || link.id} link={link} convertAndFormatPrice={convertAndFormatPrice} />)}
          </div>}
      </div>
    </div>;
}