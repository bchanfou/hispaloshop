...
export default function FiscalSetupPage() {
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)', background: 'var(--color-cream)', minHeight: '100vh' }}>
      ...
      <div className="mb-5">
        ...
        <div className="relative">
          <button
            ...
            style={{ background: 'var(--color-white)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', color: country ? 'var(--color-black)' : 'var(--color-stone)' }}
          >
            {country ? COUNTRIES.find(c => c.code === country)?.name || country : 'Selecciona tu país'}
            <ChevronDown className="w-4 h-4 shrink-0" style={{ color: 'var(--color-stone)' }} />
          </button>

          {showCountryDropdown && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setShowCountryDropdown(false)} />
              <div className="absolute top-full left-0 right-0 z-50 mt-1 overflow-hidden" style={{ background: 'var(--color-white)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', boxShadow: '0 8px 30px rgba(0,0,0,0.12)' }}>
                <div className="p-2">
                  <input
                    value={countrySearch}
      ...
      {/* 2. Certificate */}
      <div className="mb-5">
        <h2 className="text-sm font-bold mb-3" style={{ color: 'var(--color-black)' }}>Certificado de residencia fiscal</h2>

        {certStatus === null && (
          <>
            <div className="p-4 mb-3" style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-xl)' }}>
              <p className="text-sm" style={{ color: 'var(--color-stone)' }}>
                <strong style={{ color: 'var(--color-black)' }}>¿Qué es esto?</strong> El certificado de residencia fiscal acredita que pagas tus impuestos en{' '}
            ...
            >
              <Upload className="w-6 h-6" style={{ color: 'var(--color-stone)' }} />
              <p className="text-sm font-semibold" style={{ color: 'var(--color-black)' }}>Sube tu certificado</p>
              <p className="text-xs" style={{ color: 'var(--color-stone)' }}>PDF, JPG o PNG · Máx 5MB</p>
            </button>
          </>
        )}
          ...
          <div className="p-4" style={{ background: '#f5f5f4', borderRadius: 'var(--radius-xl)', border: '1px solid #d6d3d1' }}>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {fiscal?.block_reason && (
                  <p className="text-xs mt-1" style={{ color: 'var(--color-stone)' }}>{fiscal.block_reason}</p>
                )}
                <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleUpload} className="hidden" />
                <button
                  onClick={() => fileRef.current?.click()}
      ...
      {/* 3. Payout method */}
      <div className="mb-5">
        <h2 className="text-sm font-bold mb-3" style={{ color: 'var(--color-black)' }}>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            style={{
              background: 'var(--color-white)',
              borderRadius: 'var(--radius-xl)',
              border: payoutMethod === 'stripe' ? '2px solid var(--color-black)' : '1px solid var(--color-border)',
          ...
          >
            ...
            <p className="text-xs ml-8" style={{ color: 'var(--color-stone)' }}>
          ...
          <div
            ...
            style={{
              background: 'var(--color-white)',
              borderRadius: 'var(--radius-xl)',
              border: payoutMethod === 'sepa' ? '2px solid var(--color-black)' : '1px solid var(--color-border)',
            }}
          >
            <button
            ...
            {payoutMethod === 'sepa' && (
              <div className="mt-4 ml-8 space-y-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'var(--color-stone)' }}>IBAN</label>
                  <input
                    value={iban}
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'var(--color-stone)' }}>Titular de la cuenta</label>
                  <input
                    value={accountName}
      ...
      {/* 4. Payout preview */}
      {grossPreview > 0 && (
        <div className="mb-5 p-4" style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-xl)' }}>
          <h3 className="text-sm font-bold mb-3" style={{ color: 'var(--color-black)' }}>Preview del cobro</h3>
          <div className="space-y-2">
      ...
      <button
        ...
        style={{
          background: canSave ? 'var(--color-black)' : 'var(--color-surface)',
          color: canSave ? '#fff' : 'var(--color-stone)',
          borderRadius: 'var(--radius-xl)',
          border: 'none',
