...
function getWithholdingInfo(code) {
  ...
  if (code === 'ES') return {
    pct: 15,
    label: 'Retención 15% IRPF',
    color: 'stone-dark'
  };
  return {
    pct: 0,
    label: i18n.t('fiscal_setup.sinRetencion', 'Sin retención'),
    color: 'stone-light'
  };
}
export default function FiscalSetupPage() {
  ...
  const COUNTRIES = useMemo(() => {
    ...
    if (entries.length > 10) return entries.map(([code, data]) => ({
      code,
      name: data.name || code
    }));
  ...
  const loadFiscal = useCallback(async () => {
    try {
      ...
      if (data.certificate_verified) setCertStatus('verified');else if (data.needs_manual_review) setCertStatus('manual_review');else if (data.certificate_url && !data.certificate_verified) setCertStatus('rejected');
    } catch {
  ...
  // Filtered countries
  const filteredCountries = countrySearch ? COUNTRIES.filter(c => c.name.toLowerCase().includes(countrySearch.toLowerCase()) || c.code.toLowerCase().includes(countrySearch.toLowerCase())) : COUNTRIES;
  if (loading) {
  ...
  return <div className="min-h-screen bg-stone-50">
      ...
      <div className="flex items-center gap-2 mb-6 px-1">
        {[t('fiscal_setup.paisFiscal', 'País fiscal'), 'Certificado', 'Método de cobro', 'Verificación'].map((stepLabel, i) => {
          ...
          return <div key={stepLabel} className="flex items-center gap-1.5">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${isComplete || isCurrent ? 'bg-stone-950 text-white border-none' : 'bg-transparent text-stone-500 border-[1.5px] border-stone-200'}`}>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {country && certStatus !== 'verified' && <button onClick={() => {
            setCountry('');
            setCertStatus(null);
        ...
        <div className="relative">
          <button onClick={() => setShowCountryDropdown(!showCountryDropdown)} className={`w-full flex items-center justify-between h-12 px-3.5 text-sm text-left bg-white rounded-xl border border-stone-200 ${country ? 'text-stone-950' : 'text-stone-500'}`}>
          ...
          {showCountryDropdown && <>
              ...
              <div className="absolute top-full left-0 right-0 z-50 mt-1 overflow-hidden bg-white rounded-xl border border-stone-200 shadow-lg">
                <div className="p-2">
                  <input value={countrySearch} onChange={e => setCountrySearch(e.target.value)} placeholder={t('fiscal_setup.buscarPais', 'Buscar país...')} autoFocus className="w-full h-12 px-3 text-sm rounded-xl bg-stone-100 text-stone-950 focus:outline-none border-none" />
                </div>
                <div className="max-h-48 overflow-y-auto">
                  {filteredCountries.map(c => <button key={c.code} onClick={() => {
                  setCountry(c.code);
        ...
        {/* Withholding badge */}
        {withholdingInfo && <div className={`mt-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${withholdingInfo.color === 'stone-dark' ? 'bg-stone-200 text-stone-700' : 'bg-stone-100 text-stone-950'}`}>
      ...
      <div className="mb-5">
        ...
        {certStatus === null && <>
            ...
            <button onClick={() => fileRef.current?.click()} disabled={!country || uploading} className={`w-full flex flex-col items-center gap-2 p-6 transition-colors rounded-2xl border-2 border-dashed border-stone-200 bg-white ${!country ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
        ...
        {certStatus === 'uploading' && <div className="p-5 flex items-center gap-3 bg-stone-100 rounded-2xl">
        ...
        {certStatus === 'verified' && <div className="p-4 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {fiscal?.verified_at && <p className="text-xs mt-0.5 text-stone-500">
                    Verificado el {new Date(fiscal.verified_at).toLocaleDateString(undefined)}
                  </p>}
                <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleUpload} className="hidden" />
                <button onClick={() => fileRef.current?.click()} className="mt-3 text-xs font-semibold underline text-stone-500 bg-transparent border-none cursor-pointer">
        ...
        {certStatus === 'rejected' && <div className="p-4 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {fiscal?.block_reason && <p className="text-xs mt-1 text-stone-500">{fiscal.block_reason}</p>}
                <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleUpload} className="hidden" />
                <button onClick={() => fileRef.current?.click()} className="mt-3 px-4 py-2 text-sm font-semibold bg-stone-950 text-white rounded-full border-none cursor-pointer transition-colors hover:bg-stone-800">
        ...
        {certStatus === 'manual_review' && <div className="p-4 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
      ...
      <div className="mb-5">
        ...
        <div className="space-y-2">
          ...
          <button onClick={() => {
          ...
          }} disabled={savingPayout} className={`w-full text-left p-4 transition-all bg-white rounded-2xl ${payoutMethod === 'stripe' ? 'border-2 border-stone-950' : 'border border-stone-200'} ${savingPayout ? 'cursor-wait' : 'cursor-pointer'}`}>
            <div className="flex items-center gap-3 mb-1">
              ...
              {payoutMethod === 'stripe' && fiscal?.stripe_onboarding_complete && <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-950">
                  Conectado
                </span>}
            </div>
          ...
          {/* SEPA */}
          <div className={`p-4 transition-all bg-white rounded-2xl ${payoutMethod === 'sepa' ? 'border-2 border-stone-950' : 'border border-stone-200'}`}>
            <button onClick={() => setPayoutMethod('sepa')} className="w-full text-left bg-transparent border-none cursor-pointer">
              <div className="flex items-center gap-3 mb-1">
                ...
                {payoutMethod === 'sepa' && fiscal?.sepa_iban_last4 && <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-950">
                    Configurado
                  </span>}
              </div>
              <p className="text-xs ml-8 text-stone-500">
            ...
            {payoutMethod === 'sepa' && <div className="mt-4 ml-8 space-y-3">
                <div>
                  <label className="block text-xs font-medium mb-1 text-stone-500">IBAN</label>
                  <input value={iban} onChange={e => setIban(e.target.value)} placeholder="ES00 0000 0000 0000 0000 0000" className="w-full h-12 px-3 text-sm bg-stone-100 rounded-xl border border-stone-200 text-stone-950 focus:outline-none" />
                </div>
                <div>
                  ...
                  <input value={accountName} onChange={e => setAccountName(e.target.value)} placeholder="Nombre completo" className="w-full h-12 px-3 text-sm bg-stone-100 rounded-xl border border-stone-200 text-stone-950 focus:outline-none" />
                </div>
                <button onClick={() => handlePayoutSave('sepa')} disabled={savingPayout || !iban.trim() || !accountName.trim()} className={`px-4 py-2 text-sm font-semibold bg-stone-950 text-white rounded-full border-none transition-colors hover:bg-stone-800 ${savingPayout ? 'cursor-wait' : 'cursor-pointer'} ${!iban.trim() || !accountName.trim() ? 'opacity-50' : 'opacity-100'}`}>
      ...
      {/* 4. Payout preview */}
      {grossPreview > 0 && <div className="mb-5 p-4 bg-stone-100 rounded-2xl">
          ...
          <div className="space-y-2">
            ...
            {country === 'ES' && withholdingPreview > 0 && <div className="flex justify-between text-sm">
            ...
            {feePreview > 0 && <div className="flex justify-between text-sm">
      ...
      <div className="mb-5">
        ...
        {fiscal?.fiscal_setup_status === 'completed' ? <div className="p-4 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
              <Check className="w-5 h-5 shrink-0 mt-0.5 text-stone-950" />
              <div>
          ...
          </div> : canSave ? <div className="p-4 bg-stone-100 rounded-2xl shadow-sm">
            <div className="flex items-start gap-3">
              <Clock className="w-5 h-5 shrink-0 mt-0.5 text-stone-600" />
              <div>
          ...
          </div> : <div className="p-4 bg-stone-50 rounded-2xl border border-stone-200">
            <div className="flex items-start gap-3">
      ...
      }} disabled={!canSave} className={`w-full py-3.5 text-sm font-semibold rounded-full border-none transition-colors mb-8 ${canSave ? 'bg-stone-950 text-white cursor-pointer hover:bg-stone-800' : 'bg-stone-100 text-stone-500 cursor-not-allowed'}`}>
