...
export default function PayoutSettingsPage() {
  ...
  const handleStripeConnect = async () => {
    ...
    try {
      ...
      toast.error(i18n.t('influencer.stripeError', 'Error al conectar con Stripe'));
    } finally {
      setSaving(false);
    }
  };
  const handleSaveSEPA = async () => {
    ...
    if (!/^[A-Z]{2}\d{2}[A-Z0-9]{4,}$/i.test(iban.replace(/\s/g, ''))) {
      toast.error(i18n.t('payout_settings.elFormatoDelIbanNoEsValido', 'El formato del IBAN no es valido'));
      return;
    }
    setSaving(true);
    try {
  ...
  return <div className="min-h-screen bg-stone-50">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-stone-200 flex items-center gap-3 px-4 py-3">
        ...
        <span className="text-[17px] font-bold text-stone-950">{i18n.t('producer_payments.metodoDeCobro', 'Método de cobro')}</span>
      </div>

      <div className="max-w-[600px] mx-auto px-4 pt-6 pb-[100px]">
        {loading ? <div className="flex justify-center py-[60px]">
            ...
            <div className="flex flex-col gap-3 mb-7">
              {/* Stripe Connect */}
              <button onClick={() => setMethod('stripe')} className={`flex items-center gap-3.5 p-4 bg-white rounded-2xl cursor-pointer text-left ${method === 'stripe' ? 'border-2 border-stone-950' : 'border border-stone-200'}`}>
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${method === 'stripe' ? 'bg-stone-950' : 'bg-stone-100'}`}>
                ...
                {stripeConnected && method === 'stripe' && <div className="w-6 h-6 rounded-full bg-stone-950 flex items-center justify-center shrink-0">
                    <Check size={14} className="text-white" />
                  </div>}
              </button>

              {/* SEPA */}
              <button onClick={() => setMethod('sepa')} className={`flex items-center gap-3.5 p-4 bg-white rounded-2xl cursor-pointer text-left ${method === 'sepa' ? 'border-2 border-stone-950' : 'border border-stone-200'}`}>
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${method === 'sepa' ? 'bg-stone-950' : 'bg-stone-100'}`}>
                ...
                {sepaConfigured && method === 'sepa' && <div className="w-6 h-6 rounded-full bg-stone-950 flex items-center justify-center shrink-0">
                    <Check size={14} className="text-white" />
                  </div>}
              </button>
            </div>
            ...
            {method === 'stripe' && <div className="bg-white shadow-sm rounded-2xl p-5 mb-5">
                ...
                {stripeConnected ? <>
                    ...
                    <button onClick={handleStripeConnect} disabled={saving} className="w-full py-3.5 bg-stone-950 text-white rounded-full text-sm font-semibold cursor-pointer flex items-center justify-center gap-2 hover:bg-stone-800 transition-colors disabled:opacity-60">
            ...
            {/* SEPA Section */}
            {method === 'sepa' && <div className="bg-white shadow-sm rounded-2xl p-5 mb-5">
                <p className="text-[15px] font-semibold text-stone-950 mb-4">
                  Datos bancarios SEPA
                </p>

                {sepaConfigured && !iban.trim() && <div className="flex items-center gap-2 p-2.5 px-3.5 rounded-xl bg-stone-100 mb-4">
                ...
                <div className="mb-3.5">
                  ...
                  <input type="text" value={iban} onChange={e => setIban(e.target.value.toUpperCase())} placeholder={sepaConfigured ? `.... .... .... ${payoutData?.sepa_iban_last4}` : 'ES00 0000 0000 0000 0000 0000'} className="w-full h-12 px-3.5 border border-stone-200 rounded-xl text-sm text-stone-950 bg-white outline-none focus:border-stone-400 transition-colors box-border tracking-wider font-mono" />
                </div>
                ...
                <div className="mb-5">
                  ...
                  <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)} placeholder={payoutData?.sepa_account_name || 'Nombre completo o razon social'} className="w-full h-12 px-3.5 border border-stone-200 rounded-xl text-sm text-stone-950 bg-white outline-none focus:border-stone-400 transition-colors box-border" />
                </div>

                <button onClick={handleSaveSEPA} disabled={saving || !iban.trim() && !accountName.trim()} className={`w-full py-3.5 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${iban.trim() && accountName.trim() ? 'bg-stone-950 text-white cursor-pointer hover:bg-stone-800' : 'bg-stone-100 text-stone-500 cursor-default'}`}>
                  {saving ? <Loader2 size={16} className="animate-spin" /> : 'Guardar datos bancarios'}
                </button>
              </div>}

            {/* Producer note */}
            {isProducer && <div className="p-4 bg-stone-100 rounded-2xl mt-3">
