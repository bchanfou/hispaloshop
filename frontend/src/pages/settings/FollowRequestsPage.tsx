...
export default function FollowRequestsPage() {
  ...
  useEffect(() => {
    let active = true;
    apiClient.get('/users/me/follow-requests')
      .then(data => { if (active) setRequests(data?.requests || []); })
      .catch(() => { if (active) toast.error('Error al cargar solicitudes'); })
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-2">
            {requests.map(req => {
              ...
              return (
                <div key={req.request_id} className="flex items-center gap-3 rounded-2xl shadow-sm bg-white px-4 py-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      aria-label={`Aceptar solicitud de ${u.name}`}
                      className="flex min-h-[44px] items-center gap-1 rounded-full bg-stone-950 px-4 text-[12px] font-semibold text-white border-none cursor-pointer hover:bg-stone-800 disabled:opacity-50 transition-colors"
                    >
                    ...
                    <button
                      ...
                      disabled={isProcessing}
                      aria-label={`Rechazar solicitud de ${u.name}`}
                      className="flex min-h-[44px] items-center justify-center rounded-full border border-stone-200 bg-white px-3 text-stone-500 cursor-pointer hover:bg-stone-50 disabled:opacity-50 transition-colors"
                    >
                      <X size={16} />
                    </button>
                  </div>
