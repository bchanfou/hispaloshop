  ...
  return <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-[600px] mx-auto px-4 pt-6 pb-[100px]">
        ...
        <div className="bg-white shadow-sm rounded-2xl p-5 mb-6">
          ...
          {trialDays > 0 && <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-stone-100 text-stone-500 text-xs font-semibold mb-3">
              Trial activo · {trialDays} días restantes
            </div>}

          {nextBilling && <p className="text-[13px] text-stone-500 mb-4">
              Próxima factura: {new Date(nextBilling).toLocaleDateString('es-ES', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          })}
          ...
          {currentPlan !== 'free' && <button onClick={handleManageBilling} disabled={loading} className="w-full p-3 bg-white border border-stone-200 rounded-[14px] text-sm font-semibold text-stone-950 cursor-pointer flex items-center justify-center gap-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          return <div key={plan.id} className={`bg-white rounded-2xl p-4 relative ${isCurrent ? 'border-2 border-stone-950' : 'border border-stone-200'}`}>
                {isCurrent && <span className="absolute -top-2.5 right-4 text-[10px] font-bold px-2.5 py-[3px] rounded-full bg-stone-950 text-white">
                    Actual
                  </span>}
                ...
                <div className="mb-3">
                  {plan.features.map((f, i) => <div key={i} className="flex items-center gap-2 py-1">
                      <Check size={14} className="text-stone-950" />
                      <span className="text-[13px] text-stone-950">{f}</span>
                    </div>)}
                </div>

                {!isCurrent && <button onClick={() => handleChangePlan(plan.id)} className={`w-full py-2.5 rounded-full text-[13px] font-semibold cursor-pointer ${isUpgrade ? 'bg-stone-950 text-white border-none' : 'bg-white text-stone-950 border border-stone-200'}`}>
      ...
      {downgradeTarget && <div className="fixed inset-0 z-[999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true" onClick={() => setDowngradeTarget(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-5 max-w-[360px] w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2.5">
              <button onClick={() => setDowngradeTarget(null)} className="flex-1 py-2.5 rounded-full border border-stone-200 text-[13px] font-semibold text-stone-950">
                Cancelar
              </button>
              <button onClick={confirmDowngrade} className="flex-1 py-2.5 rounded-full bg-stone-950 text-[13px] font-semibold text-white">
                Confirmar
              </button>
            </div>
          </div>
...
function PlanBadge({
  ...
  return <span className={`text-[10px] font-bold tracking-[0.05em] px-2.5 py-[3px] rounded-full ${isElite ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-950'}`}>
      {upper}
    </span>;
}