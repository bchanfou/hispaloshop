...
export default function ExploreCategoryPage() {
  ...
  if (!group) {
    return <div className="flex min-h-[60vh] flex-col items-center justify-center gap-3">
        <Search size={48} className="text-stone-300" strokeWidth={1.5} />
        <p className="text-[15px] text-stone-500">{i18n.t('explore_category.categoriaNoEncontrada', 'Categoría no encontrada')}</p>
        <button onClick={() => navigate('/explore')} className="border-none bg-transparent text-sm font-semibold text-stone-950 underline">
          Volver a Explorar
        </button>
      </div>;
  }
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-40 flex items-center gap-3 border-b border-stone-200 bg-white px-4 py-3">
        <button onClick={() => navigate('/explore')} className="flex h-11 w-11 items-center justify-center" aria-label="Volver">
          <ArrowLeft size={22} className="text-stone-950" />
        </button>
        {React.createElement(getIcon(group.icon), {
          size: 22,
          className: 'text-stone-950'
        })}
        <span className="flex-1 text-[17px] font-bold text-stone-950">{group.label}</span>
        {!loading && products.length > 0 && <span className="text-xs text-stone-500">
            {products.length} producto{products.length !== 1 ? 's' : ''}
          </span>}
      </div>

      {/* Subcategory pills */}
      {subcategories.length > 1 && <div className="flex gap-2 overflow-x-auto px-4 py-3 scrollbar-hide">
          {subcategories.map(cat => {
          const isActive = cat.slug === effectiveSubSlug;
          return <button key={cat.slug} onClick={() => setActiveSubSlug(cat.slug)} aria-pressed={isActive} className={`flex shrink-0 cursor-pointer items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-[13px] font-semibold whitespace-nowrap transition-colors ${isActive ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-white text-stone-950'}`}>
      ...
      <div className="px-4 pt-2 pb-20">
        {loading ? <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
            {Array.from({
            length: 6
          }).map((_, i) => <div key={i} className="animate-pulse rounded-2xl bg-stone-100" style={{
            aspectRatio: '4/5'
          }} />)}
          </div> : error ? <div className="flex flex-col items-center justify-center gap-3 py-16">
            <Package size={48} className="text-stone-300" strokeWidth={1.5} />
            <p className="text-center text-[15px] text-stone-500">
              No se pudieron cargar los productos
            </p>
            <button onClick={() => setRetryKey(k => k + 1)} className="mt-1 flex items-center gap-1.5 rounded-full bg-stone-950 px-5 py-2.5 text-[13px] font-semibold text-white">
              <RefreshCw size={14} /> Reintentar
            </button>
          </div> : products.length === 0 ? <div className="flex flex-col items-center justify-center gap-3 py-16">
            <Package size={48} className="text-stone-300" strokeWidth={1.5} />
            <p className="text-center text-[15px] text-stone-500">
          ...
          duration: 0.3
            ...
            {products.map(product => <ProductCard key={product.product_id || product.id} product={product} />)}
          </motion.div>}
      </div>
