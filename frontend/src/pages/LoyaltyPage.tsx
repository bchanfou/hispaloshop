...
export default function LoyaltyPage() {
  ...
  const activeCodes = (loyalty?.redemptions || []).filter((r) => !r.is_used && new Date(r.expires_at) > new Date());

  const handleRedeem = async (rewardId) => {
    setRedeeming(rewardId);
    try {
      const result = await apiClient.post('/loyalty/redeem', { reward_id: rewardId });
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 bg-white/80 backdrop-blur-xl border-b border-stone-100 px-4 h-[52px]">
        <button onClick={() => navigate(-1)} className="p-1 -ml-1 rounded-full bg-transparent border-none cursor-pointer">
      ...
      <div className="max-w-[480px] mx-auto px-4 py-5">
        ...
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #1B6B40, #155534)' }}
        >
            ...
            return (
              <div
                ...
                className={`flex items-center gap-4 rounded-2xl border p-4 transition-opacity ${
                  canAfford ? 'border-stone-200' : 'border-stone-100 opacity-50'
                }`}
              >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="flex flex-col gap-3">
                {activeCodes.map((r) => (
                  <div
                    key={r.discount_code}
                    className="flex items-center justify-between rounded-2xl border-2 border-dashed p-4"
                    style={{
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[11px] text-stone-500 mt-1">
                        {r.reward_name} · Caduca {new Date(r.expires_at).toLocaleDateString('es-ES')}
                      </div>
                    </div>
                    <button
                      onClick={() => handleCopy(r.discount_code)}
        ...
        {/* History */}
        {(loyalty?.redemptions || []).filter((r) => r.is_used).length > 0 && (
          <div className="mt-8">
            <h2 className="text-sm font-semibold text-stone-500 mb-2">Historial de canjes</h2>
            {loyalty.redemptions
              .filter((r) => r.is_used)
              .map((r) => (
                <div key={r.discount_code} className="flex items-center justify-between py-2 border-b border-stone-100">
                  <div>
                    <span className="text-sm text-stone-700">{r.reward_name || r.discount_code}</span>
                    <span className="text-xs text-stone-400 ml-2">
                      {new Date(r.used_at || r.redeemed_at).toLocaleDateString('es-ES')}
                    </span>
                  </div>
