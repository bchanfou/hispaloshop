...
const IBEROESFERA = new Set(['ES', 'PT', 'MX', 'CO', 'AR', 'CL', 'PE', 'VE', 'EC', 'BO', 'PY', 'UY', 'CR', 'PA', 'DO', 'CU', 'GT', 'HN', 'SV', 'NI', 'BR', 'GQ', 'PH']);
const REGIONS = [{
  id: 'iberoesfera',
  emoji: '\u{1F30D}',
  label: 'Iberoesfera',
  countries: IBEROESFERA
}, {
  id: 'europa',
  emoji: '\u{1F1EA}\u{1F1FA}',
  label: 'Europa',
  countries: new Set(['ES', 'PT', 'FR', 'DE', 'IT', 'NL', 'BE', 'AT', 'CH', 'IE', 'GB', 'DK', 'SE', 'NO', 'FI', 'PL', 'CZ', 'SK', 'HU', 'RO', 'BG', 'HR', 'SI', 'GR', 'EE', 'LV', 'LT', 'LU', 'MT', 'CY'])
}, {
  id: 'americas',
  emoji: '\u{1F30E}',
  label: 'Am\u00e9ricas',
  countries: new Set(['US', 'CA', 'MX', 'CO', 'AR', 'CL', 'PE', 'VE', 'EC', 'BO', 'PY', 'UY', 'CR', 'PA', 'DO', 'CU', 'GT', 'HN', 'SV', 'NI', 'BR', 'JM', 'TT', 'HT'])
}, {
  id: 'asia',
  emoji: '\u{1F30F}',
  label: 'Asia',
  countries: new Set(['CN', 'JP', 'KR', 'IN', 'TH', 'VN', 'ID', 'MY', 'SG', 'PH', 'TW', 'HK', 'PK', 'BD', 'LK', 'NP', 'MM', 'KH', 'LA', 'MN', 'KZ', 'UZ', 'TM', 'KG', 'TJ'])
}, {
  id: 'mena',
  emoji: '\u{1F54C}',
  label: 'MENA',
  countries: new Set(['MA', 'DZ', 'TN', 'LY', 'EG', 'SA', 'AE', 'QA', 'KW', 'BH', 'OM', 'JO', 'LB', 'IQ', 'SY', 'YE', 'IR', 'IL', 'PS', 'TR'])
}, {
  id: 'africa',
  emoji: '\u{1F30D}',
  label: '\u00c1frica',
  countries: new Set(['NG', 'GH', 'KE', 'TZ', 'UG', 'ET', 'ZA', 'SN', 'CI', 'CM', 'CD', 'AO', 'MZ', 'MG', 'ZW', 'RW', 'ML', 'BF', 'NE', 'TD', 'GQ', 'GA', 'CG', 'BJ', 'TG', 'SL', 'LR', 'GW', 'MR', 'SO', 'DJ', 'ER', 'SS', 'MW', 'ZM', 'BW', 'NA', 'LS', 'SZ'])
}];
const COUNTRY_NAMES = {
  ES: 'Espa\u00f1a',
  PT: 'Portugal',
...
function StoreMap({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (typeof coords.lat === 'number' && typeof coords.lng === 'number' && Math.abs(coords.lat) <= 90 && Math.abs(coords.lng) <= 180) {
  ...
  useEffect(() => {
    ...
    storesWithCoords.forEach(store => {
      ...
      marker.bindPopup(`<div style="font-family:inherit;min-width:140px;padding:2px 0">` + `<p style="font-size:13px;font-weight:600;color:#0c0a09;margin:0 0 2px">${escapeHtml(store.name)}</p>` + (store.location ? `<p style="font-size:11px;color:#78716c;margin:0 0 4px">${escapeHtml(store.location)}</p>` : '') + metaHtml + `<a href="/store/${encodeURIComponent(slug)}" style="font-size:11px;font-weight:600;color:#0c0a09;text-decoration:none">Ver tienda \u2192</a>` + `</div>`, {
        closeButton: false,
        className: 'hs-popup',
        maxWidth: 220
      });
  ...
  return <div className="relative">
      ...
      `}</style>
      <div ref={mapRef} className="w-full overflow-hidden rounded-2xl" style={{
...
function StoreCard({
  ...
  return <Link to={`/store/${slug}`} aria-label={`Tienda ${store.name}`} className="block rounded-2xl bg-white p-3 no-underline transition-shadow hover:shadow-sm">
      {/* Avatar + Name row */}
      <div className="flex items-center gap-2.5">
        <div className="relative shrink-0">
          <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-stone-100">
            {store.logo ? <img src={store.logo} alt="" loading="lazy" className="h-full w-full object-cover" /> : <span className="text-lg font-bold text-stone-400">{(store.name || 'T')[0]}</span>}
          </div>
          {(store.verified || store.producer_verified) && <span className="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-stone-950">
              <Check size={10} className="text-white" strokeWidth={3} />
            </span>}
        </div>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-stone-950">{store.name}</p>
          {category && <p className="truncate text-[11px] text-stone-500">{category}</p>}
        </div>
      </div>

      {/* Location */}
      {store.location && <p className="mt-1.5 truncate text-[11px] text-stone-400 flex items-center gap-1">
          <MapPin size={9} className="shrink-0" /> {store.location}
        </p>}

      {/* Meta row */}
      <div className="mt-1 flex items-center gap-3 text-[11px] text-stone-500">
        {store.product_count > 0 && <span className="flex items-center gap-1">
            <Package size={10} /> {store.product_count}
          </span>}
        {rating > 0 && <span className="flex items-center gap-1">
...
function FeaturedCard({
  ...
  return <Link to={`/store/${slug}`} aria-label={`Tienda destacada: ${store.name}`} className="shrink-0 w-[260px] no-underline rounded-2xl overflow-hidden bg-stone-950 relative aspect-video">
...
export default function StoresListPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    apiClient.get(`/stores?plan=elite&country=${userCountry}&limit=10`).then(data => {
      if (!active) return;
      const list = Array.isArray(data) ? data : data?.stores || [];
      setEliteStores(list);
  ...
  return <div className="min-h-screen bg-white pb-20">
      <SEO title="Tiendas — Hispaloshop" description={i18n.t('stores_list.exploraTiendasDeProductoresArtesanal', 'Explora tiendas de productores artesanales de alimentación saludable local. Filtra por región y país.')} structuredData={stores.length > 0 ? [{
      ...
      <div className="sticky top-0 z-20 h-[52px] flex items-center justify-between px-4 bg-white border-b border-stone-200 max-w-[975px] mx-auto">
        ...
        <button onClick={() => setViewMode(viewMode === 'list' ? 'map' : 'list')} className="bg-transparent border-none cursor-pointer p-1 flex text-stone-950" aria-label={viewMode === 'list' ? 'Ver mapa' : 'Ver lista'}>
      ...
      <div className="px-4 pt-3 max-w-[975px] mx-auto">
        ...
        <div className="relative mb-3">
          ...
          <input type="text" placeholder="Buscar tiendas..." aria-label="Buscar tiendas" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="h-10 w-full rounded-full border-none bg-stone-100 pl-10 text-sm text-stone-950 outline-none placeholder:text-stone-400" style={{
          paddingRight: searchQuery ? 40 : 16
        }} />
          {searchQuery && <button onClick={() => setSearchQuery('')} aria-label="Borrar b\u00fasqueda" className="absolute right-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full border-none bg-stone-200 text-stone-500 cursor-pointer">
              <X size={13} />
            </button>}
        </div>

        {/* ── REGION PILLS ── */}
        <div className="scrollbar-hide flex gap-2 overflow-x-auto mb-2 pb-1" style={{
        scrollbarWidth: 'none'
      }}>
          <button onClick={() => {
          setActiveRegion(null);
          setActiveCountry(null);
        }} aria-pressed={!activeRegion} className={`shrink-0 flex items-center gap-1.5 px-3.5 py-[7px] rounded-full text-[13px] font-medium whitespace-nowrap cursor-pointer transition-all duration-150 ${!activeRegion ? 'bg-stone-950 text-white border-none' : 'bg-white text-stone-950 border border-stone-200'}`}>
            Todas
          </button>
          {availableRegions.map(r => <button key={r.id} onClick={() => handleRegionClick(r.id)} aria-pressed={activeRegion === r.id} className={`shrink-0 flex items-center gap-1.5 px-3.5 py-[7px] rounded-full text-[13px] font-medium whitespace-nowrap cursor-pointer transition-all duration-150 ${activeRegion === r.id ? 'bg-stone-950 text-white border-none' : 'bg-white text-stone-950 border border-stone-200'}`}>
              {r.emoji} {r.label}
            </button>)}
        </div>

        {/* ── COUNTRY CHIPS (appear when region selected) ── */}
        {regionCountryChips.length > 0 && <div className="scrollbar-hide flex gap-1.5 overflow-x-auto mb-3 pb-1" style={{
        scrollbarWidth: 'none'
      }}>
            {regionCountryChips.map(c => <button key={c.code} onClick={() => handleCountryClick(c.code)} aria-pressed={activeCountry === c.code} className={`shrink-0 flex items-center gap-1 px-3 py-[5px] rounded-full text-xs font-medium cursor-pointer transition-all duration-150 ${activeCountry === c.code ? 'bg-stone-950 text-white border-none' : 'bg-stone-100 text-stone-950 border border-stone-200'}`}>
                {c.flag} {c.name} <span className="opacity-50">({c.count})</span>
              </button>)}
          </div>}

        {/* ── MAP VIEW ── */}
        {viewMode === 'map' && <div className="mb-6">
            <StoreMap stores={filteredStores} />
            {/* mini list below map */}
            {filteredStores.length > 0 && <div className="mt-4">
              ...
              const slug = store.slug || store.store_slug;
              return <Link key={store.store_id || slug} to={`/store/${slug}`} aria-label={`Tienda ${store.name}`} className="flex items-center gap-3 px-3 py-2.5 bg-white rounded-[14px] border border-stone-200 no-underline">
                        {store.logo ? <img loading="lazy" src={store.logo} alt={store.name} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center text-sm font-semibold text-stone-500">
                        ...
                        {(store.average_rating || store.rating) > 0 && <span className="text-xs font-semibold text-stone-950 flex items-center gap-0.5">
        ...
        {viewMode === 'list' && <>
            ...
            {eliteStores.length > 0 && !debouncedSearch && !activeRegion && <div className="mb-6">
                <span className="block text-[10px] font-semibold uppercase tracking-[0.06em] text-stone-500 mb-3">Destacadas</span>
                <div className="scrollbar-hide flex gap-3 overflow-x-auto pb-1" style={{
            scrollbarWidth: 'none'
          }}>
                  {eliteStores.map(store => <FeaturedCard key={store.store_id ?? store.slug} store={store} />)}
                </div>
              </div>}
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="block text-[10px] font-semibold uppercase tracking-[0.06em] text-stone-500">
                  {activeRegion || activeCountry ? `${filteredStores.length} tiendas` : 'Todas las tiendas'}
                </span>
              </div>

              {loading ? <div className="stores-grid" aria-busy="true" aria-label="Cargando tiendas">
                  {[1, 2, 3, 4, 5, 6].map(i => <div key={i} aria-hidden="true" className="rounded-2xl bg-white p-3">
                      <div className="flex items-center gap-2.5">
                ...
                </div> : filteredStores.length === 0 ? <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <div className="stores-grid">
                    {visibleStores.map(store => <StoreCard key={store.store_id ?? store.slug ?? store.id} store={store} />)}
                  </div>
                  {visibleCount < filteredStores.length && <div ref={sentinelRef} className="h-px" />}
                </>}
            </div>
          </>}
      </div>
