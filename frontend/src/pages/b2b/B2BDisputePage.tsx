...
const REASONS = ['Producto no recibido', 'Producto diferente', 'Calidad inferior', "Documentación incorrecta", 'Precio incorrecto', 'Otro'];
export default function B2BDisputePage() {
  ...
  return <div className="fixed inset-0 flex flex-col bg-white">
      ...
      <div className="flex items-center gap-3 px-4 shrink-0 h-14 border-b border-stone-200 bg-white">
        <button onClick={() => navigate(-1)} className="text-stone-950 bg-transparent border-none cursor-pointer p-1">
          <ArrowLeft size={22} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 pb-[100px]">
        ...
        <div className="bg-stone-50 border border-stone-200/30 rounded-xl p-4 mb-6">
          ...
          <button onClick={() => navigate(-1)} className="bg-white border border-stone-200 rounded-full text-[13px] px-3.5 py-1.5 cursor-pointer text-stone-950 mb-2">
            Ir al chat &rarr;
          </button>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {REASONS.map(r => <button key={r} onClick={() => setReason(r)} className={`flex items-center justify-center h-9 rounded-full text-xs cursor-pointer transition-all duration-150 border ${reason === r ? 'bg-stone-950 text-white border-stone-950' : 'bg-white text-stone-950 border-stone-200'}`}>
                {r}
              </button>)}
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-stone-200 rounded-xl p-4 text-center cursor-pointer bg-white">
          ...
          <input ref={fileInputRef} type="file" multiple accept=".jpg,.jpeg,.png,.pdf" onChange={handleFileChange} className="hidden" />

          {/* Preview grid */}
          {files.length > 0 && <div className="flex flex-wrap gap-2 mt-2.5">
              {files.map((file, idx) => <div key={idx} className="relative w-[60px] h-[60px] rounded-xl overflow-hidden border border-stone-200 bg-stone-100">
                  {previews[idx] ? <img src={previews[idx]} alt="" className="w-full h-full object-cover" /> : <div className="flex items-center justify-center w-full h-full text-[9px] text-stone-500">
                      PDF
                    </div>}
                  <button onClick={e => {
              e.stopPropagation();
      ...
      {/* Confirmation modal */}
      <AnimatePresence>
        {showConfirm && <motion.div className="fixed inset-0 flex items-center justify-center px-6 z-[100]" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
            ...
            {/* Backdrop */}
            <motion.div className="fixed inset-0 bg-black/50" onClick={() => setShowConfirm(false)} initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          opacity: 0
            ...
            {/* Modal card */}
            <motion.div className="bg-white rounded-2xl p-6 max-w-[340px] w-full relative z-[101]" initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          scale: 1,
