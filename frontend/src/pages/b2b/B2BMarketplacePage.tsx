...
const CATEGORIES = ['Todos', 'Aceites', 'Quesos', 'Vinos', 'Embutidos', 'Conservas', 'Miel', 'Especias'];
const SORT_OPTIONS = [{
  value: 'relevance',
  label: 'Relevancia'
}, {
...
function ProductCard({
  ...
  return <div onClick={() => {
    ...
    if (pid) navigate(`/products/${pid}`);
      ...
      {product.image_url ? <img loading="lazy" src={product.image_url} alt={product.name || 'Producto'} className="w-full h-40 object-cover" /> : <div className="w-full h-40 bg-stone-100 flex items-center justify-center">
          <Package className="w-8 h-8 text-stone-300" />
        </div>}
      <div className="p-4">
        <p className="font-semibold text-stone-950 text-sm line-clamp-2">{product.name || 'Producto'}</p>
        {product.moq && <p className="text-xs text-stone-500 mt-1">MOQ: {product.moq} uds</p>}
        {lowestPrice && <p className="text-stone-950 font-bold text-sm mt-2">
            desde {convertAndFormatPrice((Number(lowestPrice?.unit_price_cents) || 0) / 100, 'EUR')}/ud
          </p>}
        {(product.b2b_prices?.length || 0) > 1 && <p className="text-xs text-stone-400 mt-0.5">{product.b2b_prices.length} tramos de precio</p>}
      </div>
    </div>;
}
function ProducerCard({
  ...
  return <div className="bg-white rounded-2xl shadow-sm p-4 hover:scale-[1.01] hover:-translate-y-0.5 transition-all duration-200">
      <div className="flex items-start gap-3">
        {producer.profile_image ? <img loading="lazy" src={producer.profile_image} alt={producer.full_name} className="w-12 h-12 rounded-full object-cover flex-shrink-0" /> : <div className="w-12 h-12 rounded-full bg-stone-100 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {producer.is_verified && <span className="inline-flex items-center gap-0.5 px-2 py-0.5 bg-stone-950 text-white rounded-full text-[10px] font-semibold shrink-0">
                <Check className="w-2.5 h-2.5" />
                Verificado
              </span>}
          </div>
          <p className="text-xs text-stone-500 mt-0.5">{producer.product_count || 0} productos</p>
          {producer.country && <div className="flex items-center gap-1 mt-0.5">
              <MapPin className="w-3 h-3 text-stone-400" />
              <span className="text-xs text-stone-500">{producer.country}</span>
            </div>}
          {producer.main_categories?.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
              {producer.main_categories.slice(0, 3).map(cat => <span key={cat} className="px-2 py-0.5 bg-stone-100 rounded-full text-xs text-stone-600">{cat}</span>)}
            </div>}
          {producer.certifications?.length > 0 && <div className="flex flex-wrap gap-1 mt-1.5">
              {producer.certifications.slice(0, 3).map(cert => <span key={cert} className="px-2 py-0.5 bg-stone-100 rounded-full text-[10px] text-stone-600">{cert}</span>)}
            </div>}
        </div>
      </div>
      <div className="mt-3 flex gap-2">
        <button onClick={() => onContact(producer.user_id || producer.id)} className="flex-1 py-2 bg-stone-950 text-white rounded-2xl text-sm font-medium flex items-center justify-center gap-1">
          Cotizar <ChevronRight className="w-4 h-4" />
        </button>
        <button onClick={() => onChat(producer.user_id || producer.id)} className="flex-1 py-2 border border-stone-200 text-stone-600 rounded-2xl text-sm font-medium flex items-center justify-center gap-1">
...
export default function B2BMarketplacePage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory !== 'Todos') {
      ...
      items = items.filter(p => (p.category || '').toLowerCase().includes(cat) || (p.categories || []).some(c => (c || '').toLowerCase().includes(cat)) || (p.name || '').toLowerCase().includes(cat));
    }
  ...
  const filteredProducers = useMemo(() => {
    ...
    if (search) {
      const q = search.toLowerCase();
      items = items.filter(p => (p.full_name || '').toLowerCase().includes(q) || (p.company_name || '').toLowerCase().includes(q));
    }
    if (selectedCategory !== 'Todos') {
      const cat = selectedCategory.toLowerCase();
      items = items.filter(p => (p.main_categories || []).some(c => (c || '').toLowerCase().includes(cat)));
    }
  ...
  const hasMore = activeTab === 'catalog' ? visibleCount < filteredProducts.length : visibleCount < filteredProducers.length;
  ...
  return <div className="min-h-screen bg-stone-50">
      <div className="bg-white border-b border-stone-200 px-4 pt-6 pb-0 max-w-[1100px] mx-auto">
        ...
        {activeTab !== 'rfq' && <div className="relative mb-4">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400" />
            <input type="text" value={search} onChange={e => {
          setSearch(e.target.value);
          resetPagination();
        }} placeholder={activeTab === 'catalog' ? 'Buscar productos...' : 'Buscar productores...'} className="w-full pl-9 pr-4 py-2.5 bg-stone-100 rounded-2xl text-sm focus:outline-none focus:border-stone-950" />
          </div>}

        <div className="flex gap-1">
          {TABS.map(tab => {
          const Icon = tab.icon;
          return <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={`flex-1 flex flex-col items-center gap-1 py-3 text-xs font-medium border-b-2 transition-colors ${activeTab === tab.id ? 'border-stone-950 text-stone-950' : 'border-transparent text-stone-500'}`}>
      ...
      {activeTab !== 'rfq' && <div className="max-w-[1100px] mx-auto px-4 pt-3 pb-1 flex flex-col gap-2.5">
          {/* Category pills */}
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide" style={{
        WebkitOverflowScrolling: 'touch'
      }}>
            {CATEGORIES.map(cat => <button key={cat} onClick={() => handleCategoryChange(cat)} className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${selectedCategory === cat ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'}`}>
                {cat}
              </button>)}
          </div>
      ...
      <div className="p-4 max-w-[1100px] mx-auto">
        {activeTab === 'catalog' && <>
            {catalogQuery.isLoading ? <div className="grid grid-cols-2 gap-3">
              ...
              </div> : paginatedProducts.length === 0 ? <div className="text-center py-16 text-stone-500">
                <Package className="w-12 h-12 mx-auto mb-3 text-stone-300" />
                <p>No hay productos disponibles</p>
              </div> : <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {paginatedProducts.map(product => <ProductCard key={product.id || product.product_id} product={product} convertAndFormatPrice={convertAndFormatPrice} />)}
                </div>
                {hasMore && <div className="flex justify-center mt-6">
                    <button onClick={() => setVisibleCount(c => c + PAGE_SIZE)} className="px-6 py-2.5 bg-white border border-stone-200 rounded-2xl text-sm font-medium text-stone-950 hover:bg-stone-50 transition-colors">
        ...
        {activeTab === 'producers' && <>
            {producersQuery.isLoading ? <div className="space-y-3">
              ...
              </div> : paginatedProducers.length === 0 ? <div className="text-center py-16 text-stone-500">
                <Users className="w-12 h-12 mx-auto mb-3 text-stone-300" />
                <p>No hay productores disponibles</p>
              </div> : <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {paginatedProducers.map(producer => <ProducerCard key={producer.user_id || producer.id} producer={producer} onContact={handleContactProducer} onChat={handleChat} />)}
                </div>
                {hasMore && <div className="flex justify-center mt-6">
                    <button onClick={() => setVisibleCount(c => c + PAGE_SIZE)} className="px-6 py-2.5 bg-white border border-stone-200 rounded-2xl text-sm font-medium text-stone-950 hover:bg-stone-50 transition-colors">
        ...
        {activeTab === 'rfq' && <div className="space-y-4">
