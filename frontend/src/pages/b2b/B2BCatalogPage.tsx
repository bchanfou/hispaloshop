...
export default function B2BCatalogPage() {
  ...
  const handleToggle = useCallback(async item => {
    ...
    try {
      ...
      toast.error(i18n.t('b2_b_catalog.noSePudoCambiarElEstado', 'No se pudo cambiar el estado'));
    } finally {
      setTogglingId(null);
    }
  }, [togglingId]);
  ...
  if (!user || user.role !== 'producer' && user.role !== 'importer') {
    return <div className="fixed inset-0 flex flex-col items-center justify-center gap-3 bg-white px-6 text-center">
        ...
        <p className="text-stone-950 text-[15px] font-semibold">{i18n.t('b2_b_catalog.noTienesAccesoAEstaSeccion', 'No tienes acceso a esta sección')}</p>
        <p className="text-stone-500 text-[13px]">{i18n.t('b2_b_catalog.necesitasUnPerfilDeProductorOImpor', 'Necesitas un perfil de productor o importador para acceder al catálogo B2B.')}</p>
  ...
  /* ── Render ──────────────────────────────────────────────── */
  return <div className="min-h-dvh bg-stone-50 pb-[88px] max-w-[1100px] mx-auto">
      {/* Keyframes for spinner */}
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>

      {/* Top bar */}
      <div className="sticky top-0 z-40 flex items-center gap-3 px-4 py-3.5 bg-white border-b border-stone-200">
        <button className="flex items-center justify-center w-9 h-9 rounded-full border border-stone-200 bg-white cursor-pointer shrink-0" onClick={() => navigate(-1)} aria-label="Volver">
          <ChevronLeft size={20} />
        </button>
        <span className="text-lg font-bold text-stone-950">{i18n.t('importer_dashboard.catalogoB2b', 'Catálogo B2B')}</span>
      </div>
      ...
      <div className="mx-4 mt-4 p-4 rounded-2xl bg-stone-100 flex gap-3 items-start">
        ...
        <div>
          <div className="text-[15px] font-semibold text-stone-950 mb-0.5">{i18n.t('importer_catalog.catalogoMayorista', 'Catálogo mayorista')}</div>
      ...
      {/* Empty state */}
      {!loading && products.length === 0 && <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
          <div className="w-14 h-14 rounded-full bg-stone-100 flex items-center justify-center mb-4 text-stone-500">
            <Package size={28} />
          </div>
          <div className="text-base font-semibold text-stone-950 mb-1.5">{i18n.t('b2_b_catalog.noTienesProductosEnElCatalogoB2b', 'No tienes productos en el catálogo B2B')}</div>
          <div className="text-[13px] text-stone-500 mb-5">
            Empieza añadiendo tu primer producto mayorista.
          </div>
          <button className="inline-flex items-center justify-center gap-1.5 px-6 py-2.5 rounded-full bg-stone-950 text-white text-[13px] font-semibold border-none cursor-pointer" onClick={() => navigate('/b2b/catalog/add')}>
            <Plus size={16} />
      ...
      return <div key={item.product_id || item.id} className="mx-4 mt-3 p-3.5 rounded-2xl border border-stone-200 bg-white">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.category && <span className="inline-block text-[11px] font-medium text-stone-500 bg-stone-100 rounded-full px-2 py-0.5">
                      {item.category}
                    </span>}
                  ...
                  <div className="flex items-center gap-4 mt-2.5 flex-wrap">
                    ...
                    <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full ${isActive ? 'bg-stone-950/[0.08] text-stone-950' : 'bg-stone-100 text-stone-500'}`}>
      ...
      {/* Floating add button */}
      {!loading && <button className="fixed bottom-6 left-4 right-4 z-30 flex items-center justify-center gap-2 py-3.5 rounded-full bg-stone-950 text-white text-[15px] font-semibold border-none cursor-pointer shadow-[0_4px_20px_rgba(0,0,0,0.18)]" onClick={() => navigate('/b2b/catalog/add')}>
          <Plus size={18} />
