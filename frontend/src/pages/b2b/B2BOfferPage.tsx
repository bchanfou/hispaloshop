...
import { ArrowLeft, Check, Search, Sparkles, Loader2, Package, Truck, CreditCard, Percent } from 'lucide-react';
...
const INCOTERMS = [{
  code: 'EXW',
  name: 'Ex Works',
  desc: 'El comprador asume todos los costes desde fábrica'
}, {
  code: 'FCA',
  name: 'Free Carrier',
  desc: 'Vendedor entrega en punto acordado'
}, {
  code: 'CPT',
  name: 'Carriage Paid To',
  desc: 'Vendedor paga transporte hasta destino'
}, {
  code: 'CIP',
  name: 'Carriage Insurance Paid',
  desc: 'Como CPT + seguro incluido'
}, {
  code: 'DAP',
  name: 'Delivered at Place',
  desc: 'Vendedor entrega en destino sin descargar'
}, {
  code: 'DPU',
  name: 'Delivered at Place Unloaded',
  desc: 'Vendedor entrega y descarga'
}, {
  code: 'DDP',
  name: 'Delivered Duty Paid',
  desc: 'Vendedor asume todos los costes hasta destino'
}, {
  code: 'FOB',
  name: 'Free on Board',
  desc: 'Vendedor entrega a bordo del buque'
}];
const EMPTY_FORM = {
  product_name: '',
  product_id: '',
...
const fmt = (n, currency = 'EUR') => {
  ...
  return `${sym}${Number(n).toLocaleString('es-ES', {
...
function StepIndicator({
  ...
  return <div className="flex items-center justify-center px-6 pt-4 pb-3">
      {STEP_LABELS.map((label, i) => {
      const done = i < current;
      const active = i === current;
      return <React.Fragment key={label}>
            {i > 0 && <div className={`flex-1 h-0.5 max-w-[48px] mx-1 rounded-sm transition-colors duration-300 ${done ? 'bg-stone-950' : 'bg-stone-200'}`} />}
            <div className="flex flex-col items-center min-w-[56px]">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-300 border-2 ${done || active ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-transparent text-stone-500'}`}>
                {done ? <Check size={14} strokeWidth={2.5} /> : i + 1}
              </div>
              <span className={`text-[11px] mt-1 ${active ? 'font-semibold text-stone-950' : 'font-normal text-stone-500'}`}>
                {label}
              </span>
            </div>
          </React.Fragment>;
...
function PillSelector({
  ...
  return <div className="flex flex-wrap gap-2">
      ...
      return <button key={v} type="button" onClick={() => onChange(v)} className={`h-9 px-4 rounded-full text-[13px] font-medium cursor-pointer transition-all duration-150 border-[1.5px] ${active ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-white text-stone-950'}`}>
            {l}
          </button>;
    })}
    </div>;
...
function ProductAutocomplete({
  ...
  return <div ref={wrapperRef} className="relative">
      <Search size={16} className="absolute left-3.5 top-3.5 text-stone-500 pointer-events-none" />
      <input type="text" placeholder="Buscar producto..." value={query} onChange={handleChange} onFocus={() => {
      ...
      {loading && <Loader2 size={14} className="absolute right-3.5 top-3.5 text-stone-400 animate-spin" />}

      {/* Dropdown */}
      {open && results.length > 0 && <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-stone-200 rounded-xl shadow-lg z-20 overflow-hidden max-h-[240px] overflow-y-auto">
          {results.map(product => <button key={product.id || product.product_id || product.name} type="button" onClick={() => handleSelect(product)} className="w-full flex items-center gap-3 px-3.5 py-2.5 hover:bg-stone-50 transition-colors text-left">
              {product.image_url ? <img src={product.image_url} alt={product.name} className="w-10 h-10 rounded-xl object-cover flex-shrink-0" /> : <div className="w-10 h-10 rounded-xl bg-stone-100 flex items-center justify-center flex-shrink-0">
                  <Package size={16} className="text-stone-300" />
                </div>}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-stone-950 truncate">{product.name}</p>
                {(product.b2b_prices?.[0] || product.price) && <p className="text-xs text-stone-500">
                    {product.b2b_prices?.[0] ? `desde ${fmt((Number(product.b2b_prices[0].unit_price_cents) || 0) / 100)}/ud` : `${fmt(product.price)}/ud`}
                  </p>}
              </div>
            </button>)}
        </div>}
...
function StepProducto({
  ...
  return <div className="flex flex-col gap-5">
      <div>
        ...
        <ProductAutocomplete value={form.product_name} onChange={val => set('product_name', val)} onSelect={handleProductSelect} />
      </div>

      <div>
        <label className="text-[13px] font-medium text-stone-950 mb-1.5 block">ID de producto (opcional)</label>
        <input type="text" placeholder="SKU o referencia" value={form.product_id} onChange={e => set('product_id', e.target.value)} className="w-full h-11 rounded-xl border border-stone-200 px-3.5 text-[13px] text-stone-500 bg-white outline-none box-border" />
      </div>

      <div>
        <label className="text-[13px] font-medium text-stone-950 mb-1.5 block">Cantidad *</label>
        <input type="number" min="1" placeholder="0" value={form.quantity} onChange={e => set('quantity', e.target.value)} className="w-full h-11 rounded-xl border border-stone-200 px-3.5 text-sm text-stone-950 bg-white outline-none box-border" />
        {form.moq && Number(form.quantity) > 0 && Number(form.quantity) < Number(form.moq) && <p className="text-xs text-stone-500 mt-1">
  ...
  return <div className="flex flex-col gap-5">
      <div>
        ...
        <div className="flex gap-3 items-end">
          <input type="number" min="0" step="0.01" placeholder="0.00" value={form.price_per_unit} onChange={e => set('price_per_unit', e.target.value)} className="flex-1 h-11 rounded-xl border border-stone-200 px-3.5 text-sm text-stone-950 bg-white outline-none box-border" />
          <PillSelector options={CURRENCIES} value={form.currency} onChange={v => set('currency', v)} />
        </div>
      </div>

      {subtotal > 0 && <motion.div initial={{
      opacity: 0,
      y: 8
    }} animate={{
      opacity: 1,
      ...
      <div>
        ...
        <PillSelector options={PAYMENT_TERMS} value={form.payment_terms} getLabel={o => o.label} getValue={o => o.value} onChange={v => set('payment_terms', v)} />
      </div>
    </div>;
}
  ...
  return <div className="flex flex-col gap-5">
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          return <button key={ic.code} type="button" onClick={() => set('incoterm', ic.code)} className={`flex flex-col items-start text-left p-3 rounded-xl cursor-pointer transition-all duration-150 border-[1.5px] ${active ? 'border-stone-950 bg-white' : 'border-stone-200 bg-white'}`}>
      ...
      <div>
        ...
        <input type="text" placeholder="Ej: Barcelona" value={form.incoterm_city} onChange={e => set('incoterm_city', e.target.value)} className={`w-full h-11 rounded-xl border px-3.5 text-sm text-stone-950 bg-white outline-none box-border transition-colors ${showCityError ? 'border-stone-500' : 'border-stone-200'}`} />
        {cityRequired && cityEmpty && <p className={`text-xs mt-1 ${showCityError ? 'text-stone-600 font-medium' : 'text-stone-500'}`}>
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          ...
          <input type="number" min="1" placeholder="0" value={form.delivery_days} onChange={e => set('delivery_days', e.target.value)} className="w-full h-11 rounded-xl border border-stone-200 px-3.5 text-sm text-stone-950 bg-white outline-none box-border" />
          {(!form.delivery_days || Number(form.delivery_days) < 1) && <p className="text-xs text-stone-500 mt-1">{i18n.t('b2_b_offer.indicaElPlazoMinimoDeEntrega', 'Indica el plazo mínimo de entrega')}</p>}
        </div>
        <div className="flex-1">
          <label className="text-[13px] font-medium text-stone-950 mb-1.5 block">{i18n.t('b2_b_offer.validezDias', 'Validez (días)')}</label>
          <input type="number" min="1" value={form.validity_days} onChange={e => set('validity_days', e.target.value)} className="w-full h-11 rounded-xl border border-stone-200 px-3.5 text-sm text-stone-950 bg-white outline-none box-border" />
        </div>
      </div>
    </div>;
}
  ...
  return <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          {modifiedFields.includes('product_name') && <span className="text-[10px] font-semibold text-stone-500 bg-stone-100 border border-stone-200 rounded-md px-1.5 py-px">Modificado</span>}
        </div>
        <p className="text-sm font-medium text-stone-950">{form.product_name}</p>
        {form.product_id && <p className="text-xs text-stone-500 mt-0.5">Ref: {form.product_id}</p>}
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        ...
        {subtotal > 0 && <div className="bg-stone-50 rounded-xl p-3">
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} className="w-[18px] h-[18px] accent-stone-950 mt-0.5 shrink-0" />
        <span className="text-[13px] text-stone-950 leading-normal">
...
export default function B2BOfferPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || err?.message || i18n.t('b2_b_offer.errorAlEnviarLaOferta', 'Error al enviar la oferta'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  /* ── Render ─────────────────────────────────────────────── */
  const stepContent = [<StepProducto key="step-0" form={form} set={set} />, <StepPrecio key="step-1" form={form} set={set} />, <StepLogistica key="step-2" form={form} set={set} submitted={attemptedAdvance} />, <StepRevisar key="step-3" form={form} prefillData={prefillData} confirmed={confirmed} setConfirmed={setConfirmed} />];
  return <div className="fixed inset-0 flex flex-col bg-white">
      ...
      <div className="flex items-center gap-3 shrink-0 px-4 py-3 border-b border-stone-200 bg-white z-10">
        <button onClick={goBack} className="bg-transparent border-none cursor-pointer p-1 text-stone-950">
          <ArrowLeft size={20} />
        </button>
      ...
      <div className="shrink-0 px-5 py-3 pb-[max(12px,env(safe-area-inset-bottom))] border-t border-stone-200 bg-white">
        <button onClick={goNext} disabled={!canAdvance || submitting} className={`flex items-center justify-center gap-2 w-full h-12 rounded-full border-none text-[15px] font-semibold transition-all duration-200 ${canAdvance && !submitting ? 'bg-stone-950 text-white cursor-pointer' : 'bg-stone-200 text-stone-500 cursor-not-allowed'}`}>
