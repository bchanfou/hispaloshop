...
const CommunityPreviewCard = ({
  ...
  return <div className="rounded-2xl border border-stone-200 bg-white overflow-hidden">
      {/* Cover */}
      <div className="overflow-hidden flex items-center justify-center text-[40px] relative" style={{
      aspectRatio: '3/1',
      background: coverPreview ? '#f5f5f4' : coverColor
    }}>
        {coverPreview ? <img src={coverPreview} alt="" className="w-full h-full object-cover block" /> : form.emoji || '🌿'}
        {/* Gradient overlay */}
        ...
        {/* Category badge */}
        {form.category && <span className="absolute top-2 left-2 bg-black/60 text-white text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase tracking-wide">
            {form.category}
          </span>}
      </div>
...
export default function CreateCommunityPage() {
          ...
          <div className="bg-stone-100 rounded-[14px] px-6 py-8">
            ...
            <div className="mt-4 bg-white rounded-xl px-4 py-3 border border-stone-200">
              ...
              <div className="h-1.5 bg-stone-200 rounded-full overflow-hidden">
                ...
                width: `${Math.min(100, (user?.followers_count || 0) / 100 * 100)}%`
              }} />
              </div>
  ...
  return <div className="bg-stone-50 min-h-screen">
      ...
      <div className="md:hidden pt-2.5 px-4 max-w-[540px] mx-auto">
        <button type="button" onClick={() => setShowPreview(v => !v)} className="w-full flex items-center justify-between px-3.5 py-2.5 bg-white border border-stone-200 rounded-2xl cursor-pointer text-[13px] font-semibold text-stone-950" aria-expanded={showPreview}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <label className="block mb-4 cursor-pointer">
          <div className="h-[120px] rounded-[14px] overflow-hidden flex items-center justify-center cursor-pointer border-2 border-dashed border-stone-200 relative" style={{
            background: coverPreview ? '#f5f5f4' : ['#d6d3d1', '#a8a29e', '#78716c', '#57534e', '#44403c'][(form.name.charCodeAt(0) || 100) % 5]
          }}>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <FormField label={i18n.t('create_community.urlDeLaComunidad', 'URL de la comunidad *')}>
            <div className="flex items-center">
              ...
              <input value={form.slug} onChange={e => update('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-'))} className={`flex-1 px-3 py-2.5 bg-stone-100 border border-l-0 rounded-r-xl outline-none text-stone-950 text-sm box-border ${slugStatus === 'taken' ? 'border-stone-400' : 'border-stone-200'}`} placeholder="aceites-de-espana" />
            </div>
          ...
          <FormField label={i18n.t('create_community.etiquetasMax5', 'Etiquetas (máx. 5)')} hint="Presiona Enter para añadir">
            <div className="flex gap-1.5 mb-1.5 flex-wrap">
              {form.tags.map(tag => <span key={tag} className="flex items-center gap-1 text-xs px-2.5 py-0.5 bg-stone-100 text-stone-950 rounded-full">
                  #{tag}
                  <button type="button" onClick={() => update('tags', form.tags.filter(t => t !== tag))} aria-label={`Eliminar etiqueta ${tag}`} className="bg-transparent border-none cursor-pointer text-sm text-stone-500 p-0 leading-none">
                    ×
                  </button>
                </span>)}
            </div>
