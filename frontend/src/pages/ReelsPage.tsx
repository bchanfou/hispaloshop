...
export default function ReelsPage() {
  ...
  const fetchReels = useCallback(async p => {
    ...
    try {
      ...
      let items = Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : Array.isArray(data?.reels) ? data.reels : Array.isArray(data?.data?.items) ? data.data.items // double-wrapped
      : [];
      ...
      if (items.length === 0 && p === 1) {
        ...
        const feedItems = Array.isArray(feedData) ? feedData : feedData?.items || feedData?.posts || [];
        items = feedItems.filter(item => item.type === 'reel' || item.is_reel === true || item.video_url && item.media_type === 'video').slice(0, 10);
      }
      ...
      if (p > 1) toast.error(i18n.t('reels.errorAlCargarMasReels', 'Error al cargar más reels'));
    } finally {
      setLoading(false);
  ...
  if (loading && reels.length === 0) {
    return <div className="h-dvh bg-black flex flex-col items-center justify-center gap-6" role="status" aria-label="Cargando reels">
        {/* Reel skeleton cards */}
  ...
  if (!loading && reels.length === 0) {
    return <div className="h-dvh bg-black flex flex-col items-center justify-center gap-4 px-8">
        <button onClick={() => navigate(-1)} className="fixed top-[max(1rem,env(safe-area-inset-top))] left-4 z-[100] w-10 h-10 rounded-full bg-transparent flex items-center justify-center" aria-label="Volver">
        ...
        <p className="text-white text-lg font-semibold font-sans text-center">{i18n.t('reels.aunNoHayReels', 'Aún no hay reels')}</p>
        <p className="text-white/50 text-sm font-sans text-center -mt-2">{i18n.t('reels.seElPrimeroEnPublicar', 'Sé el primero en publicar')}</p>
        <button onClick={() => navigate('/create/reel')} className="mt-2 text-black text-sm font-semibold font-sans bg-white rounded-full px-6 py-3 border-none cursor-pointer hover:bg-stone-100 transition-colors">
          Crear Reel
        </button>
        <button onClick={() => {
        setLoading(true);
  ...
  return <div ref={containerRef} className="h-dvh overflow-y-scroll snap-y snap-mandatory bg-black [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" style={{
    scrollBehavior: 'smooth'
      ...
      return <div className="fixed right-3 top-1/2 -translate-y-1/2 flex flex-col gap-1.5 z-[101]">
            {Array.from({
          length: dotCount
        }).map((_, i) => {
          const realIdx = dotStart + i;
          return <div key={realIdx} className={`rounded-full transition-all duration-200 ${realIdx === activeIndex ? 'w-2 h-2 bg-white' : 'w-1.5 h-1.5 bg-white/30'}`} />;
        })}
          </div>;
    })()}
      ...
      <div className="fixed top-0 left-0 right-0 z-[100] bg-gradient-to-b from-black/40 to-transparent pt-[max(8px,env(safe-area-inset-top))]">
        <div className="flex items-center gap-2 px-4 pb-1">
          <button onClick={() => navigate(-1)} className="w-11 h-11 rounded-full bg-transparent flex items-center justify-center shrink-0" aria-label="Volver">
          ...
          <div className="flex gap-0 overflow-x-auto scrollbar-hide flex-1">
            <SlideTabIndicator tabs={REEL_TABS.map(t => ({
            id: t.key,
            label: t.label
          }))} activeTab={activeTab} onTabChange={setActiveTab} variant="pill" layoutId="reels-tab" className="flex-1" />
          </div>
        </div>
      </div>

      {reels.map((reel, idx) => <div key={reel.id || reel.post_id || `reel-${idx}`} data-reel-item data-index={idx} className="h-dvh snap-start snap-always">
          <ReelCard reel={reel} isActive={idx === activeIndex} priority={idx <= 1} nextVideoUrl={reels[idx + 1]?.video_url || reels[idx + 1]?.videoUrl} />
        </div>)}

      {!hasMore && reels.length > 0 && <div className="h-20 flex items-center justify-center text-white/40 text-[13px] font-sans">
          No hay más reels
        </div>}
    </div>;
}