...
function TabContent({
  ...
  if (isLoading) {
    ...
    return <div className={`grid gap-1 ${cols === 3 ? 'grid-cols-3' : 'grid-cols-2 sm:grid-cols-2 gap-3'}`}>
        {Array.from({
        length: 12
      }).map((_, i) => <div key={i} className={`bg-stone-100 animate-pulse rounded-sm ${cols === 3 ? 'aspect-square' : 'aspect-[4/5] rounded-xl'}`} />)}
      </div>;
  }
  ...
  if (isError) {
    return <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <button onClick={() => refetch()} className="bg-stone-950 text-white rounded-full px-6 py-2.5 text-sm font-semibold hover:bg-stone-800 transition-colors border-none cursor-pointer">
          Reintentar
        </button>
      </div>;
  }
  ...
  if (allItems.length === 0) {
    const emptyConfig: Record<TabKey, {
      icon: React.ReactNode;
      text: string;
    }> = {
      posts: {
        icon: <Grid3X3 className="w-12 h-12 text-stone-300" />,
        text: 'No tienes posts guardados'
      },
      reels: {
        icon: <Play className="w-12 h-12 text-stone-300" />,
        text: 'No tienes reels guardados'
      },
      products: {
        icon: <Package className="w-12 h-12 text-stone-300" />,
        text: 'Tu lista de deseos está vacía'
      },
      recipes: {
        icon: <BookOpen className="w-12 h-12 text-stone-300" />,
        text: 'No tienes recetas guardadas'
      },
      collections: {
        icon: <FolderOpen className="w-12 h-12 text-stone-300" />,
    ...
    return <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-sm text-stone-500 text-center">{i18n.t('saved.guardaContenidoTocandoElIconoDeMar', 'Guarda contenido tocando el icono de marcador')}</p>
      </div>;
  }
...
function PostsGrid({
  ...
  return <div className="grid grid-cols-3 gap-1">
      ...
      return <button key={postId} onClick={() => navigate(`/posts/${postId}`)} className="relative aspect-square bg-stone-100 overflow-hidden group cursor-pointer border-none p-0">
...
function ReelsGrid({
  ...
  return <div className="grid grid-cols-3 gap-1">
      ...
      return <button key={reelId} onClick={() => navigate(`/posts/${reelId}`)} className="relative aspect-[9/16] bg-stone-100 overflow-hidden group cursor-pointer border-none p-0">
            {thumb ? <img src={resolveUserImage(thumb)} alt="" className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center">
            ...
            {reel.views_count != null && <div className="absolute bottom-1.5 left-1.5 flex items-center gap-1">
...
function ProductsGrid({
  ...
  return <div className="grid grid-cols-2 gap-3">
      ...
      return <Link key={productId} to={`/products/${productId}`} className="block no-underline">
            <div className="overflow-hidden rounded-xl bg-white border border-stone-100">
              <div className="relative aspect-square overflow-hidden bg-stone-50">
                {img ? <img src={resolveUserImage(img)} alt={product.name || ''} className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center">
                ...
                {product.original_price != null && price != null && Number(product.original_price) > Number(price) && <span className="absolute top-2 left-2 flex items-center gap-0.5 rounded-full bg-stone-950 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
                    <ArrowDown size={10} />
                    Precio reducido
                  </span>}
              </div>
              <div className="px-2.5 py-2">
                ...
                {product.store_name && <p className="text-xs text-stone-500 mt-0.5 truncate">{product.store_name}</p>}
                {price != null && <div className="mt-1 flex items-center gap-1.5">
                    <p className="text-sm font-bold text-stone-950">
                      {Number(price).toFixed(2)} &euro;
                    </p>
                    {product.original_price != null && Number(product.original_price) > Number(price) && <p className="text-xs text-stone-400 line-through">
...
function CollectionsGrid({
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      toast.error(i18n.t('saved.errorAlCrearLaColeccion', 'Error al crear la colección'));
    }
  }, [newName, queryClient]);
  return <div className="space-y-3">
      {/* Create new collection */}
      {creating ? <div className="flex items-center gap-2">
          <input autoFocus type="text" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => {
        if (e.key === 'Enter') handleCreate();
      }} placeholder={i18n.t('saved.nombreDeLaColeccion', 'Nombre de la colección')} className="flex-1 h-10 rounded-xl border border-stone-200 bg-white px-3 text-sm text-stone-950 placeholder:text-stone-400 outline-none" />
        ...
        </div> : <button onClick={() => setCreating(true)} className="flex items-center gap-2 w-full h-14 rounded-xl border border-dashed border-stone-200 bg-stone-50 px-4 text-sm font-medium text-stone-600 cursor-pointer hover:border-stone-400 hover:bg-stone-100 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        return <button key={id} type="button" onClick={() => navigate(`/saved/collections/${id}`)} className="overflow-hidden rounded-xl bg-white border border-stone-100 text-left cursor-pointer p-0">
              <div className="relative aspect-[4/3] overflow-hidden bg-stone-100">
                {coverImg ? <img src={resolveUserImage(coverImg)} alt="" className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center">
...
function RecipesGrid({
  ...
  return <div className="grid grid-cols-2 gap-3">
      ...
      return <Link key={recipeId} to={`/recipes/${recipeId}`} className="block no-underline">
            <div className="overflow-hidden rounded-xl bg-white">
              ...
              <div className="px-1 pt-2 pb-1.5">
                ...
                <div className="mt-1 flex items-center gap-2 flex-wrap">
                  {cookTime > 0 && <span className="flex items-center gap-0.5 text-[11px] text-stone-500">
                      <Clock size={10} />
                      {cookTime} min
                    </span>}
                  {recipe.difficulty && <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${recipe.difficulty === 'hard' ? 'bg-stone-950 text-stone-50' : recipe.difficulty === 'medium' ? 'bg-stone-100 text-stone-700' : 'bg-stone-100 text-stone-600'}`}>
                      {recipe.difficulty === 'easy' ? 'Fácil' : recipe.difficulty === 'medium' ? 'Media' : recipe.difficulty === 'hard' ? i18n.t('recipes.hard', 'Difícil') : ''}
                    </span>}
                </div>
                {recipe.author_name && <p className="text-xs text-stone-500 mt-0.5 truncate">{recipe.author_name}</p>}
              </div>
            </div>
          </Link>;
    })}
