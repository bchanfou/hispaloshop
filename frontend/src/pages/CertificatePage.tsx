...
import {
  AlertTriangle, Award, Check, ChevronDown, ChevronRight, Copy, Download,
  FileCheck, Globe, MapPin, Package, Printer, Send, Shield,
  Eye, CheckCircle2,
} from 'lucide-react';
...
const CERT_LANGUAGES = [
  { code: 'es', label: t('certificate.espanol', 'Español'), flag: '🇪🇸' },
...
const NUTRIENT_LABELS = {
  calories: t('certificate.nutritionLabels.energy', 'Energía'), energy: 'Energía', energia: 'Energía', valor_energetico: 'Energía', valor_energético: 'Energía',
  protein: t('certificate.nutritionLabels.protein', 'Proteínas'), proteinas: 'Proteínas', proteína: 'Proteínas', proteínas: 'Proteínas',
...
const ALLERGEN_ICONS = {
  ...
  'frutos secos': '🥜', nueces: '🥜', nuts: '🥜', 'tree nuts': '🥜',
  cacahuete: '🥜', peanut: '🥜', maní: '🥜',
  soja: '🫘', soy: '🫘',
  pescado: '🐟', fish: '🐟',
...
export default function CertificatePage() {
  ...
  const fetchTranslation = useCallback(async (lang) => {
    ...
    } catch { setTranslatedCert(null); }
    finally { setTranslating(false); }
  }, [productId]);

  useEffect(() => { fetchTranslation(certLang); }, [certLang, fetchTranslation]);
  ...
  const handleBuy = () => {
    if (canBuyFromStore) navigate(`/store/${storeSlug}?product=${productId}`);
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-950" data-testid="certificate-page">
      ...
      <div className={`mx-auto max-w-[600px] px-4 ${isQrScan ? 'pt-6' : 'py-8 sm:px-6'} pb-8`}>
        ...
        <div className={`${isQrScan ? '' : 'mt-5'} overflow-hidden rounded-[32px] border border-stone-200 bg-white shadow-sm dark:bg-stone-900 dark:border-stone-800`} dir={isRtl ? 'rtl' : 'ltr'}>
          ...
          <div className="flex flex-col sm:flex-row">
            <div className="flex flex-none items-center justify-center border-b border-stone-100 bg-stone-50 p-8 sm:w-52 sm:border-b-0 sm:border-r dark:bg-stone-800 dark:border-stone-700">
              {productImage ? (
                <img src={productImage} alt={product.name} loading="lazy" className="max-h-[300px] w-full rounded-2xl object-cover" />
              ) : (
                <div className="flex h-32 w-32 items-center justify-center rounded-full bg-stone-100 dark:bg-stone-700">
            ...
            <div className="flex-1 p-6">
              ...
              {certifications.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {certifications.map(item => (
                    <span key={item} className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1 text-xs font-medium text-stone-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300">
              ...
              {certNumber && (
                <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-stone-400">
                  ...
                  {issueDate && <span>{txt.issued}: <span className="font-semibold text-stone-600 dark:text-stone-300">{new Date(issueDate).toLocaleDateString(certLang === 'es' ? 'es-ES' : certLang, { day: 'numeric', month: 'short', year: 'numeric' })}</span></span>}
                </div>
              )}
        ...
        {(allergens.length > 0 || certifications.some(c => /sin gluten|gluten.free|vegano|vegan|sin lactosa|lactose.free/i.test(c))) && (
          <div className="mt-5 rounded-[28px] border border-stone-100 bg-white p-6 shadow-sm dark:bg-stone-900 dark:border-stone-800">
        ...
        {nutrition && typeof nutrition === 'object' && Object.keys(nutrition).length > 0 && (
          <div className="mt-5 overflow-hidden rounded-[28px] border-2 border-stone-950 bg-white dark:bg-stone-900">
            ...
            <div className="divide-y divide-stone-100 dark:divide-stone-800">
              {Object.entries(nutrition).map(([key, value], i) => {
                ...
                return (
                  <div key={key} className={`flex items-center justify-between px-5 py-2.5 text-sm ${i === 0 ? 'font-bold text-stone-950 dark:text-white' : 'text-stone-700 dark:text-stone-300'}`}>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <div className="rounded-[28px] border border-stone-100 bg-white p-6 shadow-sm dark:bg-stone-900 dark:border-stone-800">
            ...
            {ingredients.length > 0 ? (
              <p className="text-sm leading-relaxed text-stone-700 dark:text-stone-300">{ingredients.join(', ')}.</p>
            ) : (
        ...
        <div className="mt-4 rounded-[28px] border border-stone-100 bg-white p-6 shadow-sm dark:bg-stone-900 dark:border-stone-800">
          ...
          {(product.country_origin || product.region) && (
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {product.country_origin && (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs text-stone-600 dark:bg-stone-800 dark:border-stone-700 dark:text-stone-300">
                    <MapPin className="h-3 w-3" /> {product.country_origin}
                  </span>
                )}
              </div>
          ...
          {canBuyFromStore && (
            <button type="button" onClick={handleBuy} className="mt-4 flex items-center gap-1.5 text-sm font-medium text-stone-950 hover:text-stone-600 dark:text-white">
          ...
          <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-start">
            ...
            <div className="min-w-0 flex-1 text-center sm:text-left">
              ...
              <div className="mt-3 flex flex-wrap justify-center gap-2 sm:justify-start">
                <button type="button" onClick={() => { navigator.clipboard.writeText(certUrl); toast.success(txt.link_copied); }} className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-medium text-stone-700 hover:bg-stone-50 dark:bg-stone-800 dark:border-stone-700 dark:text-stone-300">
                  <Copy className="h-3 w-3" /> {txt.copy_link}
                </button>
                <button type="button" onClick={async () => { if (navigator.share) { try { await navigator.share({ title: `${txt.cert_title} - ${product.name}`, url: certUrl }); } catch {} } else { navigator.clipboard.writeText(certUrl); toast.success(txt.link_copied); } }} className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-medium text-stone-700 hover:bg-stone-50 dark:bg-stone-800 dark:border-stone-700 dark:text-stone-300">
