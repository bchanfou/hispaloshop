...
function WizardStepper({
  ...
  return <div className="flex items-center gap-1 mb-6">
      {WIZARD_STEPS.map((step, i) => {
      const Icon = step.icon;
      const done = i < current;
      const active = i === current;
      return <React.Fragment key={step.key}>
            {i > 0 && <div className={`flex-1 h-[2px] transition-colors ${done ? 'bg-stone-950' : 'bg-stone-200'}`} />}
            <div className="flex flex-col items-center gap-1">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-semibold transition-all ${done ? 'bg-stone-950 text-white' : active ? 'bg-stone-950 text-white ring-4 ring-stone-200' : 'bg-stone-100 text-stone-400'}`}>
...
function ImageUploader({
  ...
  const handleFileSelect = async e => {
    ...
    if (availableSlots <= 0) {
      toast.error(t('producerProducts.maxImagesError', {
        max: maxImages
      }));
      return;
    }
    ...
    try {
      for (const file of filesToUpload) {
        ...
        if (!file.type.startsWith('image/')) {
          toast.error(t('producerProducts.notAnImage', {
            name: file.name
          }), {
            duration: 3000
          });
        ...
        if (file.size > 5 * 1024 * 1024) {
          toast.error(t('producerProducts.fileTooLarge', {
            name: file.name
          }), {
            duration: 3000
          });
        ...
        try {
          ...
          toast.success(t('producerProducts.uploadSuccess'), {
            duration: 2000
          });
        } catch (uploadError) {
          toast.error(uploadError?.message || t('producerProducts.uploadFailed'), {
            duration: 4000
          });
        }
      }
    } catch {
      toast.error(t('producerProducts.uploadFailed'), {
        duration: 4000
      });
    } finally {
      // Reset file input
  ...
  return <div className="space-y-3">
      <label className="block text-sm font-medium text-stone-600 mb-1">
        {t('producerProducts.imagesCount', {
      ...
      <div className="flex flex-wrap gap-3">
        {normalizedImages.filter(img => img && typeof img === 'string' && img.trim()).map((image, index) => <div key={index} className="relative group w-24 h-24 rounded-2xl overflow-hidden border border-stone-200 bg-stone-50">
            <img loading="lazy" src={image} alt={`Product ${index + 1}`} className="w-full h-full object-cover" onError={e => {
          e.target.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect fill="%23f5f5f4" width="100" height="100"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23a8a29e" font-size="12">Error</text></svg>';
            ...
            <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 p-1 bg-stone-950 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Eliminar imagen ${index + 1}`} title="Remove image">
              <X className="w-3 h-3" />
            </button>
          </div>)}
        
        {/* Upload Button */}
        {currentCount < maxImages && <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="w-24 h-24 rounded-2xl border-2 border-dashed border-stone-200 hover:border-stone-950 hover:bg-stone-50 transition-colors flex flex-col items-center justify-center gap-1 text-stone-400 hover:text-stone-950 disabled:opacity-50 disabled:cursor-not-allowed">
...
function StockEditor({
  ...
  if (editing) {
    return <div className="flex items-center gap-2">
        <input type="number" min="0" value={stock} onChange={e => setStock(e.target.value)} className="w-20 h-8 text-sm px-3 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid={`stock-input-${productId}`} />
        <button type="button" onClick={handleSave} disabled={updateStockLoading} className="flex items-center px-3 py-1.5 text-sm font-medium bg-stone-950 hover:bg-stone-800 disabled:opacity-40 text-white rounded-2xl transition-colors">
  ...
  return <button onClick={() => setEditing(true)} className={`flex items-center gap-2 px-3 py-1.5 rounded-2xl text-sm font-medium transition-colors ${isOutOfStock ? 'bg-stone-50 text-stone-700 border border-stone-200 hover:bg-stone-100' : isLowStock ? 'bg-stone-50 text-stone-700 border border-stone-200 hover:bg-stone-100' : 'bg-stone-50 text-stone-700 border border-stone-200 hover:bg-stone-100'}`} data-testid={`stock-badge-${productId}`}>
      {isOutOfStock ? <>
...
export default function ProducerProducts() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Packs configuration
    packs: [],
    // Array of {quantity: number, price: number, label: ''}
  ...
  const countryOriginOptions = [{
    code: 'ES',
    name: t('admin.countries.ES', 'España')
  }, {
    code: 'IT',
    name: 'Italia'
  }, {
    ...
    code: 'CA',
    name: t('admin.countries.CA', 'Canadá')
  }, {
    code: 'MX',
    name: 'México'
  }, {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const normalizedImages = Array.isArray(formData.images) ? formData.images.filter(img => img && typeof img === 'string' && img.trim()) : [];
      ...
      const data = {
        ...
        ingredients: formData.ingredients.length > 0 ? formData.ingredients.map(i => i.name || i).filter(Boolean) : ingredientsStr.split(',').map(i => i.trim()).filter(Boolean),
        allergens: [...formData.allergens, ...allergensStr.split(',').map(a => a.trim()).filter(a => a && !formData.allergens.includes(a))],
        certifications: [...formData.certifications, ...certificationsStr.split(',').map(c => c.trim()).filter(c => c && !formData.certifications.includes(c))],
        // New fields
  ...
  const startEdit = product => {
    ...
    setFormData({
      ...
      ingredients: product.ingredients?.map(i => typeof i === 'string' ? {
        name: i,
        origin: ''
      } : i) || [],
  ...
  if (showCreateForm) {
    ...
    const nutritionalFields = [{
      key: 'calories',
      label: t('producerProducts.nutritionLabels.calories'),
      unit: 'kcal'
    }, {
      key: 'protein',
      label: t('producerProducts.nutritionLabels.protein'),
      unit: 'g'
    }, {
      key: 'carbohydrates',
      label: t('producerProducts.nutritionLabels.carbohydrates'),
      unit: 'g'
    }, {
      key: 'sugars',
      label: t('producerProducts.nutritionLabels.sugars'),
      unit: 'g'
    }, {
      key: 'fat',
      label: t('producerProducts.nutritionLabels.fat'),
      unit: 'g'
    }, {
      key: 'saturated_fat',
      label: t('producerProducts.nutritionLabels.saturated_fat'),
      unit: 'g'
    }, {
      key: 'fiber',
      label: t('producerProducts.nutritionLabels.fiber'),
      unit: 'g'
    }, {
      key: 'sodium',
      label: t('producerProducts.nutritionLabels.sodium'),
      unit: 'mg'
    }, {
      key: 'salt',
      label: t('producerProducts.nutritionLabels.salt'),
      unit: 'g'
    }];
    ...
    const EU_ALLERGENS = [{
      ...
      key: 'crustaceans',
      label: t('producer_products.crustaceos', 'Crustáceos')
    }, {
      key: 'egg',
      label: 'Huevo'
    }, {
      ...
      key: 'dairy',
      label: t('home.dairy', 'Lácteos')
    }, {
      key: 'nuts',
      label: t('producer_products.frutosDeCascara', 'Frutos de cáscara')
    }, {
      key: 'celery',
      label: 'Apio'
    }, {
      ...
      key: 'sesame',
      label: t('producerProducts.commonAllergens.sesame', 'Sésamo')
    }, {
      key: 'sulfites',
      label: 'Sulfitos'
    }, {
    ...
    const commonCertifications = [{
      key: 'organic',
      label: t('producerProducts.commonCertifications.organic')
    }, {
      key: 'vegan',
      label: t('producerProducts.commonCertifications.vegan')
    }, {
      key: 'glutenFree',
      label: t('producerProducts.commonCertifications.glutenFree')
    }, {
      key: 'kosher',
      label: t('producerProducts.commonCertifications.kosher')
    }, {
      key: 'halal',
      label: t('producerProducts.commonCertifications.halal')
    }, {
      key: 'bio',
      label: t('producerProducts.commonCertifications.bio')
    }, {
      key: 'fairTrade',
      label: t('producerProducts.commonCertifications.fairTrade')
    }, {
      key: 'dop',
      label: t('producerProducts.commonCertifications.dop')
    }];
    ...
    return <div className="pb-8">
        ...
        resetForm();
        setWizardStep(0);
      }} className="flex items-center gap-2 text-stone-600 hover:text-stone-950 mb-6" data-testid="back-button">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 md:p-6 max-w-3xl">
          ...
          <form onSubmit={handleSubmit}>
            <AnimatePresence mode="wait">
              {/* ═══ STEP 0 — IMAGES ═══ */}
              {wizardStep === 0 && <motion.div key="step-images" initial={{
              opacity: 0,
              x: 20
            }} animate={{
              opacity: 1,
              ...
              {/* ═══ STEP 1 — INFO ═══ */}
              {wizardStep === 1 && <motion.div key="step-info" initial={{
              opacity: 0,
              x: 20
            }} animate={{
              opacity: 1,
              ...
              duration: 0.2
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    ...formData,
                    name: e.target.value
                  })} placeholder={t('producerProducts.productNamePlaceholder')} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-name-input" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.category')} *</label>
                      <select value={formData.category_id} onChange={e => setFormData({
                    ...formData,
                    category_id: e.target.value
                  })} className="w-full px-4 py-2 rounded-2xl border border-stone-200 bg-white text-sm" data-testid="product-category-select">
                ...
                })} className="w-full px-4 py-2 rounded-2xl border border-stone-200 min-h-[80px] text-sm" placeholder={t('producerProducts.descriptionPlaceholder')} data-testid="product-description-input" />
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.unitPrice')} *</label>
                      <input type="number" step="0.01" min="0" value={formData.price} onChange={e => setFormData({
                    ...formData,
                    price: e.target.value
                  })} placeholder="0.00" className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-price-input" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.countryOrigin')} *</label>
                      <select value={formData.country_origin} onChange={e => setFormData({
                    ...formData,
                    country_origin: e.target.value
                  })} className="w-full px-3 py-2 rounded-2xl border border-stone-200 bg-white text-sm" data-testid="product-country-input">
                        <option value="">{t('producerProducts.selectCountry', 'Seleccionar país')}</option>
                        {countryOriginOptions.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.sku')}</label>
                      <input value={formData.sku} onChange={e => setFormData({
                    ...formData,
                    sku: e.target.value
                  })} placeholder={t('producerProducts.skuPlaceholder')} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-sku-input" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.flavorVariant')}</label>
                      <input value={formData.flavor} onChange={e => setFormData({
                    ...formData,
                    flavor: e.target.value
                  })} placeholder={t('producerProducts.flavorPlaceholder')} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-flavor-input" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-stone-600 mb-1">{t('producerProducts.stock', 'Stock inicial')} *</label>
                      <input type="number" min="0" value={formData.stock} onChange={e => setFormData({
                    ...formData,
                    stock: e.target.value
                  })} placeholder="100" className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-stock-input" />
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-stone-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="number" step="0.01" min="0" value={formData.shipping_cost} onChange={e => setFormData({
                      ...formData,
                      shipping_cost: e.target.value
                    })} placeholder={t('producerProducts.shippingCostPlaceholder')} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="product-shipping-cost-input" />
                      </div>
                      <div>
                        <label className="block text-xs text-stone-600 mb-1">{t('producerProducts.freeShippingFrom')}</label>
                        <input type="number" min="1" value={formData.free_shipping_min_qty} onChange={e => setFormData({
                      ...formData,
                  ...
                  <div className="pt-3 border-t border-stone-100">
                    ...
                    {formData.packs.length === 0 ? <p className="text-sm text-stone-500 text-center py-2">{t('producerProducts.noPacksConfigured')}</p> : <div className="space-y-2">
                        {formData.packs.map((pack, index) => {
                    const discount = calculatePackDiscount(pack);
                    return <div key={index} className="flex items-center gap-2 p-2.5 bg-stone-50 rounded-2xl">
                  ...
                  <div className="pt-3 border-t border-stone-100">
                    ...
                    {formData.b2b_enabled && <div className="space-y-3">
                        {/* MOQ */}
                        <div>
                          <label className="block text-xs text-stone-600 mb-0.5">{t('producer_products.pedidoMinimoUds', 'Pedido mínimo (uds.)')}</label>
                          <input type="number" min="1" value={formData.b2b_moq} onChange={e => setFormData({
                      ...formData,
                      b2b_moq: e.target.value
                    })} placeholder="10" className="w-32 px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                        </div>
                        ...
                        setFormData({
                          ...
                          {formData.b2b_tiers.length === 0 ? <p className="text-sm text-stone-500 text-center py-2">Sin tramos configurados</p> : <div className="space-y-2">
                              {formData.b2b_tiers.map((tier, index) => {
                        const retailPrice = parseFloat(formData.price) || 0;
                        const tierPrice = parseFloat(tier.unit_price) || 0;
                        const discountPct = retailPrice > 0 && tierPrice > 0 ? Math.round((1 - tierPrice / retailPrice) * 100) : null;
                        return <div key={index} className="flex items-center gap-2 p-2.5 bg-stone-50 rounded-2xl">
                              ...
                              }} placeholder="10" className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                                      </div>
                                      <div>
                                        <label className="block text-xs text-stone-600 mb-0.5">Precio/ud. (€)</label>
                                        <input type="number" step="0.01" min="0" value={tier.unit_price} onChange={e => {
                                const tiers = [...formData.b2b_tiers];
                                tiers[index] = {
                                  ...tiers[index],
                              ...
                              }} placeholder="8.50" className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                                      </div>
                                    </div>
                                    {discountPct !== null && discountPct > 0 && <div className="bg-stone-100 text-stone-700 px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap">
                                        -{discountPct}%
                                      </div>}
                                    <button type="button" onClick={() => {
              ...
              {/* ═══ STEP 2 — COMPOSITION ═══ */}
              {wizardStep === 2 && <motion.div key="step-composition" initial={{
              opacity: 0,
              x: 20
            }} animate={{
              opacity: 1,
              ...
              duration: 0.2
                  ...
                  <div className="rounded-2xl border border-stone-200 p-5">
                    ...
                    {formData.ingredients.length === 0 ? <div>
                  ...
                  <div className="rounded-2xl border border-stone-200 p-5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {nutritionalFields.map(field => <div key={field.key}>
                          <label className="block text-xs text-stone-600 mb-0.5">
                            {field.label} <span className="text-stone-400">({field.unit})</span>
                          </label>
                          <input type="number" step="0.1" min="0" value={formData.nutritional_info[field.key]} onChange={e => setFormData({
                      ...formData,
                      nutritional_info: {
                        ...formData.nutritional_info,
                        [field.key]: e.target.value
                      }
                    })} placeholder="0" className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid={`nutrition-${field.key}`} />
                        </div>)}
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-stone-200 p-5">
                    ...
                    return <label key={allergen.key} className="flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" checked={isSelected} onChange={() => {
                        if (isSelected) setFormData({
                          ...formData,
                          allergens: formData.allergens.filter(a => a !== allergen.label)
                        });else setFormData({
                          ...formData,
                          allergens: [...formData.allergens, allergen.label]
                        });
                      }} className="accent-stone-950 w-4 h-4 rounded" data-testid={`allergen-${allergen.key}`} />
                            <span className="text-sm text-stone-700">{allergen.label}</span>
                          </label>;
                  })}
                      {/* Otros */}
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" checked={allergensStr.length > 0} onChange={e => {
                      if (!e.target.checked) setAllergensStr('');
                    }} className="accent-stone-950 w-4 h-4 rounded" data-testid="allergen-otros" />
                        <span className="text-sm text-stone-700">Otros</span>
                      </label>
                    </div>
                    <input value={allergensStr} onChange={e => setAllergensStr(e.target.value)} placeholder={t('producer_products.especificaOtrosAlergenos', 'Especifica otros alérgenos...')} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" data-testid="allergens-text-input" />
                  </div>
                  ...
                  <div className="rounded-2xl border border-stone-200 p-5">
                    ...
                    return <button key={cert.key} type="button" onClick={() => {
                      if (isSelected) setFormData({
                        ...formData,
                        certifications: formData.certifications.filter(c => c !== cert.label)
                      });else setFormData({
                        ...formData,
                        certifications: [...formData.certifications, cert.label]
                      });
              ...
              {wizardStep === 3 && <motion.div key="step-publish" initial={{
              opacity: 0,
              x: 20
            }} animate={{
              opacity: 1,
              ...
              duration: 0.2
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {normalizedImgs.map((img, i) => <div key={i} className="w-20 h-20 shrink-0 rounded-2xl overflow-hidden border border-stone-200 bg-stone-50">
                  ...
                  <div className="rounded-2xl border border-stone-200 divide-y divide-stone-100">
                    ...
                    <div className="px-5 py-3 flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-stone-950">{formData.price ? parseFloat(formData.price).toLocaleString('es-ES', {
                      style: 'currency',
                      currency: 'EUR'
                    }) : '—'}</span>
                    </div>
                    ...
                    {formData.allergens.length > 0 && <div className="px-5 py-3">
                        <span className="text-sm text-stone-500 block mb-1.5">{t('producerProducts.allergens')}</span>
                        <div className="flex flex-wrap gap-1.5">
                          {formData.allergens.map(a => <span key={a} className="inline-flex items-center gap-1 rounded-full bg-stone-950 px-2.5 py-0.5 text-[11px] font-medium text-white">
                              <AlertTriangle className="w-3 h-3" />{a}
                            </span>)}
                        </div>
                      </div>}
                    {formData.certifications.length > 0 && <div className="px-5 py-3">
                        <span className="text-sm text-stone-500 block mb-1.5">{t('producerProducts.certifications')}</span>
                        <div className="flex flex-wrap gap-1.5">
                          {formData.certifications.map(c => <span key={c} className="inline-flex items-center gap-1 rounded-full border border-stone-200 px-2.5 py-0.5 text-[11px] font-medium text-stone-700">
            ...
            <div className="flex items-center justify-between pt-5 mt-5 border-t border-stone-100">
              <button type="button" onClick={wizardStep === 0 ? () => {
              setShowCreateForm(false);
              setEditingProduct(null);
              resetForm();
              setWizardStep(0);
            } : handleBack} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium border border-stone-200 rounded-2xl hover:bg-stone-50 transition-colors">
                <ArrowLeft className="w-4 h-4" />
                {wizardStep === 0 ? t('common.cancel') : 'Atrás'}
              </button>

              {wizardStep < 3 ? <button type="button" onClick={handleNext} className="flex items-center gap-1.5 px-5 py-2 text-sm font-medium bg-stone-950 hover:bg-stone-800 text-white rounded-2xl transition-colors">
                  Siguiente
                  <ArrowRight className="w-4 h-4" />
                </button> : <button type="submit" disabled={saveProductLoading} data-testid="submit-product-btn" className="flex items-center gap-2 px-5 py-2 text-sm font-medium bg-stone-950 hover:bg-stone-800 disabled:opacity-40 text-white rounded-2xl transition-colors">
  ...
  return <div className="max-w-[975px] mx-auto">
      ...
      <div className="relative mb-3">
        ...
        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Buscar productos..." className="w-full h-10 rounded-xl bg-stone-100 border-none pl-9 pr-4 text-sm text-stone-950 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-stone-300" data-testid="product-search" />
      </div>
      ...
      <AnimatePresence>
        ...
        opacity: 0
      }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !bulkDeleting && setShowDeleteModal(false)}>
            <motion.div initial={{
          scale: 0.95,
          opacity: 0
        }} animate={{
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {loading ? <div className="divide-y divide-stone-200">
            {[1, 2, 3, 4].map(i => <div key={i} className="p-4 flex items-center gap-4">
            ...
            <div className="md:hidden divide-y divide-stone-200">
              ...
              <div className="p-3 flex items-center gap-2 border-b border-stone-200 bg-stone-50">
                <input type="checkbox" checked={allFilteredSelected} onChange={toggleSelectAll} className="w-4 h-4 rounded accent-stone-950" />
            ...
            return <div key={product.product_id} className="p-4 space-y-3" data-testid={`product-card-${product.product_id}`}>
                    <div className="flex items-center gap-3">
                      <input type="checkbox" checked={selectedIds.has(product.product_id)} onChange={() => toggleSelect(product.product_id)} className="w-4 h-4 rounded accent-stone-950 shrink-0" />
                      <div className="w-14 h-14 rounded-2xl bg-stone-100 overflow-hidden shrink-0">
                        {product.images?.[0] && <img src={product.images[0]} alt={product.name || 'Producto'} className="w-full h-full object-cover" loading="lazy" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="font-semibold text-stone-950">{asNumber(product.price).toLocaleString('es-ES', {
                        style: 'currency',
                        currency: 'EUR'
                      })}</span>
            ...
            <table className="w-full hidden md:table" data-testid="products-table">
              ...
              return <tr key={product.product_id} className="hover:bg-stone-50">
                    <td className="px-3 py-4 w-10">
                      <input type="checkbox" checked={selectedIds.has(product.product_id)} onChange={() => toggleSelect(product.product_id)} className="w-4 h-4 rounded accent-stone-950" />
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <p className="font-medium text-stone-950">{asNumber(product.price).toLocaleString('es-ES', {
                      style: 'currency',
                      currency: 'EUR'
                    })}</p>
                    </td>
                    <td className="px-6 py-4">
                      <StockEditor productId={product.product_id} currentStock={stock} isLowStock={isLowStock} isOutOfStock={isOutOfStock} onUpdate={refetchProducts} t={t} />
                    </td>
                    <td className="px-6 py-4">
                      <button onClick={() => setVariantManagerProduct(product)} className={`flex items-center gap-2 px-3 py-1.5 rounded-2xl text-sm transition-colors ${variantCount > 0 ? 'bg-stone-50 text-stone-700 border border-stone-200 hover:bg-stone-100' : 'bg-stone-50 text-stone-600 border border-stone-200 hover:bg-stone-100'}`} data-testid={`manage-variants-${product.product_id}`}>
                        <Layers className="w-4 h-4" />
                        {variantCount > 0 ? <span>{t('producerProducts.table.varPacks', {
                        vars: variantCount,
                        packs: packCount
                      })}</span> : <span>{t('producerProducts.table.addVariants')}</span>}
                      </button>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button type="button" onClick={() => window.open(`/producer/products/${product.product_id}/countries`, '_self')} title={t('producer.manageCountries')} className="flex items-center px-3 py-1.5 text-sm font-medium bg-stone-950 hover:bg-stone-800 disabled:opacity-40 text-white rounded-2xl transition-colors">
                          <Globe className="w-4 h-4" />
                        </button>
                        <button type="button" onClick={() => window.open(`/products/${product.product_id}`, '_blank')} className="flex items-center px-3 py-1.5 text-sm font-medium bg-stone-950 hover:bg-stone-800 disabled:opacity-40 text-white rounded-2xl transition-colors">
      ...
      {variantManagerProduct && <VariantPackManager product={variantManagerProduct} onClose={() => setVariantManagerProduct(null)} onUpdate={refetchProducts} />}
    </div>;
}