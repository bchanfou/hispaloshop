...
const formatPrice = v => new Intl.NumberFormat('es-ES', {
  style: 'currency',
  currency: 'EUR',
...
export default function ProducerInsights() {
  ...
  const load = d => {
    setLoading(true);
    apiClient.get(`/discovery/producer-insights?days=${d}`).then(data => setData(data)).catch(() => setData(null)).finally(() => setLoading(false));
  };
  useEffect(() => {
    load(days);
  }, [days]);
  ...
  return <div className="min-h-screen bg-stone-50 pb-20">
        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">{i18n.t('producer_insights.descubrimientoYConversionDeTuCatalo', 'Descubrimiento y conversión de tu catálogo')}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => load(days)} disabled={loading} className="flex h-9 w-9 items-center justify-center rounded-2xl border border-stone-200 bg-white text-stone-600 hover:text-stone-950 disabled:opacity-40">
      ...
      <div className="mx-auto max-w-5xl space-y-8 px-6 py-6">
        ...
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-4">
          ...
          <KpiCard icon={BarChart3} label={i18n.t('producer_insights.conversionDesdeContenido', 'Conversión desde contenido')} value={overview.content_conversion_rate != null ? `${overview.content_conversion_rate}%` : '—'} sub="Clics / vistas" />
        </div>
                ...
                {loading ? <tr>
                    ...
                    <td colSpan={6} className="px-4 py-8 text-center text-sm text-stone-400">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <img src={p.image || '/placeholder-product.png'} alt={p.name} className="h-9 w-9 shrink-0 rounded-2xl object-cover" />
        ...
        {recipes.length > 0 && <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {recipes.map(r => <button key={r.id} onClick={() => navigate(`/recipes/${r.id}`)} className="flex items-center gap-3 rounded-2xl border border-stone-200 bg-white p-4 text-left shadow-sm hover:shadow-md transition-shadow">
                  {r.image ? <img src={r.image} alt={r.name} className="h-12 w-12 shrink-0 rounded-2xl object-cover" /> : <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-stone-100">
                  ...
                  <div className="min-w-0">
                    ...
                    {r.saves_count ? <p className="mt-0.5 text-xs text-stone-500">{r.saves_count} guardados</p> : null}
                  </div>
                </button>)}
            </div>
          </div>}
        ...
        {/* Empty recipes state */}
        {!loading && recipes.length === 0 && <div className="rounded-2xl border border-dashed border-stone-200 bg-white p-8 text-center">
            <ChefHat className="mx-auto mb-3 h-8 w-8 text-stone-300" />
            <p className="text-sm font-medium text-stone-950">{i18n.t('producer_insights.todaviaNoHayRecetasConTusProductos', 'Todavía no hay recetas con tus productos')}</p>
