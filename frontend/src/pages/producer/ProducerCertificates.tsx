  ...
  steps
    ...
    {steps.map((step, idx) => <React.Fragment key={step.id}>
        <div className="flex flex-col items-center">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${idx < currentStep ? 'bg-stone-950 border-stone-950 text-white' : idx === currentStep ? 'border-stone-950 text-stone-950 bg-white' : 'border-stone-200 text-stone-400 bg-white'}`}>
            {idx < currentStep ? <Check className="w-5 h-5" /> : <span className="font-semibold">{idx + 1}</span>}
          </div>
          <span className={`text-xs mt-2 font-medium ${idx <= currentStep ? 'text-stone-950' : 'text-stone-400'}`}>
            {step.label}
          </span>
        </div>
        {idx < steps.length - 1 && <div className={`flex-1 h-0.5 mx-2 ${idx < currentStep ? 'bg-stone-950' : 'bg-stone-200'}`} />}
      </React.Fragment>)}
  </div>;
export default function ProducerCertificates() {
  ...
  const WIZARD_STEPS = [{
    id: 'type',
    label: t('certificates.stepType', 'Tipo')
  }, {
    id: 'document',
    label: t('certificates.stepDocument', 'Documento')
  }, {
    id: 'details',
    label: t('certificates.stepDetails', 'Detalles')
  }, {
    id: 'review',
    label: t('certificates.stepReview', 'Revisar')
  }];
  const fetchData = useCallback(async () => {
    try {
      const [certsData, productsData] = await Promise.all([apiClient.get('/producer/certificates'), apiClient.get('/producer/products')]);
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0:
        // Type
        return formData.product_id && formData.certificate_type && (formData.certificate_type !== 'other' || formData.custom_type);
      case 1:
        // Document
        return true;
  ...
  // Wizard Form
  if (showCreateForm) {
    const productsWithoutCert = products.filter(p => !certificates.some(c => c.product_id === p.product_id));
    return <div>
        <button onClick={resetForm} className="flex items-center gap-2 text-stone-600 hover:text-stone-950 mb-6" data-testid="back-button">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 max-w-3xl">
          ...
          {productsWithoutCert.length === 0 ? <div className="text-center py-8">
              ...
              {currentStep === 0 && <div className="space-y-6">
                  <div>
                    ...
                    <select value={formData.product_id} onChange={e => setFormData({
                ...formData,
                product_id: e.target.value
              })} className="w-full px-4 py-3 rounded-2xl border border-stone-200 focus:outline-none focus:border-stone-950" data-testid="product-select">
                      <option value="">{t('certificates.selectProductPlaceholder', 'Seleccionar producto')}</option>
                      {productsWithoutCert.map(p => <option key={p.product_id} value={p.product_id}>{p.name}</option>)}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {CERTIFICATE_TYPES.map(type => <button key={type.id} type="button" onClick={() => setFormData({
                  ...formData,
                  ...
                  {formData.certificate_type === 'other' && <div>
                      <label className="block text-sm font-medium text-stone-950 mb-2">
                        {t('certificates.customType', 'Especificar tipo')} *
                      </label>
                      <input value={formData.custom_type} onChange={e => setFormData({
                ...formData,
                custom_type: e.target.value
              })} placeholder={t('certificates.customTypePlaceholder', 'Ej: Certificado de Denominación de Origen')} className="w-full h-12 px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                    </div>}
                </div>}
              ...
              {currentStep === 1 && <div className="space-y-6">
                  <div>
                    ...
                    <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${formData.document_url ? 'border-stone-200 bg-stone-100' : 'border-stone-200 hover:border-stone-950 hover:bg-stone-50'}`}>
                      {formData.document_url ? <div className="flex flex-col items-center">
                    ...
                    document_name: ''
                          ...
                          <p className="font-medium text-stone-950 mb-1">
                            {uploading ? t('common.uploading', 'Subiendo...') : t('certificates.dragOrClick', 'Arrastra o haz clic para subir')}
                          </p>
                          <p className="text-sm text-stone-500">{t('producer_certificates.pdfJpgPngMax5mb', 'PDF, JPG, PNG (máx. 5MB)')}</p>
                          <input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={handleFileUpload} className="hidden" disabled={uploading} />
                        </label>}
                    </div>
                  </div>
              ...
              {currentStep === 2 && <div className="space-y-5">
                  <div>
                    ...
                    <input value={formData.data.certifying_body} onChange={e => setFormData({
                ...formData,
                data: {
                  ...formData.data,
                  certifying_body: e.target.value
                }
              })} placeholder={t('certificates.certifyingBodyPlaceholder', 'Ej: CAAE, Ecocert, BCS...')} className="w-full h-12 px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                  </div>
                  ...
                  <div>
                    ...
                    <input type="date" value={formData.data.expiry_date} onChange={e => setFormData({
                ...formData,
                data: {
                  ...formData.data,
                  ...
                  <div>
                    ...
                    <input value={formData.data.claims} onChange={e => setFormData({
                ...formData,
                data: {
                  ...formData.data,
                  claims: e.target.value
                }
              })} placeholder={t('producer_certificates.100OrganicoSinPesticidasCosechaM', '100% Orgánico, Sin Pesticidas, Cosecha Manual...')} className="w-full h-12 px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 placeholder:text-stone-400 focus:outline-none focus:border-stone-950" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea value={formData.data.ingredient_origins} onChange={e => setFormData({
                ...formData,
                data: {
                  ...formData.data,
                  ingredient_origins: e.target.value
                }
              })} className="w-full px-4 py-3 rounded-2xl border border-stone-200 min-h-[100px] focus:outline-none focus:border-stone-950" placeholder={t('producer_certificates.aceitunasEspanaJaen10salPortug', 'Aceitunas: España (Jaén)&#10;Sal: Portugal&#10;...')} />
                  </div>
                </div>}
                  ...
                  <div className="bg-stone-50 rounded-2xl p-6">
                    ...
                    <dl className="space-y-3">
                      ...
                      <div className="flex justify-between py-2 border-b border-stone-200">
                        ...
                        <dd className="font-medium text-stone-950">
                          {formData.certificate_type === 'other' ? formData.custom_type : CERTIFICATE_TYPES.find(ct => ct.id === formData.certificate_type)?.label}
                        </dd>
                      </div>
                      {formData.document_url && <div className="flex justify-between py-2 border-b border-stone-200">
                      ...
                      {formData.data.certifying_body && <div className="flex justify-between py-2 border-b border-stone-200">
              ...
              <div className="flex justify-between mt-8 pt-6 border-t border-stone-200">
                <div>
                  {currentStep > 0 && <button type="button" onClick={() => setCurrentStep(currentStep - 1)} className="flex items-center px-4 py-2 text-sm font-medium border border-stone-200 rounded-full hover:bg-stone-50 transition-colors">
                ...
                <div className="flex gap-3">
                  {currentStep === 3 && <button type="button" onClick={() => handleSubmit(true)} className="flex items-center px-4 py-2 text-sm font-medium border border-stone-200 rounded-full hover:bg-stone-50 transition-colors">
                      <Save className="w-4 h-4 mr-2" />
                      {t('certificates.saveDraft', 'Guardar borrador')}
                    </button>}
                  
                  {currentStep < 3 ? <button type="button" onClick={() => setCurrentStep(currentStep + 1)} disabled={!canProceed()} className="flex items-center px-4 py-2 text-sm font-medium bg-stone-950 hover:bg-stone-800 disabled:opacity-40 text-white rounded-full transition-colors">
                      {t('common.next', 'Siguiente')}
                      <ArrowRight className="w-4 h-4 ml-2" />
                    </button> : <button type="button" onClick={() => handleSubmit(false)} className="flex items-center px-4 py-2 text-sm font-medium bg-stone-950 hover:bg-stone-800 text-white rounded-full transition-colors">
  ...
  return <div className="max-w-[975px] mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={() => setShowCreateForm(true)} className="flex items-center px-4 py-2 text-sm font-medium bg-stone-950 hover:bg-stone-800 text-white rounded-full transition-colors" data-testid="create-certificate">
      ...
      {!loading && certificates.length > 0 && <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        {loading ? <div className="p-8 text-center text-stone-500">
            {t('common.loading', 'Cargando...')}
          </div> : certificates.length === 0 ? <div className="p-8 text-center">
            ...
            <table className="w-full" data-testid="certificates-table">
              ...
              return <tr key={cert.certificate_id} className="hover:bg-stone-50">
                      ...
                      <td className="px-4 md:px-6 py-4">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isExpiringSoon && <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-stone-100 text-stone-700">
                              Caduca pronto
                            </span>}
                        </div>
                        {status === 'rejected' && cert.rejection_reason && <p className="text-xs text-stone-500 mt-1">
                            {cert.rejection_reason}
                          </p>}
                        {showRenew && <button type="button" onClick={() => {
                    ...
                    setShowCreateForm(true);
                      ...
                      <td className="px-4 md:px-6 py-4 hidden sm:table-cell">
                        <div className="flex items-center gap-2">
                          ...
                          {cert.approved && <a href={`/certificate/${cert.product_id}`} target="_blank" rel="noopener noreferrer" className="text-stone-400 hover:text-stone-700 transition-colors">
                              <ExternalLink className="h-3.5 w-3.5" />
                            </a>}
                        </div>
                        {cert.last_scanned_at && <p className="text-[10px] text-stone-400 mt-0.5">{new Date(cert.last_scanned_at).toLocaleDateString('es-ES', {
                      day: 'numeric',
                      month: 'short'
                    })}</p>}
                      </td>
