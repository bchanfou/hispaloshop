...
import { ArrowLeft, Check, Upload, Loader2, AlertCircle, Clock, Building2, Camera, Award, Plus, X, ChevronRight, ShieldCheck } from 'lucide-react';
...
function StepIndicator({
  ...
  return <div className="flex items-center justify-between p-5 mb-5 bg-stone-950 rounded-2xl">
      {steps.map((s, i) => {
      const done = s.done;
...
function StatusCard({
  ...
  const Icon = s.icon;
  return <div className={`p-4 flex items-start gap-3 rounded-2xl border ${s.bg} ${s.border}`}>
...
function UploadArea({
  ...
  return <div className={`flex flex-col items-center justify-center gap-2 p-8 cursor-pointer transition-colors border-2 border-dashed border-stone-200 rounded-2xl bg-stone-50 ${uploading ? 'opacity-50 pointer-events-none' : 'hover:border-stone-400'}`} onClick={() => ref.current?.click()}>
      <input ref={ref} type="file" accept={accept} className="hidden" onChange={handleChange} />
      {uploading ? <Loader2 className="w-8 h-8 animate-spin text-stone-500" /> : <Upload className="w-8 h-8 text-stone-500" />}
...
export default function ProducerVerificationPage() {
  ...
  return <div className="bg-white min-h-dvh">
      {/* TopBar */}
      ...
      <div className="max-w-[600px] mx-auto px-4 pb-8">
        ...
        <section className="mb-5">
          ...
          {cifDoc.status === 'verified' && <StatusCard status="verified">
          ...
          {cifDoc.status === 'rejected' && <div>
              <StatusCard status="rejected">
                <p className="font-semibold">Documento rechazado</p>
                <p className="mt-1">{cifDoc.rejection_reason}</p>
              </StatusCard>
              <button className="w-full mt-3 py-2.5 text-sm font-semibold transition-colors bg-stone-950 text-white rounded-full hover:bg-stone-800" onClick={() => {
            setVs(prev => ({
              ...prev,
              documents: {
                ...prev.documents,
        ...
        <section className="mb-5">
          ...
          {facilityDoc.status === 'rejected' && <div>
              <StatusCard status="rejected">
        ...
        <section className="mb-5">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {CERT_TYPES.map(ct => <button key={ct.id} className={`px-3 py-1.5 text-xs font-medium transition-colors rounded-full border ${selectedCertType === ct.id ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-white text-stone-950 hover:border-stone-400'}`} onClick={() => setSelectedCertType(ct.id)}>
                {ct.emoji} {ct.label}
              </button>)}
          </div>

          {/* Upload area for new cert */}
          <UploadArea accept=".pdf,.jpg,.jpeg,.png" maxSize="5MB" hint={`Certificado de ${CERT_TYPES.find(c => c.id === selectedCertType)?.label} · PDF, JPG, PNG · Máx 5MB`} onFile={handleCertUpload} uploading={certUploading} />

          {/* Uploaded certificates list */}
          {certs.length > 0 && <div className="mt-4 space-y-2">
              {certs.map((c, i) => {
            const typeInfo = CERT_TYPES.find(ct => ct.id === c.type) || {
              emoji: '📋',
              label: c.type
            };
            ...
            return <div key={c.cert_id || i} className="flex items-center gap-3 p-3 bg-white rounded-2xl border border-stone-200">
                    ...
                    <div className="flex items-center gap-2">
                      {c.status === 'verified' && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-800">
                          Verificado
                        </span>}
                      {c.status === 'rejected' && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-200 text-stone-700">
                          Rechazado
                        </span>}
                      {c.status === 'manual_review' && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-600">
                          En revisión
                        </span>}
                      {isExpired && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-200 text-stone-700">
                          Caducado
                        </span>}
                      {hasWarning && daysLeft !== null && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-600">
        ...
        <AnimatePresence mode="wait">
          {vs?.is_verified && <motion.div initial={{
          opacity: 0,
          y: 12
        }} animate={{
          opacity: 1,
          y: 0
              ...
              <button className="w-full py-3 text-sm font-bold transition-colors bg-stone-950 text-white rounded-full hover:bg-stone-800" onClick={async () => {
            await refreshUser();
          ...
          {vs?.admin_review_required && !vs?.is_verified && <motion.div initial={{
          opacity: 0,
          y: 12
        }} animate={{
          opacity: 1,
          ...
          {!vs?.is_verified && !vs?.admin_review_required && (cifDone || facilityDone || certsDone) && <motion.div initial={{
          opacity: 0,
          y: 12
        }} animate={{
          opacity: 1,
          y: 0
              ...
              <div className="space-y-2">
                ...
                {!facilityDone && <div className="flex items-center gap-2 text-sm text-stone-600">
                    <AlertCircle className="w-4 h-4 shrink-0" /> Falta: Foto de instalación
                  </div>}
                {!certsDone && <div className="flex items-center gap-2 text-sm text-stone-600">
