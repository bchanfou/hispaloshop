...
export default function ProducerConnectPage() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      toast.error(error.message || i18n.t('producer_connect.noSePudoIniciarElOnboarding', 'No se pudo iniciar el onboarding'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return <div className="max-w-[600px] mx-auto p-4 md:p-6">
      <h1 className="text-2xl font-bold text-stone-950 mb-2">Stripe Connect</h1>
      <p className="text-sm text-stone-500 mb-6">{i18n.t('producer_connect.configuraTuCuentaParaRecibirTransfe', 'Configura tu cuenta para recibir transferencias automaticas.')}</p>
      ...
      <div className="mb-8">
        <div className="flex items-center">
          ...
          return <React.Fragment key={step.key}>
              ...
              flex: isLast ? '0 0 auto' : 0
            }}>
                  <div className={`w-9 h-9 flex items-center justify-center rounded-full transition-colors ${isCompleted || isActive ? 'bg-stone-950' : 'bg-transparent border-2 border-stone-200'}`}>
                    {isCompleted ? <Check size={16} className="text-white" strokeWidth={2.5} /> : <span className={`text-[13px] font-bold ${isActive ? 'text-white' : 'text-stone-400'}`}>
                        {i + 1}
                      </span>}
                  </div>
                  <span className={`mt-2 text-center whitespace-nowrap text-[11px] font-semibold ${isCompleted || isActive ? 'text-stone-950' : 'text-stone-400'}`}>
                    {step.label}
                  </span>
                </div>
                {!isLast && <div className={`flex-1 h-0.5 -mt-[18px] transition-colors ${i < currentStep ? 'bg-stone-950' : 'bg-stone-300'}`} />}
              </React.Fragment>;
        })}
        </div>
      </div>
      ...
      <div className="rounded-2xl border p-5 bg-stone-50 border-stone-200">
        <div className="flex items-start gap-3">
          {isReady ? <CheckCircle2 className="w-5 h-5 text-stone-700 mt-0.5" /> : <AlertCircle className="w-5 h-5 text-stone-500 mt-0.5" />}
          <div className="flex-1">
            <p className="font-medium text-stone-950">
              {isReady ? 'Cuenta activa' : 'Onboarding pendiente'}
            </p>
            <p className="text-sm text-stone-500 mt-1">
              {isReady ? i18n.t('producer_connect.tuCuentaEstaListaParaRecibirPagos', 'Tu cuenta esta lista para recibir pagos.') : i18n.t('producer_connect.completaLosDatosEnStripeParaActiva', 'Completa los datos en Stripe para activar cobros y payouts.')}
            </p>
            {!isReady && status?.requirements_due?.length > 0 && <p className="text-xs text-stone-600 mt-2">
      ...
      {!isReady && <div className="mt-4">
          <button type="button" onClick={handleStart} disabled={submitting} className="px-4 py-2 bg-stone-950 hover:bg-stone-800 disabled:opacity-50 text-white rounded-full transition-colors">
