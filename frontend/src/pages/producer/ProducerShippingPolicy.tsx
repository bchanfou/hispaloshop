...
export default function ProducerShippingPolicy() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        toast.error(error.message || i18n.t('producer_shipping_policy.noSePudoCargarLaPoliticaDeEnvio', 'No se pudo cargar la política de envío'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const savePolicy = async () => {
    ...
    try {
      ...
      toast.success(i18n.t('producer_shipping_policy.politicaDeEnvioGuardada', 'Política de envío guardada'));
    } catch (error) {
  ...
  return <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-stone-950">{i18n.t('producer_shipping_policy.politicaDeEnvio', 'Política de Envío')}</h1>
        <p className="text-sm text-stone-500 mt-1">{i18n.t('producer_shipping_policy.configuraCostesDeEnvioPorPedidoPar', 'Configura costes de envío por pedido para tus productos.')}</p>
      </div>

      <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-5">
        <label className="flex items-center justify-between gap-4">
          <span className="text-sm font-medium text-stone-950">{i18n.t('producer_shipping_policy.habilitarGastosDeEnvio', 'Habilitar gastos de envío')}</span>
          <input type="checkbox" checked={policy.enabled} onChange={e => setPolicy(prev => ({
          ...prev,
        ...
        {policy.enabled && <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" step="0.01" value={toEuros(policy.base_cost_cents)} onChange={e => setPolicy(prev => ({
              ...prev,
              base_cost_cents: toCents(e.target.value)
              ...
              <div>
                ...
                <input type="number" step="0.01" value={toEuros(policy.per_item_cents)} onChange={e => setPolicy(prev => ({
              ...prev,
              per_item_cents: toCents(e.target.value)
            }))} className="w-full px-3 py-2 border border-stone-200 rounded-2xl text-stone-950 focus:outline-none focus:border-stone-950" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" placeholder={i18n.t('producer_shipping_policy.0SinEnvioGratis', '0 = sin envío gratis')} value={policy.free_threshold_cents === null ? '' : toEuros(policy.free_threshold_cents)} onChange={e => setPolicy(prev => ({
            ...prev,
            free_threshold_cents: e.target.value ? toCents(e.target.value) : null
              ...
              <p className="text-xs text-stone-500 mt-1">{i18n.t('producer_shipping_policy.pedidosQueSuperenEsteImporteTendran', 'Pedidos que superen este importe tendrán envío gratuito. Déjalo vacío o en 0 para desactivar.')}</p>
            </div>
        ...
        <label className="flex items-center justify-between gap-4 pt-2 border-t border-stone-200">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">{i18n.t('producer_shipping_policy.losClientesPuedenRecogerEnTuObrado', 'Los clientes pueden recoger en tu obrador (sin coste de envío)')}</p>
          </div>
          <input type="checkbox" checked={policy.local_pickup_enabled} onChange={e => setPolicy(prev => ({
          ...prev,
          local_pickup_enabled: e.target.checked
        }))} className="h-4 w-4 accent-stone-950" />
        </label>
