...
const TYPE_BADGES = {
  ...
  collab: {
    label: "Colaboración",
    classes: 'bg-stone-100 text-stone-950'
  },
  c2c: null
};
...
function ConversationItem({
  ...
  return <div className="relative overflow-hidden">
      ...
      <motion.button drag="x" dragConstraints={{
      left: -100,
      right: 0
    }} dragElastic={{
      left: 0.3,
    ...
    }} onClick={onClick} className={`flex h-[72px] w-full items-center gap-3 border-none px-4 text-left font-apple transition-colors duration-150 active:bg-stone-50 md:hover:bg-stone-50 md:focus-visible:outline md:focus-visible:outline-2 md:focus-visible:outline-offset-[-2px] md:focus-visible:outline-stone-300 ${isActive ? 'bg-stone-50' : 'bg-white'}`}>
        {/* Avatar */}
        <div className="relative shrink-0">
          {avatar_url ? <img src={avatar_url} alt={name} className={`h-12 w-12 object-cover ${isStore ? 'rounded-2xl' : 'rounded-full'}`} /> : <div className={`flex h-12 w-12 items-center justify-center bg-stone-950 text-base font-semibold text-white ${isStore ? 'rounded-2xl' : 'rounded-full'}`}>
              {getInitial(name)}
            </div>}
          {online && <motion.span animate={{
          scale: [1, 1.2, 1],
          opacity: [1, 0.7, 1]
        }} transition={{
          duration: 2,
      ...
      <div className="min-w-0 flex-1">
        ...
        {badge && <span className={`mt-0.5 inline-block rounded-full px-1.5 py-px text-[10px] font-medium ${badge.classes}`}>
            {badge.label}
          </span>}

        <div className="mt-0.5 flex items-center justify-between gap-2">
          {isTyping ? <span className="text-stone-950 italic text-xs font-medium flex items-center gap-1">
              <span className="inline-flex gap-0.5">
                {[0, 1, 2].map(i => <span key={i} className="w-1 h-1 rounded-full bg-stone-950 animate-bounce" style={{
...
export default function ChatsPage() {
  ...
  const onlineConversations = useMemo(() => (conversations || []).filter(c => c.online), [conversations]);
  const isEmpty = filteredConversations.length === 0;
  return <div className="mx-auto min-h-screen max-w-[1240px] bg-white font-apple md:px-4 md:py-4">
      <div className="flex min-h-screen flex-col bg-white md:min-h-0 md:h-[calc(100vh-88px)] md:grid md:grid-cols-[410px_1fr] md:overflow-hidden md:rounded-xl md:border md:border-stone-200/90">
        <section className="flex min-h-0 flex-col bg-white md:border-r md:border-stone-200">
          ...
          <div className="sticky top-[52px] z-30 flex items-center gap-3 border-b border-stone-100 bg-white/90 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] backdrop-blur-xl md:static md:pt-4">
            <button onClick={() => navigate(-1)} className="flex min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-full text-stone-950 active:bg-stone-100 md:hidden" aria-label="Volver">
              <ArrowLeft size={22} />
            </button>
            <h1 className="flex-1 text-[22px] font-bold text-stone-950 md:text-[20px] md:font-semibold">Mensajes</h1>

            <button className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-stone-950 active:bg-stone-100 md:min-h-[40px] md:min-w-[40px] md:rounded-lg md:transition-colors md:duration-150 md:hover:bg-stone-100 md:focus-visible:outline md:focus-visible:outline-2 md:focus-visible:outline-offset-[-2px] md:focus-visible:outline-stone-300" aria-label="Nuevo mensaje" onClick={() => navigate('/messages/new')}>
          ...
          {onlineConversations.length > 0 && <div className="border-b border-stone-100 pb-3 pt-1 md:hidden">
              ...
              return <button key={convId || `online-${idx}`} onClick={() => {
                if (convId) navigate(`/messages/${convId}`);
              }} className="flex w-[58px] shrink-0 flex-col items-center gap-1">
                      <div className="relative">
                        {conv.avatar_url ? <img src={conv.avatar_url} alt={conv.name} className={`h-14 w-14 object-cover ${isStore ? 'rounded-2xl' : 'rounded-full'}`} /> : <div className={`flex h-14 w-14 items-center justify-center bg-stone-950 text-lg font-semibold text-white ${isStore ? 'rounded-2xl' : 'rounded-full'}`}>
          ...
          <AnimatePresence>
            {showSwipeHint && <motion.p initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} exit={{
          ...
          <div className="scrollbar-hide flex gap-2 overflow-x-auto px-4 pb-3 pt-1 md:hidden">
            ...
            return <button key={f.key} onClick={() => setActiveFilter(f.key)} className={`shrink-0 rounded-full px-3.5 py-2 text-[13px] font-medium transition-colors ${isActive ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-500 active:bg-stone-200'}`}>
                  {f.label}
                </button>;
          })}
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <AnimatePresence mode="wait">
              {isEmpty ? <motion.div key="empty" initial={{
              opacity: 0,
              y: 16
            }} animate={{
              opacity: 1,
              ...
              duration: 0.15
                ...
                const convId = getConversationId(conv);
                return <React.Fragment key={convId || `${conv.name}-${i}`}>
                      <ConversationItem conversation={conv} index={i} onClick={() => {
                    if (convId) navigate(`/messages/${convId}`);
                  }} onDelete={() => {
                    if (convId) deleteConversation(convId);
                  }} isTyping={convId ? !!typingUsers[convId] : false} isActive={false} />
                      {i < filteredConversations.length - 1 && <div className="ml-[76px] mr-4 h-px bg-stone-100/80" />}
                    </React.Fragment>;
              })}
                </motion.div>}
            </AnimatePresence>
        ...
        <aside className="hidden flex-col items-center justify-center bg-stone-50 px-8 md:flex">
          ...
          <button onClick={() => navigate('/messages/new')} className="mt-5 rounded-full bg-stone-950 px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90 md:rounded-lg">
            Enviar mensaje
          </button>
        </aside>
      </div>
