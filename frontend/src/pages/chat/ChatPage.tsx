...
function formatOnlineStatus(lastSeen) {
  ...
  if (diffMin < 2) return { text: t('chat.online', 'En línea'), online: true };
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          ...
          <div className="min-w-0" onClick={() => { const uid = conversation?.other_user_id || conversation?.user2_id; if (uid) navigate(`/${uid}`); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); const uid = conversation?.other_user_id || conversation?.user2_id; if (uid) navigate(`/${uid}`); } }} role="button" tabIndex={0}>
            ...
            <div className="mt-0.5 flex items-center gap-1">
              {isOnline ? (
                <span className={`font-medium text-stone-950 ${showBackButton ? 'text-xs' : 'text-[11px]'}`}>{t('chat.online', 'En línea')}</span>
              ) : (
          ...
          >
            <div className="mt-2 flex items-center gap-2 rounded-[10px] bg-stone-100 px-3 py-2">
              ...
              <input
                ...
                placeholder={t('chat.buscarEnLaConversacion', 'Buscar en la conversación...')}
                autoFocus
  ...
  if (message.message_type === 'system' || message.sender_id === 'system') {
    return (
      <div className="flex justify-center px-4" style={{ marginTop: gap }}>
        <div className="max-w-[85%] rounded-full bg-stone-100 px-3 py-1 text-center text-[11px] text-stone-500" style={{ overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
          {message.content}
        </div>
      </div>
    );
...
const SUGGESTION_PILLS = {
  ...
  c2c: [t('chat.hola', '¡Hola!'), '¿Dónde compraste...?', 'Te vi en el feed y...'],
  collab: [t('chat.holaMeGustariaColaborar', 'Hola, me gustaría colaborar...'), 'Vi tu tienda y...', '¿Enviáis muestras?'],
};
          ...
          <AnimatePresence mode="wait">
            {hasText ? (
              ...
              <motion.button
                ...
                aria-label={t('chat.enviarCorazon', 'Enviar corazón')}
              >
...
export default function ChatPage() {
  ...
  const handleAttachDocument = useCallback(async (file) => {
    if (!conversationId || !file) return;
    if (file.size > 10 * 1024 * 1024) { toast.error(t('chat.maximo10Mb', 'Máximo 10 MB')); return; }
    try {
      const formData = new FormData();
      formData.append('file', file);
