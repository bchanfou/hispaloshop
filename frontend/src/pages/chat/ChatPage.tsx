...
import { ArrowLeft, MessageCircle, PenSquare, Package, ChevronRight, ArrowUp, Check, CheckCheck, Image, X, Copy, Reply, Trash2, Search, Mic, Clock, Pause, Play, Download, MoreVertical, FileText } from 'lucide-react';
...
function formatOnlineStatus(lastSeen) {
  ...
  if (seen.toDateString() === now.toDateString()) return {
    text: `Hoy a las ${seen.toLocaleTimeString('es-ES', {
      hour: '2-digit',
      minute: '2-digit'
    })}`,
    online: false
  ...
  const yesterday = new Date(now);
  yesterday.setDate(now.getDate() - 1);
  if (seen.toDateString() === yesterday.toDateString()) return {
    text: 'Ayer',
    online: false
  };
  return {
    text: seen.toLocaleDateString('es-ES', {
      weekday: 'short'
    }),
    online: false
  };
...
function ChatHeader({
  ...
  return <header className="sticky top-0 z-30 border-b border-stone-100 bg-white/95 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] font-apple backdrop-blur-md">
      <div className="flex items-center gap-2">
        {/* Left: back button */}
        <button onClick={() => navigate(-1)} className={`min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-full bg-transparent text-stone-950 active:bg-stone-100 md:min-h-[40px] md:min-w-[40px] md:rounded-lg md:transition-colors md:duration-150 md:hover:bg-stone-100 md:focus-visible:outline md:focus-visible:outline-2 md:focus-visible:outline-offset-[-2px] md:focus-visible:outline-stone-300 ${showBackButton ? 'flex' : 'hidden'}`} aria-label="Volver">
          <ArrowLeft size={22} />
        </button>
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-full bg-stone-200">
            {conversation?.avatar_url ? <img src={conversation.avatar_url} alt="" className="h-full w-full object-cover" loading="lazy" onError={e => {
            e.target.style.display = 'none';
          }} /> : <div className="flex h-full w-full items-center justify-center text-[13px] font-semibold text-stone-500">
                {(conversation?.name || '?')[0]?.toUpperCase()}
              </div>}
            {isOnline && <span className="absolute bottom-0 right-0 h-[9px] w-[9px] rounded-full border-2 border-white bg-stone-950" />}
          </div>

          <div className="min-w-0" onClick={() => {
          const uid = conversation?.other_user_id || conversation?.user2_id;
          if (uid) navigate(`/${uid}`);
        }} onKeyDown={e => {
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <button onClick={onToggleSearch} className={`flex min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-full active:bg-stone-100 md:min-h-[40px] md:min-w-[40px] md:rounded-lg md:transition-colors md:duration-150 md:hover:bg-stone-100 md:focus-visible:outline md:focus-visible:outline-2 md:focus-visible:outline-offset-[-2px] md:focus-visible:outline-stone-300 ${showSearch ? 'text-stone-950' : 'text-stone-500'}`} aria-label="Buscar">
            <Search size={20} />
          </button>
        ...
        <AnimatePresence>
          {showMenu && <>
            ...
            opacity: 0
          }} onClick={() => setShowMenu(false)} className="fixed inset-0 z-40" />
              <motion.div initial={{
            opacity: 0,
            scale: 0.95,
            y: -4
...
function ContextBanner({
  ...
  return <button onClick={() => navigate(`/orders/${orderId}`)} className="flex h-11 w-full items-center gap-3 border-b border-stone-200 bg-stone-100 px-4 font-apple">
...
function DateSeparator({
  ...
  return <div className={`flex justify-center ${compact ? 'py-2' : 'py-3'}`}>
...
function ReactionBar({
  ...
  return <div className="mt-0.5 flex gap-1">
      {Object.entries(grouped).map(([emoji, count]) => <span key={emoji} className="flex items-center gap-0.5 rounded-full border border-stone-200 bg-white px-1.5 py-0.5 text-[11px] shadow-sm">
  ...
  return <div className="flex items-center gap-2">
      ...
      <div className="flex-1">
        <div className={`h-1 rounded-full ${isOwn ? 'bg-white/30' : 'bg-stone-200'}`}>
          <div className={`h-1 rounded-full transition-[width] ${isOwn ? 'bg-white' : 'bg-stone-950'}`} style={{
          width: `${progress * 100}%`
        }} />
        </div>
      </div>
      <span className={`text-[11px] ${isOwn ? 'text-white/70' : 'text-stone-500'}`}>{fmt(duration || 0)}</span>
      {playing && <button onClick={cycleSpeed} className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${isOwn ? 'bg-white/20 text-white' : 'bg-stone-200 text-stone-700'}`}>
          {speed}x
        </button>}
    </div>;
});
...
const MessageBubble = React.memo(function MessageBubble({
  ...
  const gap = compact ? isConsecutive && !isFirstInGroup ? 1 : isConsecutive ? 3 : 9 : isConsecutive && !isFirstInGroup ? 2 : isConsecutive ? 4 : 12;
  ...
  if (message.message_type === 'system' || message.sender_id === 'system') {
    return <div className="flex justify-center px-4" style={{
      marginTop: gap
    }}>
        <div className="max-w-[85%] rounded-full bg-stone-100 px-3 py-1 text-center text-[11px] text-stone-500" style={{
  ...
  if (message.message_type === 'product_card') {
    return <div className={`flex px-4 ${isOwn ? 'justify-end' : 'justify-start'}`} style={{
      marginTop: gap
    }} {...touchProps}>
        <div className="min-w-0 max-w-[75%]">
          <ProductCardMessage product={message.metadata?.product || message.product || {
          name: message.content || 'Producto compartido'
        }} />
          <ReactionBar reactions={message.reactions} />
          {showTimestamp && <div className={`mt-1 flex items-center gap-1 ${isOwn ? 'justify-end' : 'justify-start'}`}>
    ...
    return <div className={`flex px-4 ${isOwn ? 'justify-end' : 'justify-start'}`} style={{
      marginTop: gap
        ...
        <div className="max-w-[240px]">
          <div className="overflow-hidden rounded-2xl bg-stone-100" style={{
          borderRadius: bubbleRadius,
          cursor: message.image_url ? 'pointer' : 'default'
        }} onClick={() => message.image_url && onImageTap?.(message.image_url)}>
            {message.image_url ? <img src={message.image_url} alt="Imagen en chat" className="block h-auto w-full" loading="lazy" onError={e => {
            e.target.style.display = 'none';
  ...
  /* Collab cards */
  if (message.message_type === 'collab_proposal' && message.metadata?.collab_id) return <CollabProposalMessage message={message} isOwn={isOwn} gap={gap} touchProps={touchProps} />;
  if (message.message_type === 'collab_affiliate' && message.metadata?.collab_id) return <CollabAffiliateMessage message={message} isOwn={isOwn} gap={gap} touchProps={touchProps} />;
  if (message.message_type === 'collab_sample' && message.metadata?.collab_id) return <CollabSampleMessage message={message} isOwn={isOwn} gap={gap} touchProps={touchProps} />;
    ...
    return <div className={`flex px-4 ${isOwn ? 'justify-end' : 'justify-start'}`} style={{
      marginTop: gap
        ...
        <div className="min-w-0 max-w-[80%]">
          <div className={`min-w-[min(200px,calc(100vw-96px))] break-words font-apple ${compact ? 'px-3 py-2' : 'px-3.5 py-2.5'} ${isOwn ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-950'}`} style={{
          borderRadius: bubbleRadius,
          overflowWrap: 'anywhere'
        }}>
  ...
  const bubbleContent = <div className="min-w-0 max-w-[75%]">
      <div className={`whitespace-pre-wrap break-words font-apple ${compact ? 'px-3 py-2 text-[14px] leading-5' : 'px-3.5 py-2.5 text-[15px] leading-[21px]'} ${isOwn ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-950'}`} style={{
      borderRadius: bubbleRadius,
      overflowWrap: 'anywhere',
      wordBreak: 'break-word'
    }}>
        {/* Reply reference */}
        {replyPreview && <div className={`mb-1.5 rounded-xl px-2.5 py-1.5 text-[12px] leading-[16px] ${isOwn ? 'bg-white/15 text-white/80' : 'bg-stone-200/60 text-stone-600'}`}>
  ...
  return <div className={`relative flex px-4 ${isOwn ? 'justify-end' : 'justify-start'}`} style={{
    marginTop: gap
  }}>
      {/* Reply hint icon appears behind during swipe */}
      <motion.div className="absolute left-2 top-1/2 -translate-y-1/2" style={{
      opacity: replyIconOpacity
    }}>
        <Reply size={18} className="text-stone-400" />
      </motion.div>
      <motion.div className="min-w-0 w-full" drag="x" dragConstraints={{
      left: 0,
      right: 0
    }} dragElastic={{
      left: 0,
      right: 0.5
    }} dragSnapToOrigin style={{
      x: dragX
    }} onDragEnd={(_, info) => {
  ...
  return prev.message === next.message && prev.isOwn === next.isOwn && prev.isConsecutive === next.isConsecutive && prev.isFirstInGroup === next.isFirstInGroup && prev.isLastInGroup === next.isLastInGroup && prev.isMiddleInGroup === next.isMiddleInGroup && prev.searchHighlight === next.searchHighlight;
});

/* ================================================================
   Collab message wrappers
   ================================================================ */
function CollabProposalMessage({
  message,
  isOwn,
  gap,
  touchProps
  ...
  useEffect(() => {
    if (!collabId) return;
    let active = true;
    apiClient.get(`/collaborations/${collabId}`).then(d => {
      if (active) setCollab(d);
    }).catch(() => {});
    return () => {
      active = false;
    };
  ...
  const handleDecline = async () => {
    setActing(true);
    try {
      await apiClient.post(`/collaborations/${collabId}/decline`, {
        reason: ''
      });
      const updated = await apiClient.get(`/collaborations/${collabId}`);
      setCollab(updated);
  ...
  return <div className={`flex px-4 ${isOwn ? 'justify-end' : 'justify-start'} ${acting ? 'pointer-events-none opacity-60' : ''}`} style={{
    marginTop: gap
  }} {...touchProps}>
      <CollabProposalCard proposal={{
      ...
      status
    }} isReceiver={isReceiver} onAccept={handleAccept} onDecline={handleDecline} />
    </div>;
}
function CollabAffiliateMessage({
  message,
  isOwn,
  gap,
  touchProps
  ...
  useEffect(() => {
    ...
    apiClient.get(`/collaborations/${collabId}/stats`).then(d => {
  ...
  return <div className={`flex px-4 ${isOwn ? 'justify-end' : 'justify-start'}`} style={{
    marginTop: gap
      ...
      url: collab.affiliate_link.url,
      code: collab.affiliate_link.code
    }} stats={stats ? {
      clicks: stats.clicks,
      sales: stats.sales
    } : null} />
...
function TypingIndicator() {
  return <div className="mt-3 flex justify-start px-4" aria-live="polite" aria-label="Escribiendo">
      <div className="flex items-center gap-1 rounded-[20px] rounded-bl-[4px] border border-stone-100 bg-stone-50 px-4 py-3">
        {[0, 1, 2].map(i => <motion.span key={i} className="block h-[7px] w-[7px] rounded-full bg-stone-400" animate={{
        y: [0, -5, 0]
      }} transition={{
        duration: 0.6,
        repeat: Infinity,
...
function NewMessagesPill({
  ...
  return <motion.button initial={{
    ...
    opacity: 0,
    y: 10
  }} onClick={onClick} className="absolute bottom-20 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1 rounded-full bg-stone-950 px-4 py-1.5 text-[13px] font-semibold text-white shadow-[0_4px_12px_rgba(12,10,9,0.25)]">
...
function MessageContextMenu({
  ...
  return <>
      ...
      opacity: 0
    }} onClick={onClose} className="fixed inset-0 z-[60] bg-black/20" />
      <motion.div initial={{
      scale: 0.9,
      opacity: 0
    }} animate={{
      ...
      stiffness: 400,
      damping: 25
    }} className="fixed z-[61] w-[200px] rounded-[20px] bg-white p-2 shadow-[0_8px_32px_rgba(0,0,0,0.15)] font-apple" style={{
      left: Math.min(x - 100, window.innerWidth - 220),
      top: Math.max(y - 200, 20)
    }}>
        {options.map(opt => {
        if (opt.isReaction) {
          return <div key="reactions" className="flex h-10 items-center justify-between px-2">
                {reactions.map(emoji => <button key={emoji} onClick={() => {
              onReact?.(message, emoji);
              onClose();
            }} className="rounded-full bg-transparent p-0.5 text-xl leading-none hover:bg-stone-100">
                    {emoji}
                  </button>)}
              </div>;
        }
        const Icon = opt.icon;
        return <button key={opt.label} onClick={opt.action} className={`flex h-10 w-full items-center gap-2.5 rounded-2xl px-2 text-[13px] font-medium transition-colors hover:bg-stone-50 ${opt.danger ? 'text-stone-500' : 'text-stone-950'}`}>
              {Icon && <Icon size={16} />}
              <span>{opt.label}</span>
            </button>;
      })}
...
function EmptyConversation({
  ...
  return <div className="flex flex-col items-center justify-center px-6 py-12 font-apple">
...
function MessageInput({
  ...
  return <div className="sticky bottom-0 shrink-0 border-t border-stone-100 bg-white font-apple" style={{
    paddingBottom: 'max(8px, env(safe-area-inset-bottom))'
      ...
      {isRecording ? <div className="flex items-center gap-3 px-3 pt-2">
          ...
          repeat: Infinity,
          repeatType: 'reverse'
        }} className="h-2.5 w-2.5 rounded-full bg-stone-950" />
        ...
        </div> : <div className="flex items-end gap-1.5 px-3 pt-2">
          ...
          <div className="flex min-h-[44px] flex-1 items-end rounded-full border border-stone-200 bg-white">
            <textarea ref={textareaRef} value={text} onChange={handleChange} onKeyDown={handleKeyDown} placeholder="Mensaje..." rows={1} className="min-h-[44px] max-h-[120px] flex-1 resize-none bg-transparent px-4 py-2.5 text-sm text-stone-950 outline-none placeholder:text-stone-400" />
            {!hasText && <button onClick={startRecording} className="mb-1 mr-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-stone-400 active:bg-stone-50" aria-label="Nota de voz">
                <Mic size={18} />
              </button>}
          </div>

          <AnimatePresence mode="wait">
            {hasText ? <motion.button key="send" initial={{
          scale: 0,
          opacity: 0
        }} animate={{
          scale: 1,
          ...
          stiffness: 400,
          damping: 20
        }} onClick={handleSend} className="mb-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-stone-950 text-white active:opacity-75" aria-label="Enviar">
                <ArrowUp size={20} />
              </motion.button> : <motion.button key="heart" initial={{
          scale: 0,
          opacity: 0
        }} animate={{
          scale: 1,
...
export default function ChatPage() {
  ...
  const conversation = useMemo(() => conversations.find(c => String(getConversationKey(c)) === String(conversationId)), [conversations, conversationId]);
  ...
  useEffect(() => {
    ...
    if (localMessages.length > prevMsgCountRef.current) {
      if (isNearBottomRef.current) bottomRef.current?.scrollIntoView({
        behavior: 'smooth'
  ...
  useEffect(() => {
    ...
    const unread = localMessages.filter(m => !isChatMessageRead(m) && String(m.sender_id || m.user_id) !== String(user.user_id || user.id)).map(m => getChatMessageId(m)).filter(Boolean);
  ...
  const handleContextMenu = useCallback((message, x, y) => setContextMenu({
    message,
    x,
    y
  }), []);
  ...
  const handleDeleteMessage = useCallback(async message => {
    ...
    try {
      ...
      // Re-add on failure
      setLocalMessages(prev => [...prev, message].sort((a, b) => new Date(a.created_at || a.timestamp) - new Date(b.created_at || b.timestamp)));
    }
  }, []);
  ...
  const renderThread = showBackButton => {
    ...
    return <>
      <ChatHeader conversation={conversation} navigate={navigate} showSearch={showSearch} onToggleSearch={() => {
        setShowSearch(s => !s);
        if (showSearch) setSearchQuery('');
      }} searchQuery={searchQuery} onSearchChange={setSearchQuery} onDeleteConversation={handleDeleteConversation} showBackButton={showBackButton} />
      <ContextBanner orderId={conversation?.order_id} navigate={navigate} />

      {polling && <div className="flex items-center justify-center gap-1.5 bg-stone-100 px-3 py-1.5 text-xs text-stone-500">
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="relative flex-1 overflow-y-auto overscroll-none bg-white scroll-smooth" role="log" aria-live="polite" aria-label="Mensajes">
        <div className={compact ? 'pb-3 pt-1.5' : 'pb-4 pt-2'}>
          ...
          {loadingMore && <div className="flex justify-center py-3">
              <div className="h-5 w-5 animate-spin rounded-full border-2 border-stone-200 border-t-stone-950" />
            </div>}

          {localMessages.length === 0 && !loadingMore && <EmptyConversation conversation={conversation} onSendSuggestion={handleSend} />}
          ...
          {groupedMessages.map(item => {
            ...
            const bubble = <MessageBubble key={item.key} message={item.message} isOwn={item.isOwn} isConsecutive={item.isConsecutive} isFirstInGroup={item.isFirstInGroup} isLastInGroup={item.isLastInGroup} isMiddleInGroup={item.isMiddleInGroup} onImageTap={setLightboxImage} onContextMenu={handleContextMenu} onReply={handleReply} onReact={handleReact} searchHighlight={searchQuery.trim() || null} compact={compact} />;
            if (isNewlySent) {
              return <motion.div key={`entrance-${item.key}`} initial={{
                opacity: 0,
                y: 8
              }} animate={{
                opacity: 1,
      ...
      <MessageInput onSend={handleSend} onTyping={handleTyping} onAttachImage={handleAttachImage} onAttachDocument={handleAttachDocument} replyTo={replyTo} onCancelReply={() => setReplyTo(null)} onVoiceSend={handleVoiceSend} />
    </>;
  };
  return <>
      <div className="fixed inset-0 z-40 flex flex-col bg-white font-apple md:hidden" style={{
        ...
        <div className="grid h-[calc(100vh-88px)] grid-cols-[410px_1fr] overflow-hidden rounded-xl border border-stone-200/90">
          <aside className="flex min-h-0 flex-col border-r border-stone-200 bg-white">
            <div className="flex items-center gap-3 border-b border-stone-100 px-4 py-4">
              ...
              <button className="flex min-h-[40px] min-w-[40px] items-center justify-center rounded-lg text-stone-950 transition-colors duration-150 hover:bg-stone-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-stone-300" aria-label="Nuevo mensaje" onClick={() => navigate('/messages/new')}>
            ...
            <div className="border-b border-stone-100 px-4 pb-3 pt-2">
              <label className="flex h-10 items-center gap-2 rounded-lg bg-stone-100 px-4">
                ...
                <input type="text" value={desktopSearchQuery} onChange={e => setDesktopSearchQuery(e.target.value)} placeholder="Buscar" className="flex-1 bg-transparent text-sm text-stone-950 outline-none placeholder:text-stone-400" />
              </label>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              return <button key={convKey} onClick={() => {
                if (convKey) navigate(`/messages/${convKey}`);
                      ...
                      {conv.avatar_url ? <img src={conv.avatar_url} alt={conv.name} className="h-12 w-12 rounded-full object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-full bg-stone-950 text-base font-semibold text-white">
          ...
          <main className="flex min-h-0 flex-col bg-white">
            {conversation ? renderThread(false) : <div className="flex h-full flex-col items-center justify-center bg-stone-50 px-8 text-center">
      ...
      <AnimatePresence>
        {contextMenu && <MessageContextMenu contextMenu={contextMenu} onClose={() => setContextMenu(null)} userId={user?.user_id || user?.id} onReact={handleReact} onReply={handleReply} onDelete={handleDeleteMessage} />}
      </AnimatePresence>
      <AnimatePresence>
