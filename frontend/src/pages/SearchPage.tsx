...
function SuggestionItem({
  ...
  return <button type="button" onClick={onClick} className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors hover:bg-stone-50">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-stone-100">
        {img ? <img src={img} alt={name} className="h-full w-full object-cover" /> : <Icon size={14} className="text-stone-400" />}
      </div>
...
function AutocompleteDropdown({
  ...
  return <div ref={dropdownRef} className="absolute left-0 right-0 top-full z-50 mt-1 max-h-[420px] overflow-y-auto rounded-2xl bg-white p-2 shadow-lg">
      {sections.map(section => <div key={section.key}>
          ...
          {section.items.map((item, i) => <SuggestionItem key={item.product_id || item.store_id || item.user_id || item.id || i} item={item} type={section.key} onClick={() => onSelect(item, section.key)} />)}
        </div>)}
    </div>;
}
...
const pillCls = active => `flex shrink-0 cursor-pointer items-center gap-1 rounded-full px-3.5 py-1.5 text-[13px] font-medium whitespace-nowrap transition-colors ${active ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-700'}`;
const TABS = [{
  key: 'all',
  label: 'Todo'
}, {
...
function ToggleSwitch({
  ...
  return <button type="button" role="switch" aria-checked={checked} aria-label={label} onClick={() => onChange(!checked)} className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${checked ? 'bg-stone-950' : 'bg-stone-200'}`}>
...
function CardSkeleton() {
  return <div className="overflow-hidden rounded-2xl bg-stone-100">
...
function ProductCardLocal({
  ...
  return <Link to={`/products/${p.product_id || p.id}`} className="group block overflow-hidden rounded-2xl bg-white no-underline shadow-sm">
      ...
      <div className="px-2 pb-2 pt-1.5">
        {p.producer_name && <p className="truncate text-xs text-stone-500">{p.producer_name}</p>}
...
function RecipeCard({
  ...
  return <Link to={`/recipes/${r.recipe_id || r.id}`} className="group block overflow-hidden rounded-2xl bg-white no-underline shadow-sm">
      <div className="overflow-hidden bg-stone-100 aspect-[4/5]">
        {img ? <img src={img} alt={r.title} loading="lazy" className="h-full w-full object-cover transition-transform group-hover:scale-[1.03]" /> : <div className="flex h-full w-full items-center justify-center">
      ...
      <div className="px-2 pb-2 pt-1.5">
        ...
        {r.prep_time_minutes && <p className="mt-0.5 text-xs text-stone-500">{r.prep_time_minutes} min</p>}
      </div>
    </Link>;
}
function PersonRow({
  ...
  return <Link to={`${linkBase}${person.slug || person.store_slug || id}`} className="flex items-center gap-3 border-b border-stone-100 py-2.5 no-underline last:border-b-0">
      <div className={`flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden bg-stone-100 ${isStore ? 'rounded-2xl' : 'rounded-full'}`}>
        {img ? <img src={img} alt={name} loading="lazy" className="h-full w-full object-cover" /> : isStore ? <Store size={18} className="text-stone-400" /> : <span className="text-sm font-semibold text-stone-400">{(name[0] || '?').toUpperCase()}</span>}
      </div>
      ...
      {person.followers_count > 0 && <span className="shrink-0 text-xs text-stone-400">{abbreviateCount(person.followers_count)} seguidores</span>}
    </Link>;
}
function SectionHeader({
  ...
  return <div className="flex items-center justify-between pb-2 pt-4">
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get('/discovery/trending', {
      ...
      const items = (data?.items || data || []).filter(item => item.tag || item.name || item.hashtag);
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-40 bg-white px-3 py-2">
        ...
        }} aria-label="Volver" className="flex h-11 w-11 shrink-0 items-center justify-center">
        ...
        }} aria-label="Filtros" className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-stone-200 bg-white transition-colors hover:bg-stone-50 lg:hidden">
            <SlidersHorizontal size={18} className="text-stone-950" />
            {hasActiveFilters && <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full bg-stone-950 ring-2 ring-white" />}
          </button>

          {/* ── Autocomplete Dropdown ── */}
          {showSuggestions && suggestions && <AutocompleteDropdown suggestions={suggestions} onSelect={handleSuggestionSelect} onClose={() => setShowSuggestions(false)} />}
        </form>
              ...
              <div className="mt-2 rounded-2xl bg-white p-4 shadow-sm">
                ...
                <div className="mb-4 flex items-center gap-2">
                  <div className="flex h-10 flex-1 items-center rounded-xl border border-stone-200 px-3">
                    ...
                    <input type="number" min="0" value={filterMinPrice} onChange={e => setFilterMinPrice(e.target.value)} placeholder="Min" className="w-full bg-transparent text-sm text-stone-950 outline-none placeholder:text-stone-400" />
                  </div>
                  <span className="text-sm text-stone-400">—</span>
                  <div className="flex h-10 flex-1 items-center rounded-xl border border-stone-200 px-3">
                    <span className="mr-1 text-sm text-stone-400">€</span>
                    <input type="number" min="0" value={filterMaxPrice} onChange={e => setFilterMaxPrice(e.target.value)} placeholder="Max" className="w-full bg-transparent text-sm text-stone-950 outline-none placeholder:text-stone-400" />
                  </div>
                </div>
                ...
                return <button key={cert.value} type="button" onClick={() => toggleFilterCert(cert.value)} className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] font-medium transition-colors ${isActive ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-700'}`}>
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={handleApplyFilters} className="flex-1 rounded-full bg-stone-950 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-stone-800">
                    Aplicar filtros
                  </button>
                  <button type="button" onClick={handleClearFilters} className="text-sm text-stone-500 transition-colors hover:text-stone-700">
                    Limpiar
                  </button>
                </div>
              </div>
      ...
      <div className="mx-auto max-w-[975px] px-3 lg:flex lg:gap-6">
        ...
        <aside className="hidden lg:block lg:w-[240px] lg:shrink-0">
          <div className="sticky top-[72px] rounded-2xl bg-white p-4 shadow-sm">
            ...
            <div className="flex flex-col gap-2">
              <button type="button" onClick={handleApplyFilters} className="w-full rounded-full bg-stone-950 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-stone-800">
                Aplicar filtros
              </button>
              {hasActiveFilters && <button type="button" onClick={handleClearFilters} className="text-sm text-stone-500 transition-colors hover:text-stone-700">
                  Limpiar filtros
                </button>}
            </div>
          </div>
        ...
        {!loading && hasResults && <div className="flex gap-2 overflow-x-auto py-2 scrollbar-hide">
            {TABS.map(tab => {
            const count = tab.key === 'all' ? totalCount : counts[tab.key] || 0;
            if (tab.key !== 'all' && count === 0) return null;
            return <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={pillCls(activeTab === tab.key)}>
                  {tab.label}
                  {count > 0 && <span className={`ml-1 min-w-[16px] rounded-full px-1 text-center text-[10px] font-bold ${activeTab === tab.key ? 'bg-white/20 text-white' : 'bg-stone-200 text-stone-500'}`}>
                      {count}
                    </span>}
                </button>;
          })}
        ...
        {/* ── Sort pills ── */}
        {!loading && hasResults && (activeTab === 'all' || activeTab === 'products') && counts.products > 1 && <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {SORT_OPTIONS.map(opt => <button key={opt.value} onClick={() => setSortBy(opt.value)} className={pillCls(sortBy === opt.value)}>
                {opt.label}
              </button>)}
          </div>}
        ...
        {/* ── No results ── */}
        {!loading && results && !hasResults && <motion.div initial={{
          opacity: 0,
          y: 16
        }} animate={{
          opacity: 1,
        ...
        {!loading && hasResults && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} className="pb-20">
        ...
        {!loading && isEmpty && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} className="pb-20">
            {history.length > 0 && <section className="pt-4">
                ...
                <div className="flex flex-col">
                  {history.map(term => <div key={term} className="flex items-center border-b border-stone-100 last:border-b-0">
                      <button onClick={() => handleHistoryClick(term)} className="flex flex-1 items-center gap-3 py-3 text-left">
                        <Clock size={16} className="shrink-0 text-stone-400" />
                        <span className="text-sm text-stone-950">{term}</span>
                      </button>
                      <button onClick={() => handleRemoveHistoryItem(term)} aria-label={`Eliminar ${term} del historial`} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full hover:bg-stone-100">
            ...
            {trendingHashtags.length > 0 && <section className="pt-6">
                ...
                return <button key={tagName + i} onClick={() => navigate(`/hashtag/${encodeURIComponent(tagName)}`)} className="flex items-center gap-3 py-3 text-left border-b border-stone-100 last:border-b-0">
                        <Hash size={16} className="shrink-0 text-stone-400" />
                        <div className="flex-1 min-w-0">
                          <span className="text-sm font-semibold text-stone-950">#{tagName}</span>
                          {count > 0 && <span className="ml-2 text-xs text-stone-500">{count} publicaciones</span>}
                        </div>
                      </button>;
              })}
                </div>
