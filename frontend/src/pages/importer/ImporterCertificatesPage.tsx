  ...
  return <div className={`bg-white rounded-2xl p-3.5 flex items-center gap-3 border ${isExpired ? 'border-stone-400' : isExpiring ? 'border-stone-200' : 'border-stone-200'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-stone-500 flex items-center gap-1.5 flex-wrap">
          ...
          {cert.expiry_date && cert.days_until_expiry != null && <span className={`font-semibold ${isExpired ? 'text-stone-700' : isExpiring ? 'text-stone-700' : isWarning ? 'text-stone-600' : 'text-stone-400'}`}>
              · {isExpired ? `Venció hace ${Math.abs(cert.days_until_expiry)} días` : `Vence en ${cert.days_until_expiry} días`}
            </span>}
        </p>
      </div>
      ...
      <div className="flex gap-1.5 shrink-0">
        {cert.certificate_id && <a href={`/certificate/${cert.product_id || cert.certificate_id}`} target="_blank" rel="noopener noreferrer" className="px-2.5 py-1.5 border border-stone-200 rounded-2xl text-xs font-medium text-stone-700 hover:bg-stone-50 transition-colors">
            Ver
          </a>}
        {cert.pdf_url && <a href={cert.pdf_url} target="_blank" rel="noopener noreferrer" className="px-2.5 py-1.5 border border-stone-200 rounded-2xl text-xs font-medium text-stone-700 hover:bg-stone-50 transition-colors">
...
export default function ImporterCertificatesPage() {
  ...
  const filteredCerts = (statusFilter === 'expiring' ? expiring : statusFilter === 'expired' ? expired : certs).filter(c => {
    if (!search) return true;
    const q = search.toLowerCase();
    return (c.certification_name || c.name || '').toLowerCase().includes(q) || (c.producer_name || '').toLowerCase().includes(q);
  });
  if (!producerId) {
    return <div className="flex items-center justify-center min-h-[60vh] px-4">
  ...
  return <div>
      ...
      <div className="mb-4">
        ...
        <div className="relative mb-3">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por proveedor o tipo..." className="w-full pl-9 pr-4 py-2.5 border border-stone-200 rounded-2xl text-sm bg-white focus:outline-none focus:border-stone-400" />
        </div>

        {/* Filter tabs */}
        <div className="flex gap-2">
          {FILTER_TABS.map(f => <button key={f.id} onClick={() => setStatusFilter(f.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${statusFilter === f.id ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
              {f.label}
      ...
      {error ? <div className="text-center py-16">
          ...
          <p className="text-sm font-semibold text-stone-950 mb-1">{i18n.t('importer_certificates.errorAlCargarLosCertificados', 'Error al cargar los certificados')}</p>
          <button onClick={loadCerts} className="mt-3 px-5 py-2 bg-stone-950 text-white text-sm font-medium rounded-2xl hover:bg-stone-800 transition-colors">
            Reintentar
          </button>
        </div> : loading ? <div className="space-y-2">
          {Array(5).fill(0).map((_, i) => <div key={i} className="h-[72px] rounded-2xl bg-stone-100 animate-pulse" />)}
        </div> : filteredCerts.length === 0 ? <div className="text-center py-16">
          <Award className="w-10 h-10 text-stone-300 mx-auto mb-3" />
          <p className="text-sm font-semibold text-stone-950">Sin certificados registrados</p>
          <p className="text-sm text-stone-500">{i18n.t('importer_certificates.losCertificadosDeTusProveedoresApar', 'Los certificados de tus proveedores aparecerán aquí')}</p>
        </div> : <div className="space-y-2">
          {filteredCerts.map((cert, i) => <CertificateRow key={cert.certificate_id || cert.id || i} cert={cert} />)}
        </div>}
    </div>;
}