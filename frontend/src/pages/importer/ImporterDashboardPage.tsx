...
function KPICard({
  ...
  return <Wrapper to={href} className="p-4 bg-white rounded-2xl shadow-sm transition-all hover:shadow-md">
...
function AlertCard({
  ...
  return <div className={`flex items-start gap-3 p-3 mb-2 rounded-2xl border ${isDanger ? 'bg-stone-100 border-stone-200' : 'bg-stone-100 border-stone-200'}`}>
      ...
      {alert.action_href && <Link to={alert.action_href} className="text-xs font-bold shrink-0 hover:underline text-stone-950">
...
function QuickAction({
  ...
  return <Link to={href} className={`flex items-center gap-3 p-3.5 rounded-2xl transition-all text-sm font-semibold ${isPrimary ? 'bg-stone-950 text-white' : 'bg-white border border-stone-200 text-stone-950 hover:border-stone-200'}`}>
      <Icon className={`w-5 h-5 shrink-0 ${isPrimary ? 'text-white' : 'text-stone-500'}`} />
      {label}
    </Link>;
}
function ImporterPlanCard({
  plan
}) {
  if (plan === 'ELITE' || plan === 'elite') {
    return <Link to="/importer/catalog" className="flex items-center gap-3.5 p-4 transition-colors bg-stone-950 rounded-2xl text-white">
...
function B2BOrderStatusBadge({
  ...
  const config = {
    ...
    confirmed_by_producer: {
      label: 'Confirmado',
      cls: 'bg-stone-100 text-stone-950'
    },
    paid: {
      label: 'Pagado',
      cls: 'bg-stone-100 text-stone-950'
    ...
    delivered: {
      label: 'Recibido',
      cls: 'bg-stone-950 text-white'
    },
    cancelled: {
      label: 'Cancelado',
      cls: 'bg-white text-stone-500'
    },
    rejected: {
      label: 'Rechazado',
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${c.cls}`}>
      {c.label}
    </span>;
}
function B2COrderStatusBadge({
  status
}) {
  const config = {
    pending: {
...
function PeriodSelector({
  ...
  return <div className="flex gap-1 p-0.5 rounded-full bg-stone-100">
      {options.map(o => <button key={o.key} onClick={() => onChange(o.key)} className={`px-3 py-1 text-[11px] font-semibold transition-all rounded-full ${value === o.key ? 'bg-white text-stone-950 shadow-sm' : 'bg-transparent text-stone-500'}`}>
          {o.label}
        </button>)}
    </div>;
}
...
export default function ImporterDashboardPage() {
  ...
  const loadData = useCallback(() => {
    ...
    setError(false);
    setLoading(true);
    Promise.all([apiClient.get(`/importer/stats?period=${period}`).catch(() => null), apiClient.get('/importer/alerts').catch(() => []), apiClient.get('/importer/b2b-orders?limit=3').catch(() => ({
      orders: []
    })), apiClient.get('/producer/orders').catch(() => []), apiClient.get('/b2b/operations').catch(() => []), apiClient.get('/products?seller_id=me&limit=5&sort=sales').catch(() => [])]).then(([s, a, b, orders, ops, products]) => {
      if (!active) return;
      setStats(s || {});
  ...
  if (error) {
    return <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="text-sm font-semibold text-stone-950">{i18n.t('importer_dashboard.errorAlCargarElPanel', 'Error al cargar el panel')}</p>
        <button onClick={loadData} className="px-5 py-2 text-sm font-semibold transition-colors bg-stone-950 text-white rounded-2xl border-none cursor-pointer">
          Reintentar
        </button>
      </div>;
  }
  ...
  const actionableOps = b2bOperations.filter(op => {
    ...
    const actionStatuses = isImporter ? ['offer_received', 'contract_pending', 'contract_generated', 'payment_pending'] : ['offer_sent', 'contract_signed', 'payment_confirmed', 'in_transit'];
     ...
     <div className="max-w-[975px] mx-auto">
      ...
      <div className="flex gap-0 mb-6 p-1 rounded-2xl bg-stone-100">
        {[{
          ...
          return <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold transition-all rounded-2xl ${isActive ? 'bg-white text-stone-950 shadow-sm' : 'bg-transparent text-stone-500'}`}>
      ...
      {activeTab === 'b2c' ? <>
          ...
          {/* Pending B2C orders */}
          {pendingB2cOrders.length > 0 && <div className="mb-5">
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="space-y-2">
                {pendingB2cOrders.slice(0, 4).map((order, i) => <div key={order.order_id || i} className="flex items-center gap-3 p-3.5 bg-white rounded-2xl shadow-sm">
                    <div className="w-9 h-9 flex items-center justify-center shrink-0 rounded-2xl bg-stone-100">
                    ...
                    <Link to={`/producer/orders`} className="w-8 h-8 shrink-0 rounded-full bg-stone-950 flex items-center justify-center">
          ...
          <div className="p-4 mb-5 bg-white rounded-2xl shadow-sm">
            ...
            {topProducts.length > 0 ? topProducts.map((product, i) => <div key={product.product_id || i} className={`flex items-center gap-3 py-2.5 ${i < topProducts.length - 1 ? 'border-b border-stone-200' : ''}`}>
                <div className="w-10 h-10 shrink-0 overflow-hidden rounded-2xl bg-stone-100">
                  {product.images?.[0] ? <img loading="lazy" src={product.images[0]} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
              ...
              </div>) : <p className="text-sm py-3 text-stone-500">
                Aún no tienes productos publicados.
              </p>}
          </div>

          {/* B2C recent orders (completed) */}
          {b2cOrders.filter(o => o.status !== 'pending' && o.status !== 'processing').length > 0 && <div className="p-4 bg-white rounded-2xl shadow-sm">
              <div className="flex items-center justify-between mb-3">
              ...
              {b2cOrders.filter(o => o.status !== 'pending' && o.status !== 'processing').slice(0, 3).map((order, i) => <div key={order.order_id || i} className={`flex items-center justify-between py-2.5 ${i < 2 ? 'border-b border-stone-200' : ''}`}>
          ...
          {/* KPIs */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            <KPICard icon={Package} value={stats?.b2b_active_orders || stats?.total_orders || 0} label="Pedidos B2B activos" href="/importer/orders" />
            <KPICard icon={ShoppingBag} value={convertAndFormatPrice(stats?.volume_month || 0, 'EUR')} label="Volumen este mes" />
            <KPICard icon={Factory} value={stats?.active_suppliers || (stats?.countries_of_origin || []).length || 0} label="Proveedores activos" href="/importer/catalog" />
            {stats?.b2b_operations_count != null ? <KPICard icon={FileCheck} value={stats.b2b_operations_count} label="Operaciones B2B" href="/b2b/operations" /> : <KPICard icon={Store} value="3%" label={i18n.t('influencer.commissionRate', 'Comisión')} />}
          </div>

          {/* Operations requiring action */}
          {actionableOps.length > 0 && <div className="mb-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-bold text-stone-950">{i18n.t('importer_dashboard.requierenTuAccion', 'Requieren tu acción')}</h2>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-stone-100 text-stone-700">
                  {actionableOps.length}
                </span>
              </div>
              <div className="space-y-2">
                {actionableOps.slice(0, 3).map(op => <OperationCard key={op.id || op._id} operation={op} userId={user?.user_id} onNavigate={path => navigate(path)} />)}
              </div>
              {actionableOps.length > 3 && <Link to="/b2b/operations" className="flex items-center justify-center gap-2 mt-3 py-2.5 text-sm font-semibold transition-colors text-stone-950 rounded-2xl border border-stone-200 bg-white">
          ...
          <div className="grid grid-cols-2 gap-2 mb-5">
            <QuickAction icon={Search} label={i18n.t('importer_dashboard.catalogoB2b', 'Catálogo B2B')} href="/importer/catalog" />
          ...
          <div className="p-4 mb-5 bg-white rounded-2xl shadow-sm">
            ...
            {recentB2B.length > 0 ? recentB2B.filter((order, i, arr) => arr.findIndex(o => o.producer_id === order.producer_id) === i).slice(0, 5).map((order, i, arr) => <div key={order.producer_id || i} className={`flex items-center gap-3 py-2.5 ${i < arr.length - 1 ? 'border-b border-stone-200' : ''}`}>
                    <div className="w-9 h-9 flex items-center justify-center shrink-0 rounded-full bg-stone-100 overflow-hidden">
                      {order.producer_avatar ? <img src={order.producer_avatar} alt="" className="w-full h-full object-cover" /> : <span className="text-xs font-bold text-stone-500">
                  ...
                  return d && !isNaN(d.getTime()) ? d.toLocaleDateString('es-ES', {
                    ...
                    {order.producer_id && <button onClick={() => handleB2BChat(order.producer_id)} className="w-8 h-8 shrink-0 flex items-center justify-center rounded-full bg-stone-100 border-none cursor-pointer" aria-label="Chat B2B">
                        <MessageCircle className="w-4 h-4 text-stone-500" />
                      </button>}
                  </div>) : <div className="flex flex-col items-center py-6 gap-3">
                <Factory className="w-8 h-8 text-stone-300" />
                <p className="text-sm text-stone-500">{i18n.t('importer_dashboard.aunNoTienesProveedores', 'Aún no tienes proveedores')}</p>
                <Link to="/b2b/marketplace" className="px-5 py-2.5 bg-stone-950 text-white rounded-full text-sm font-bold hover:bg-stone-800 transition-colors inline-flex items-center gap-1.5">
          ...
          {/* Recent B2B orders */}
          {recentB2B.length > 0 && <div className="p-4 bg-white rounded-2xl shadow-sm">
              <div className="flex items-center justify-between mb-3">
              ...
              {recentB2B.map((order, i) => <div key={order.id || i} className={`flex items-center justify-between py-2.5 ${i < recentB2B.length - 1 ? 'border-b border-stone-200' : ''}`}>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold truncate text-stone-950">
                      {order.producer_name || 'Productor'}
                    </p>
                  ...
                  {order.producer_id && <button onClick={e => {
              e.stopPropagation();
              handleB2BChat(order.producer_id);
            }} className="w-8 h-8 shrink-0 flex items-center justify-center rounded-full bg-stone-100 border-none cursor-pointer" aria-label="Chat B2B">
                      <MessageCircle className="w-4 h-4 text-stone-500" />
                    </button>}
                  <div className="text-right shrink-0 ml-3">
                    <p className="text-sm font-bold text-stone-950">
          ...
          <Link to="/b2b/operations" className="flex items-center justify-center gap-2 mt-4 py-3 text-sm font-semibold transition-colors text-stone-950 rounded-2xl border border-stone-200 bg-white">
          ...
          <div className="mt-5 p-4 bg-white rounded-2xl shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {(stats?.certificates || []).length > 0 ? (stats.certificates || []).map((cert, i) => <span key={i} className="px-3 py-1 rounded-full text-xs font-medium bg-stone-100 text-stone-950">
                  {cert.name || cert}
                </span>) : <p className="text-sm text-stone-500">Sin certificados activos</p>}
            </div>
          </div>
        </>}
      ...
      <div className="mt-5 p-4 bg-white rounded-2xl shadow-sm">
        <h3 className="text-sm font-bold mb-3 text-stone-950">{i18n.t('producer_overview.cuentaYConfiguracion', 'Cuenta y configuración')}</h3>
        <div className="space-y-1">
          {[{
            ...
            sublabel: 'Contratos y certificados',
            to: '/documents'
          }, {
            icon: CreditCard,
            label: i18n.t('producer_payments.metodoDeCobro', 'Método de cobro'),
            sublabel: 'Stripe o transferencia',
            to: '/producer/payments'
          }, {
            icon: KeyRound,
            label: i18n.t('producer_overview.cambiarContrasena', 'Cambiar contraseña'),
            sublabel: '',
            to: '/settings/password'
          }].map((item, i) => <Link key={i} to={item.to} className="flex items-center gap-3 p-3 transition-colors rounded-2xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.sublabel && <p className={`text-[11px] ${item.sublabelCls || 'text-stone-500'}`}>{item.sublabel}</p>}
              </div>
