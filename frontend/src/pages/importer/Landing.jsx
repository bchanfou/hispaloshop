...
const solutionCards = [
  ...
  { icon: BrainCircuit, title: 'Mejor contexto', body: 'Puedes cruzar producto, creadores y demanda con más orden.' },
  { icon: Rocket, title: 'Infraestructura base', body: 'La idea es que no tengas que improvisar cada paso por separado.' },
...
export default function ImporterLanding() {
  ...
  return (
    <div className="min-h-screen bg-white text-stone-950">
      ...
      <main>
        <section className="relative overflow-hidden bg-stone-950 text-[#f5f5f5]">
            ...
            <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
              <div className="max-w-3xl">
                ...
                <motion.div {...fadeUp} transition={{ duration: 0.55, delay: 0.15 }} className="mt-8 flex flex-col gap-3 sm:flex-row">
                  <button type="button" onClick={() => openOnboarding('free')} className="inline-flex items-center justify-center rounded-full bg-white px-7 py-4 text-base font-semibold text-stone-950 transition hover:-translate-y-0.5">
                    Comenzar
                  </button>
                  <button type="button" onClick={scrollToPlans} className="inline-flex items-center justify-center rounded-full border border-white/20 px-7 py-4 text-base font-semibold text-white transition hover:bg-white/10">
                    Ver planes
                  </button>
                </motion.div>
              </div>
              ...
              <motion.aside {...fadeUp} transition={{ duration: 0.55, delay: 0.12 }} className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[0_22px_65px_-38px_rgba(15,23,42,0.45)] backdrop-blur-sm lg:p-8">
                ...
                <p className="mt-4 text-[17px] leading-8 text-white/88">
                  Durante meses ese stock se quedó en mi salón de Seúl mientras yo intentaba abrir puertas sin idioma, sin red y sin una infraestructura comercial real. Perdí 15.000 EUR.
                </p>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div {...fadeUp} className="mx-auto max-w-4xl text-center">
              ...
              <p className="mt-6 text-lg leading-8 text-stone-600">
                Hispaloshop te deja validar productos, conectar con productores, abrir canal directo y reducir la parte ciega del proceso. No elimina el riesgo, pero evita que tomes decisiones sin contexto.
              </p>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div {...fadeUp} className="mx-auto max-w-3xl text-center">
              ...
              <p className="mt-5 text-lg leading-8 text-stone-600">
                No quería una landing llena de promesas vacías. Por eso esta parte es directa: tres niveles y un riesgo más visible desde el principio.
              </p>
            </motion.div>
            ...
            <div className="mt-14 grid gap-6 xl:grid-cols-3">
              {planCards.map((plan, index) => (
                <motion.article key={plan.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: index * 0.08 }} className={`flex h-full flex-col rounded-[30px] border bg-white p-7 shadow-[0_18px_45px_-30px_rgba(15,23,42,0.25)] transition duration-300 hover:-translate-y-1 hover:scale-[1.01] hover:shadow-[0_28px_60px_-32px_rgba(15,23,42,0.3)] ${plan.accent}`}>
                  ...
                  <p className="mt-3 text-sm leading-7 text-stone-600">{plan.audience}</p>
                  <ul className="mt-8 space-y-3 text-sm text-stone-600">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-3">
                        <Handshake className="mt-0.5 h-4 w-4 shrink-0 text-stone-900" aria-hidden="true" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                  <button type="button" onClick={() => openOnboarding(plan.id)} className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-stone-900 px-5 py-4 text-sm font-semibold text-white transition hover:bg-black">
                    {plan.cta}
