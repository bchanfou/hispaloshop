...
function SACard({ children, className = '' }) {
  return (
    <div className={`bg-stone-900 rounded-[14px] border border-stone-800 hover:border-stone-700 transition-colors p-5 ${className}`}>
      {children}
    </div>
  );
}
...
function SkeletonCard({ className = '' }) {
  return (
    <div className={`bg-stone-900 rounded-[14px] border border-stone-800 p-5 animate-pulse ${className}`}>
      <div className="h-3 w-20 bg-stone-800 rounded mb-3" />
      <div className="h-6 w-24 bg-stone-800 rounded mb-2" />
      <div className="h-3 w-16 bg-stone-800 rounded" />
    </div>
  );
}

function StatCard({ icon: Icon, label, value, sub, highlight = false, testId }) {
  return (
    <SACard className="!p-4" data-testid={testId}>
      ...
      <p className={`text-xl font-extrabold ${highlight ? 'text-stone-400' : 'text-stone-100'}`}>{value}</p>
...
function EventBadge({ type }) {
  const config = {
    order_paid: 'bg-stone-200 text-stone-950',
    seller_transfer: 'bg-stone-700/30 text-stone-400',
    influencer_scheduled: 'bg-stone-600/20 text-stone-400',
    influencer_paid: 'bg-stone-700/30 text-stone-300',
    refund: 'bg-stone-700/30 text-stone-400',
  };
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ${config[type] || 'bg-stone-800 text-stone-400'}`}>
...
const DarkTooltip = ({ active, payload, label, formatter }) => {
  ...
  return (
    <div className="bg-stone-950 border border-stone-800 rounded-2xl px-3 py-2 text-xs shadow-xl">
      <p className="text-stone-400 mb-1">{label}</p>
      {payload.map((p, i) => (
        <p key={i} className="text-stone-100 font-medium">
...
export default function FinancialDashboard() {
  ...
  if (loading) {
    return (
      <div className="max-w-[1100px] mx-auto pb-16 space-y-5">
        <div className="mb-4">
  ...
  return (
    <div className="max-w-[1100px] mx-auto pb-16 space-y-5" data-testid="financial-dashboard">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3 py-2 bg-stone-800 rounded-2xl text-stone-300 hover:bg-stone-700 transition-colors"
          >
          ...
          <input
            ...
            className="bg-stone-800/60 border border-stone-800 rounded-2xl px-3 py-2 text-xs text-stone-100 focus:outline-none focus:border-stone-600"
            title="Desde"
          />
          <input
            type="date"
          ...
          <button
            ...
            className="px-4 py-2 bg-stone-800 rounded-2xl text-stone-300 hover:bg-stone-700 transition-colors disabled:opacity-50 inline-flex items-center text-sm"
          >
      ...
      {duePayouts.length > 0 && (
        <div className="bg-stone-700/20 border border-stone-700 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3" data-testid="due-payouts-alert">
          ...
          <button
            ...
            disabled={processing}
            className="px-4 py-2 bg-stone-600 hover:bg-stone-500 disabled:opacity-50 text-stone-100 rounded-2xl transition-colors self-start inline-flex items-center text-sm font-semibold"
      ...
      {/* Summary Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <StatCard icon={DollarSign} label={t('superAdmin.grossRevenue')} value={`${fixed2(summary.total_gross)}\u20AC`} testId="stat-gross" />
        <StatCard icon={TrendingUp} label={t('superAdmin.platformCommission')} value={`${fixed2(summary.total_platform_fee)}\u20AC`} highlight testId="stat-platform-fee" />
        <StatCard icon={Receipt} label={t('superAdmin.paidToSellers')} value={`${fixed2(summary.total_seller_net)}\u20AC`} highlight testId="stat-seller-net" />
        <StatCard icon={Globe} label="Consolidado USD" value={`$${fixed2(summary.total_usd_equivalent)}`} sub="Florida LLC" highlight testId="stat-usd" />
        <StatCard
          icon={Clock}
          label={t('superAdmin.pendingPayouts')}
          value={pendingPayouts.length}
          sub={`${duePayouts.length} vencidos`}
          highlight={duePayouts.length > 0}
          testId="stat-payouts"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <SACard className="lg:col-span-2">
          <h2 className="text-[15px] font-bold text-stone-100 mb-4 flex items-center gap-2">
          ...
          <div className="flex items-center gap-4 mt-3 justify-center text-xs text-stone-400">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-stone-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="text-xs bg-stone-800/60 border border-stone-800 rounded-2xl px-2 py-1.5 text-stone-100 focus:outline-none"
              data-testid="ledger-filter"
            >
              <option value="all" className="bg-stone-900">Todos</option>
              <option value="order_paid" className="bg-stone-900">Pagos</option>
              <option value="seller_transfer" className="bg-stone-900">Transferencias</option>
              <option value="influencer_scheduled" className="bg-stone-900">Influencer prog.</option>
              <option value="influencer_paid" className="bg-stone-900">Influencer pagado</option>
              <option value="refund" className="bg-stone-900">Reembolsos</option>
            </select>
          </div>
        </div>
              ...
              <tbody>
                {filteredEntries.map((e, idx) => (
                  <React.Fragment key={e.ledger_id || idx}>
                    <tr
                      className="border-t border-stone-800 hover:bg-stone-800/50 cursor-pointer transition-colors"
                      onClick={() => setExpandedRow(expandedRow === idx ? null : idx)}
                      data-testid={`ledger-row-${idx}`}
                    >
                      <td className="px-5 py-2.5 text-stone-400 whitespace-nowrap">
                      ...
                      <td className="px-3 py-2.5 text-right font-medium text-stone-100">
                      ...
                      <td className="px-3 py-2.5 text-right font-medium text-stone-300">
                      ...
                      <tr className="bg-stone-800/30">
                        <td colSpan={8} className="px-5 py-3">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                            <div><span className="text-stone-500">Productor:</span> <span className="text-stone-400">{e.seller_id || '-'}</span></div>
                            <div><span className="text-stone-500">Neto productor:</span> <span className="text-stone-400">{fixed2(e.seller_net)} {e.currency}</span></div>
                            <div><span className="text-stone-500">Platform Fee:</span> <span className="text-stone-400">{fixed2(e.platform_fee)} {e.currency}</span></div>
                            <div><span className="text-stone-500">Platform Net:</span> <span className="text-stone-400">{fixed2(e.platform_net)} {e.currency}</span></div>
                            <div><span className="text-stone-500">Tax Type:</span> <span className="text-stone-400">{e.product_tax_type}</span></div>
                            <div><span className="text-stone-500">VAT Rate:</span> <span className="text-stone-400">{(safeNumber(e.vat_rate_applied) * 100).toFixed(1)}%</span></div>
                            <div><span className="text-stone-500">Reverse Charge:</span> <span className="text-stone-400">{e.reverse_charge_applied ? 'Sí' : 'No'}</span></div>
                            <div><span className="text-stone-500">Transfer ID:</span> <span className="text-stone-400 font-mono">{e.transfer_id || '-'}</span></div>
                            {e.influencer_id && (
                              <div><span className="text-stone-500">Influencer:</span> <span className="text-stone-400">{e.influencer_id}</span></div>
                            )}
                            {safeNumber(e.influencer_amount) > 0 && (
                              <div><span className="text-stone-500">Influencer Amount:</span> <span className="text-stone-400">{fixed2(e.influencer_amount)} {e.currency}</span></div>
                            )}
                          </div>
                        </td>
                      </tr>
      ...
      {pendingPayouts.length > 0 && (
        <SACard className="overflow-hidden !p-0" data-testid="scheduled-payouts-table">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-stone-500 text-[11px] uppercase tracking-wider border-b border-stone-800">
              ...
              <tbody>
                {pendingPayouts.map((p) => {
                  ...
                  return (
                    <tr key={p.payout_id} className="border-t border-stone-800">
                      <td className="px-5 py-2.5 text-stone-400">{p.influencer_id}</td>
                      <td className="px-3 py-2.5 font-mono text-stone-500">{(p.order_id || '').slice(-8)}</td>
                      <td className="px-3 py-2.5 text-right font-medium text-stone-100">{fixed2(p.amount)} {p.currency}</td>
                      <td className="px-3 py-2.5 text-stone-400">{(p.due_date || '').slice(0, 10)}</td>
                      <td className="px-3 py-2.5">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ${isDue ? 'bg-stone-700/30 text-stone-300' : 'bg-stone-700/30 text-stone-400'}`}>
