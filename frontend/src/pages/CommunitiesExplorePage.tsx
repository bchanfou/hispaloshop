...
function stoneBg(name) {
  return STONE_BG[(name || 'C').charCodeAt(0) % 5];
}
...
export default function CommunitiesExplorePage() {
  ...
  return <div className="min-h-screen bg-stone-50">
      ...
      <div className="mx-auto flex max-w-[975px] items-center gap-3 px-4 py-3">
        ...
        {canCreate && <Link to="/communities/new" className="rounded-full bg-stone-950 px-3.5 py-1.5 text-[13px] font-semibold text-white no-underline">
            + Crear
          </Link>}
      </div>
      </div>
      ...
      {featuredCommunities.length > 0 && !searchInput && <section className="mx-auto max-w-[975px] px-4 pt-4">
          <h2 className="mb-2.5 text-sm font-bold uppercase tracking-wide text-stone-950">Comunidades destacadas</h2>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {featuredCommunities.map(c => <FeaturedCard key={c.id || c._id} community={c} />)}
          </div>
        </section>}
      ...
      <div className="mx-auto flex max-w-[975px] gap-2 overflow-x-auto px-4 py-3 scrollbar-hide">
        ...
        return <button key={f.id} onClick={() => setFilter(f.id)} aria-label={`Filtrar: ${f.label}`} aria-pressed={active} className={`shrink-0 cursor-pointer whitespace-nowrap rounded-full border px-4 py-1.5 text-[13px] font-medium transition-colors active:scale-95 ${active ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-white text-stone-950'}`}>
              {f.label}
            </button>;
      })}
      </div>
      ...
      <div className="mx-auto max-w-[975px] px-4 pb-24">
        ...
        {user && myCommunities.length > 0 && filter !== 'joined' && !searchInput && <section className="mb-6">
        ...
        {filter === 'joined' && <section className="mb-5">
        ...
        {/* ── Section label ── */}
        {filter !== 'joined' && <div className="mb-3 flex items-center justify-between">
            <h2 className="text-base font-bold uppercase tracking-wide text-stone-950">
              {filter === 'popular' ? 'Comunidades populares' : 'Comunidades'}
            </h2>
            {!isLoading && <span className="text-xs text-stone-500">{communities.length} resultados</span>}
          </div>}

        {/* ── Grid ── */}
        {filter !== 'joined' && (isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5" aria-busy="true" aria-label="Cargando comunidades">
              {Array(6).fill(0).map((_, i) => <div key={i} aria-hidden="true" className="overflow-hidden rounded-2xl border border-stone-100 bg-white">
                  {/* Cover placeholder */}
            ...
            </div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {communities.map((c, i) => <motion.div key={c.id || c._id} initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
...
const MemberAvatars = ({
  ...
  return <div className="flex items-center gap-1.5">
      <div className="flex items-center">
        {shown.map((m, i) => <img key={m.id || m._id || i} src={m.avatar_url || m.avatar || m.profile_image} alt="" className={`w-6 h-6 rounded-full border-2 border-white object-cover ${i > 0 ? '-ml-2' : ''}`} loading="lazy" />)}
      </div>
      {total > shown.length && <span className="text-[10px] text-stone-500">+ {(total - shown.length).toLocaleString()} miembros</span>}
    </div>;
};
...
const FeaturedCard = ({
  ...
  return <div onClick={() => navigate(`/communities/${community.slug || community.id || community._id}`)} className="w-[280px] shrink-0 cursor-pointer rounded-2xl bg-stone-950 p-4 text-white">
      ...
      <button onClick={handleJoin} disabled={joining} className="rounded-full border border-white/30 bg-transparent px-4 py-1.5 text-[13px] font-semibold text-white cursor-pointer hover:bg-white/10 transition-colors disabled:opacity-50">
        {joining ? '...' : 'Unirse'}
      </button>
    </div>;
};
...
const CommunityCard = React.memo(({
  ...
  return <Link to={`/communities/${community.slug || community.id || community._id}`} className="block no-underline">
      <div className="overflow-hidden rounded-2xl shadow-sm bg-white transition-transform duration-200 hover:scale-[1.02]">
        ...
        <div className="relative aspect-[16/7] overflow-hidden">
          {community.cover_image ? <img src={community.cover_image} alt={community.name || ''} loading="lazy" className="block h-full w-full object-cover" /> : <div className={`flex h-full w-full items-center justify-center text-[32px] ${stoneBg(community.name)}`}>
              {community.emoji || '🌿'}
            </div>}
          {community.category && <span className="absolute left-1.5 top-1.5 rounded bg-black/60 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-white">
              {community.category}
            </span>}
        </div>
        ...
        <div className="p-2.5">
          ...
          {community.description && <p className="mb-1.5 truncate text-[11px] text-stone-500">
          ...
          scale: 0.92
        }} onClick={toggle} disabled={isToggling} aria-label={joined ? `Salir de ${community.name || 'comunidad'}` : `Unirse a ${community.name || 'comunidad'}`} className={`w-full cursor-pointer rounded-full border py-1.5 text-xs font-semibold transition-colors ${joined ? 'border-stone-200 bg-white text-stone-500' : 'border-stone-950 bg-stone-950 text-white'}`}>
