...
const DIFFICULTY_CLASSES = {
  ...
  hard: {
    pill: 'bg-stone-950 text-stone-50',
    label: "Difícil"
  }
};
...
function RecipeCard({
  ...
  return <Link to={`/recipes/${recipe.recipe_id}`} className="block no-underline">
      <div className="overflow-hidden rounded-2xl bg-white">
        ...
        <div className="relative aspect-[4/5] overflow-hidden bg-stone-100">
          ...
          {recipe.is_saved && <span className="absolute right-2 top-2 flex items-center justify-center rounded-full bg-white/90 p-1 shadow-sm">
        ...
        <div className="px-1 pt-2 pb-1.5">
          ...
          <div className="mt-1 flex items-center gap-2 flex-wrap">
            ...
            {recipe.difficulty && DIFFICULTY_CLASSES[recipe.difficulty] && <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${DIFFICULTY_CLASSES[recipe.difficulty].pill}`}>
                {DIFFICULTY_CLASSES[recipe.difficulty].label}
              </span>}
            {recipe.avg_rating > 0 && <span className="flex items-center gap-0.5 text-[11px] text-stone-500">
                <Star size={10} fill="currentColor" /> {recipe.avg_rating}
              </span>}
          </div>
          {recipe.likes_count > 0 && <p className="mt-0.5 flex items-center gap-0.5 text-[10px] text-stone-400">
...
function FilterSection({
  ...
  return <div className="scrollbar-hide flex gap-2 overflow-x-auto">
      {pills.map(p => {
      const isActive = active === p.id;
      return <button key={p.id} onClick={() => onSelect(p.id)} aria-pressed={isActive} className={`shrink-0 whitespace-nowrap rounded-full px-3.5 py-2 text-xs font-medium transition-colors ${isActive ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
            {p.label}
          </button>;
    })}
    </div>;
...
export default function RecipesPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setFetchError(false);
    apiClient.get('/recipes').then(data => {
      if (active) {
  ...
  const filtered = useMemo(() => {
    ...
    return recipes.filter(r => {
      if (needle && !(r.title || '').toLowerCase().includes(needle) && !(r.author_name || '').toLowerCase().includes(needle) && !r.tags?.some(t => t.toLowerCase().includes(needle))) return false;
      ...
      if (allergenFilters.size > 0) {
        ...
        for (const allergen of allergenFilters) {
          const isFree = allergenFreeFlags[allergen] === true || tags.includes(`sin_${allergen}`) || tags.includes(`${allergen}_free`) || allergen === 'gluten' && tags.includes('sin gluten') || allergen === 'lactose' && tags.includes('sin lactosa') || allergen === 'nuts' && tags.includes('sin frutos secos') || allergen === 'vegan' && (tags.includes('vegano') || (r.diet || '').toLowerCase() === 'vegano');
  ...
  useEffect(() => {
    setVisibleCount(12);
  }, [debouncedSearch, difficulty, timeFilter, dietFilter, allergenKey]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) setVisibleCount(p => p + 12);
    }, {
      rootMargin: '200px'
    });
  ...
  const handleRetry = () => {
    setLoading(true);
    setFetchError(false);
    apiClient.get('/recipes').then(data => setRecipes(Array.isArray(data) ? data : [])).catch(() => setFetchError(true)).finally(() => setLoading(false));
  };
  const clearFilters = () => {
    setSearchInput('');
    setDifficulty('all');
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between pt-3 pb-1">
        ...
        {user && <Link to="/recipes/create" className="flex items-center gap-1 rounded-full bg-stone-950 px-3.5 py-2 text-[13px] font-semibold text-white no-underline hover:bg-stone-800 transition-colors">
            <Plus size={14} /> Crear
          </Link>}
      </div>
      ...
      <div className="scrollbar-hide flex gap-2 overflow-x-auto pb-2">
        {ALLERGEN_PILLS.map(p => {
          ...
          return <button key={p.id} onClick={() => toggleAllergen(p.id)} aria-pressed={isActive} className={`shrink-0 whitespace-nowrap rounded-full px-3.5 py-2 text-xs font-medium transition-colors ${isActive ? 'bg-stone-950 text-white' : 'bg-stone-50 text-stone-500 hover:bg-stone-100'}`}>
              {p.label}
            </button>;
        })}
      </div>
      ...
      <div className="pb-24 pt-1">
        ...
        <div className="mb-2 flex items-center justify-between">
          {!loading && <span className="text-xs text-stone-500">
              {filtered.length} receta{filtered.length !== 1 ? 's' : ''}
            </span>}
          {hasFilters && <button type="button" onClick={clearFilters} className="border-none bg-transparent text-xs font-semibold text-stone-500 cursor-pointer hover:text-stone-700">
              Limpiar filtros
            </button>}
        </div>

        {/* Grid */}
        {loading ? <div aria-busy="true" aria-label="Cargando recetas" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {Array(6).fill(0).map((_, i) => <div key={i} aria-hidden="true" className="aspect-[4/5] animate-pulse rounded-2xl bg-stone-100" />)}
            ...
            {hasFilters ? <button type="button" onClick={clearFilters} className="rounded-full border border-stone-200 px-5 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">
                Limpiar filtros
              </button> : user ? <Link to="/recipes/create" className="bg-stone-950 text-white rounded-full px-6 py-2.5 text-sm font-semibold hover:bg-stone-800 transition-colors no-underline">
                Crear receta
              </Link> : null}
          </div> : <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered.slice(0, visibleCount).map((recipe, i) => <motion.div key={recipe.recipe_id} initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
            ...
            {visibleCount < filtered.length && <div ref={sentinelRef} className="flex justify-center p-6">
