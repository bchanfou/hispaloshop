...
const Step2Basic = ({
  ...
  const fieldClass = (error, wasTouched) => `mt-2 h-12 w-full rounded-xl border bg-white px-3 text-base md:h-11 md:text-sm ${error && wasTouched ? 'border-stone-950' : 'border-stone-200'}`;
  const handleSubmit = () => {
  ...
  return <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-stone-950">{i18n.t('step2_basic.datosBasicos', 'Datos básicos')}</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <input id="consumer-first-name" value={values.firstName} onChange={handleChange('firstName')} onBlur={handleBlur('firstName')} placeholder={i18n.t('step2_basic.maria', 'María')} className={fieldClass(errors.firstName, touched.firstName)} autoCapitalize="words" />
          ...
          <div>
            ...
            <input id="consumer-last-name" value={values.lastName} onChange={handleChange('lastName')} onBlur={handleBlur('lastName')} placeholder={i18n.t('step2_basic.garciaLopez', 'García López')} className={fieldClass(errors.lastName, touched.lastName)} autoCapitalize="words" />
        ...
        <div>
          ...
          <input id="consumer-password" type="password" value={values.password} onChange={handleChange('password')} onBlur={handleBlur('password')} placeholder={i18n.t('step2_basic.creaUnaContrasenaSegura', 'Crea una contraseña segura')} className={fieldClass(errors.password, touched.password)} />
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-stone-500">{i18n.t('step2_basic.debesTenerAlMenos16Anos', 'Debes tener al menos 16 años.')}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-stone-200 bg-stone-50 p-3">
          ...
          <span className="text-sm text-stone-600">
            Acepto los <a href="/terms" className="font-medium text-stone-950 hover:underline">{i18n.t('step2_basic.terminos', 'términos')}</a> y la <a href="/privacy" className="font-medium text-stone-950 hover:underline">{i18n.t('step2_basic.politicaDePrivacidad', 'política de privacidad')}</a><span className="ml-1 text-stone-500">*</span>
          </span>
        </label>
      ...
      <button type="button" onClick={handleSubmit} disabled={!acceptTerms} className="flex w-full items-center justify-center gap-2 rounded-full bg-stone-950 py-3 font-medium text-white transition-colors hover:bg-black disabled:cursor-not-allowed disabled:bg-stone-300">
        Continuar
