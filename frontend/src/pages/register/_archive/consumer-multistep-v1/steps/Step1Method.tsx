...
const Step1Method = ({
  ...
  return <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => navigate('/register')} aria-label={i18n.t('step1_method.volverASeleccionDeRol', 'Volver a selección de rol')} className="rounded-full p-2 transition-colors hover:bg-stone-100">
      ...
      onNext();
        ...
        <div className="flex-1">
          ...
          <span className="mt-1 block text-sm text-stone-500">{i18n.t('step1_method.crearasTuCuentaYDespuesCompletaremo', 'Crearás tu cuenta y después completaremos tus preferencias.')}</span>
        </div>
      </button>
      ...
      <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-stone-950">{i18n.t('b2_b_tracking.proximamente', 'Próximamente')}</p>
      ...
      <p className="text-center text-sm text-stone-600">
        ¿Ya tienes cuenta?{' '}
        <button type="button" onClick={() => navigate('/login')} className="font-medium text-stone-950 transition-colors hover:text-black">
          Inicia sesión
        </button>
      </p>
    </div>;
