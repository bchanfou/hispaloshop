...
const FREQUENCY_OPTIONS = ['Semanalmente', 'Mensualmente', 'Solo en ocasiones especiales'];
const Step4Preferences = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        return <button key={option.value} type="button" onClick={() => setDiscoveryMethod(option.value)} className={`flex w-full items-start gap-3 rounded-2xl border p-4 text-left transition-colors ${selected ? 'border-stone-950 bg-stone-100' : 'border-stone-200 bg-white hover:border-stone-200'}`}>
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          return <button key={option} type="button" onClick={() => setFrequency(value)} className={`flex w-full items-center justify-between rounded-2xl border p-4 text-left transition-colors ${selected ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-white text-stone-700 hover:border-stone-200'}`}>
