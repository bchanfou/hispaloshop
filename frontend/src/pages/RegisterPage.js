...
const inputClass = (hasError) =>
  `mt-2 h-12 rounded-2xl border bg-white px-3 text-base md:h-11 md:text-sm ${hasError ? 'border-red-500' : 'border-stone-200'}`;
...
const renderField = ({ id, label, required, error, children }) => (
  <div>
    ...
    {children}
    {error ? <p className="mt-1 text-xs text-red-600 md:text-sm">{error}</p> : null}
  </div>
);
          ...
          <section className="rounded-[28px] border border-stone-200 bg-white p-4 shadow-sm md:p-8" data-testid="register-form">
            <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
              ...
              <div>
                ...
                {formErrors.country ? <p className="mt-1 text-xs text-red-600 md:text-sm">{formErrors.country}</p> : null}
              </div>
              ...
              {formData.role === 'customer' && (
                <div className="overflow-hidden rounded-2xl border border-stone-200 bg-stone-50" data-testid="consent-section">
                  ...
                  <div className="border-t border-stone-200 bg-white px-4 py-4">
                    <label htmlFor="analytics_consent" className={`flex cursor-pointer items-start gap-4 rounded-2xl border-2 p-3 transition-colors ${formData.analytics_consent ? 'border-stone-900 bg-stone-100' : 'border-stone-200 bg-stone-50 hover:border-stone-300'}`} data-testid="consent-label">
                      ...
                      <div className="min-w-0 flex-1">
                        <span className="block text-sm font-medium text-stone-950 md:text-base">Acepto el tratamiento de datos para personalización y asistencia<span className="ml-1 text-red-500">*</span></span>
              ...
              <Button type="submit" disabled={loading} className="h-12 w-full rounded-full bg-stone-950 text-base font-medium text-white transition-colors hover:bg-black md:h-11 md:text-sm" data-testid="register-submit-button">
