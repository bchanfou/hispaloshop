...
function OrderTimeline({
  ...
  return <div className="flex items-center w-full px-1 mt-2 mb-1">
      ...
      return <React.Fragment key={step.key}>
            ...
            {i > 0 && <div className={`flex-1 h-[2px] ${isCompleted || isActive ? 'bg-stone-950' : 'bg-stone-200'}`} />}
            {/* Step circle + label */}
            <div className="flex flex-col items-center gap-0.5 shrink-0">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center ${isCompleted ? 'bg-stone-950' : isActive ? i === 0 ? 'border-2 border-stone-950 bg-white' : 'bg-stone-950' : 'bg-stone-200'}`}>
...
const FILTER_TABS = [{
  ...
  key: 'active',
  label: 'Activos',
  statuses: ['pending', 'confirmed', 'preparing', 'shipped', 'paid', 'processing']
}, {
  key: 'delivered',
  label: 'Entregados',
  ...
  key: 'cancelled',
  label: 'Cancelados',
  statuses: ['cancelled', 'refunded', 'partially_refunded', 'payment_failed']
}];
export default function CustomerOrders() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    const sorted = [...result];
    if (sortBy === 'recent') sorted.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());else if (sortBy === 'oldest') sorted.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());else if (sortBy === 'highest') sorted.sort((a, b) => asNumber(b.total_amount) - asNumber(a.total_amount));
    return sorted;
    ...
    overscrollBehavior: 'none'
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex gap-2 flex-1 overflow-x-auto scrollbar-hide pb-1">
          {FILTER_TABS.map(tab => {
            ...
            return <button key={tab.key} onClick={() => setActiveFilter(tab.key)} className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeFilter === tab.key ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
                {tab.label} {count > 0 && <span className="ml-1 text-xs opacity-70">({count})</span>}
              </button>;
          })}
        </div>
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="shrink-0 rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-700 outline-none focus:border-stone-400">
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {loading ? <div className="p-4 space-y-3">
            {[1, 2, 3].map(i => <div key={i} className="flex items-center gap-4 p-4">
          ...
          </div> : <div className="divide-y divide-stone-100" data-testid="orders-list">
            {filteredOrders.map(order => {
            const StatusIcon = getStatusIcon(order.status);
            const statusColor = getStatusColor(order.status);
            const isCancelled = CANCELLED_STATUSES.includes(order.status);
            const isDelivered = ['delivered', 'completed'].includes(order.status);
            return <div key={order.order_id} className="p-4 hover:bg-stone-50 transition-colors" data-testid={`order-row-${order.order_id}`}>
                  {/* Top row — clickable to detail */}
                  <button onClick={() => navigate(`/dashboard/orders/${order.order_id}`)} className="w-full flex items-center gap-4 text-left">
                    {/* Status icon */}
                  ...
                  {isCancelled && <div className="mt-2 ml-14 flex items-center gap-2">
                      <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium ${order.status === 'refunded' ? 'bg-stone-100 text-stone-700' : 'bg-stone-100 text-stone-500'}`}>
                        {order.status === 'refunded' || order.status === 'partially_refunded' ? <RotateCcw size={12} /> : <Clock size={12} />}
                        {STATUS_LABELS[order.status] || order.status}
                      </span>
                      {order.status === 'refunded' && order.refund_amount_cents != null && <span className="text-xs text-stone-500">
                          {convertAndFormatPrice((order.refund_amount_cents || 0) / 100, order.currency || 'EUR', currency)}
                        </span>}
                      {order.cancelled_at && <span className="text-[10px] text-stone-400">
                          {new Date(order.cancelled_at).toLocaleDateString('es-ES', {
                    day: 'numeric',
                    month: 'short'
                  })}
                        </span>}
                  ...
                  {(order.tracking_url || isDelivered) && <div className="mt-2 ml-14 flex items-center gap-3 flex-wrap">
                      {order.tracking_url && <a href={order.tracking_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="inline-flex items-center gap-1.5 text-xs font-medium text-stone-700 hover:text-stone-950 transition-colors">
                          <ShoppingBag size={14} /> Rastrear envío <ExternalLink size={12} />
                        </a>}
                      {isDelivered && <button onClick={async e => {
                  e.stopPropagation();
                  try {
                ...
                }} disabled={reorderingId === order.order_id} className="inline-flex items-center gap-1.5 border border-stone-200 rounded-full px-4 py-2 text-sm font-semibold text-stone-950 hover:bg-stone-50 transition-colors disabled:opacity-50" data-testid={`reorder-${order.order_id}`}>
