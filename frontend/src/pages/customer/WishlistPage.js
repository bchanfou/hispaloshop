  ...
  if (items.length === 0) {
    return (
      <div className="py-16 text-center" data-testid="wishlist-empty">
        ...
        <h3 className="text-lg font-semibold text-stone-700 mb-1">{t('wishlist.emptyTitle', 'Tu lista de deseos esta vacia')}</h3>
        ...
        {items.map(item => {
          ...
          return (
            <div key={item.product_id} className="flex items-center gap-3 bg-white border border-stone-200 rounded-xl p-3 hover:shadow-sm transition-shadow" data-testid={`wishlist-item-${item.product_id}`}>
              <Link to={`/product/${item.product_id}`} className="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-stone-100">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/product/${item.product_id}`} className="text-sm font-medium text-stone-900 hover:underline line-clamp-1">{item.name}</Link>
