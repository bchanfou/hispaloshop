        ...
        {items.map(item => {
          ...
          return (
            <div key={item.product_id} className="flex items-center gap-3 bg-white border border-stone-200 rounded-xl p-3 hover:shadow-sm transition-shadow" data-testid={`wishlist-item-${item.product_id}`}>
              ...
              <button onClick={() => remove(item.product_id)} className="p-2 text-stone-400 hover:text-stone-950 hover:bg-stone-100 rounded-lg transition-colors" data-testid={`remove-wishlist-${item.product_id}`}>
