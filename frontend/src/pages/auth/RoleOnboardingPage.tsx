...
function OnboardingNav({
  ...
  return <div className="flex flex-col gap-2">
      <button onClick={onNext} disabled={nextDisabled} className="flex h-12 w-full items-center justify-center rounded-full bg-stone-950 text-sm font-medium text-white transition-colors hover:bg-black disabled:cursor-not-allowed disabled:bg-stone-300">
        {nextLabel}
      </button>
      {skipLabel && onSkip && <button onClick={onSkip} className="bg-transparent border-none cursor-pointer p-2 text-sm text-stone-500">
          {skipLabel}
        </button>}
      {onBack && <button onClick={onBack} className="bg-transparent border-none cursor-pointer p-2 text-sm text-stone-500">
          ← Atrás
        </button>}
    </div>;
}
...
function SelectGrid({
  ...
  return <div className="mb-6 grid grid-cols-2 gap-2">
      {options.map(opt => <motion.button key={opt.id} whileTap={{
      scale: 0.95
    }} onClick={() => onToggle(opt.id)} className={`cursor-pointer rounded-xl border-2 px-3 py-3.5 text-center transition-all ${selected.includes(opt.id) ? 'border-stone-950 bg-stone-950 text-white' : 'border-stone-200 bg-stone-100 text-stone-950 hover:border-stone-200'}`}>
...
function CustomerOnboarding({
  ...
  useEffect(() => {
    if (step === 3) {
      apiClient.get('/stores?limit=6&sort=rating').then(data => {
  ...
  const saveAndNext = async () => {
    try {
      ...
      if (step < 3) setStep(step + 1);else onFinish();
    }
  };
  return <div className="w-full max-w-[400px]">
      {step === 1 && <>
          ...
          <SelectGrid options={DIET_OPTIONS} selected={diet} onToggle={id => toggleItem(diet, setDiet, id)} />
          <OnboardingNav onNext={saveAndNext} nextLabel={diet.length > 0 ? 'Continuar →' : 'Saltar →'} />
        </>}
      {step === 2 && <>
          ...
          <SelectGrid options={ALLERGY_OPTIONS} selected={allergies} onToggle={id => toggleItem(allergies, setAllergies, id)} />
          <OnboardingNav onBack={() => setStep(1)} onNext={saveAndNext} nextLabel={allergies.length > 0 ? 'Continuar →' : 'Sin alergias →'} />
        </>}
      {step === 3 && <>
          ...
          <div className="mb-6 flex flex-col gap-2.5">
            {suggestedProducers.map(p => <div key={p.producer_id || p.id || p.store_id} className="flex items-center gap-3 rounded-xl border border-stone-200 bg-stone-100 px-3.5 py-3">
                {p.logo_url || p.avatar_url ? <img src={p.logo_url || p.avatar_url} alt="" className="h-11 w-11 rounded-full object-cover" /> : <div className="flex h-11 w-11 items-center justify-center rounded-full bg-stone-50 text-xl">
            ...
            scale: 0.92
          }} onClick={() => toggleItem(followedProducers, setFollowedProducers, p.producer_id || p.id)} className={`shrink-0 rounded-full border-none px-4 py-1.5 text-[13px] font-semibold transition-all ${followedProducers.includes(p.producer_id || p.id) ? 'bg-stone-50 text-stone-950' : 'bg-stone-950 text-white'}`}>
                  {followedProducers.includes(p.producer_id || p.id) ? 'Siguiendo' : 'Seguir'}
                </motion.button>
              </div>)}
            {suggestedProducers.length === 0 && <p className="py-5 text-center text-[13px] text-stone-500">
                Cargando productores...
              </p>}
          </div>
          <OnboardingNav onBack={() => setStep(2)} onNext={saveAndNext} nextLabel="Ir al feed →" />
        </>}
    </div>;
}
...
function ProducerOnboarding({
    ...
    if (product.name && product.price) {
      try {
        await apiClient.post('/products', {
          ...
          category_id: product.category || 'otros',
          status: 'pending',
          description: '',
          images: []
        });
  ...
  return <div className="w-full max-w-[400px]">
      ...
      {step === 2 && <>
          ...
          <div className="mb-6 flex flex-col gap-3">
            <div>
              ...
              <input className={inputClass} value={product.name} onChange={e => setProduct({
            ...product,
            name: e.target.value
            ...
            <div>
              ...
              <input className={inputClass} type="number" value={product.price} onChange={e => setProduct({
            ...product,
          ...
          <OnboardingNav onBack={() => setStep(1)} onNext={saveStep2} nextLabel="Continuar →" skipLabel="Saltar por ahora" onSkip={() => setStep(3)} />
        </>}
      ...
      {step === 3 && <>
          ...
          <button onClick={connectStripe} disabled={stripeLoading} className="mb-2.5 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-stone-950 text-sm font-medium text-white transition-colors hover:bg-black disabled:cursor-not-allowed disabled:bg-stone-300">
            {stripeLoading ? <><Loader2 className="h-4 w-4 animate-spin" /> Redirigiendo...</> : 'Conectar cuenta bancaria →'}
          </button>
          <button onClick={onFinish} className="w-full cursor-pointer border-none bg-transparent p-3 text-sm text-stone-500">
            Lo haré más tarde
          </button>
        </>}
    </div>;
...
function InfluencerOnboarding({
  ...
  useEffect(() => {
    if (step === 1) {
      apiClient.get('/products?limit=12&sort=popular').then(data => {
  ...
  const saveProducts = async () => {
    if (selectedProducts.length > 0) {
      try {
        await apiClient.post('/influencer/product-selection', {
  ...
  return <div className="w-full max-w-[400px]">
      {step === 1 && <>
          ...
          return <motion.div key={pid} whileTap={{
            scale: 0.95
          }} onClick={() => toggleItem(selectedProducts, setSelectedProducts, pid)} className={`relative cursor-pointer overflow-hidden rounded-xl border-2 transition-all ${isSelected ? 'border-stone-950' : 'border-transparent'}`}>
                  <img src={product.images?.[0] || '/placeholder.png'} alt={product.name} className="block aspect-square w-full object-cover" />
                  {isSelected && <div className="absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-stone-950 text-[11px] text-white">
                      ✓
                    </div>}
      ...
      {step === 3 && <>
          ...
          emoji: '🔗',
          title: 'Tus links',
          desc: 'Genera links para cada producto'
        }, {
          emoji: '💸',
          ...
          <button onClick={onFinish} className="flex h-12 w-full items-center justify-center rounded-full bg-stone-950 text-sm font-medium text-white transition-colors hover:bg-black">
            Ir a mi dashboard →
          </button>
        </>}
    </div>;
...
export default function RoleOnboardingPage() {
  ...
  if (authLoading) {
    return <div className="flex min-h-dvh items-center justify-center bg-stone-50">
        <Loader2 className="h-8 w-8 animate-spin text-stone-400" />
      </div>;
  }
  return <div className="flex min-h-dvh flex-col bg-stone-50">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-stone-200 bg-stone-100 px-5 py-4">
        ...
        <button onClick={handleFinish} className="cursor-pointer border-none bg-transparent text-sm font-medium text-stone-500 transition-colors hover:text-stone-700">
          Saltar
        </button>
      </div>
      ...
      <div className="h-[3px] bg-stone-50">
        <div className="h-full bg-stone-950 transition-[width] duration-400 ease-out" style={{
      ...
      <div className="flex justify-center gap-1.5 pb-8 pt-4">
        {[1, 2, 3].map(i => <div key={i} className={`h-[7px] rounded-full transition-all duration-300 ${i === step ? 'w-5 bg-stone-950' : 'w-[7px] bg-stone-200'}`} />)}
      </div>
    </div>;
}