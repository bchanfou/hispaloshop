...
export default function CertificatePage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#FAF7F2] to-[#F4EFE9] flex items-center justify-center" data-testid="certificate-loading">
        <div className="text-center">
          <div className="relative">
            <div className="w-16 h-16 border-4 border-[#DED7CE] rounded-full"></div>
            <div className="w-16 h-16 border-4 border-[#1C1C1C] border-t-transparent rounded-full animate-spin absolute top-0 left-0"></div>
          </div>
          <p className="font-body text-sm text-[#7A7A7A] mt-4">{t('common.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!certificate || !product) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#FAF7F2] to-[#F4EFE9] flex items-center justify-center" data-testid="certificate-not-found">
        <div className="text-center bg-white p-12 rounded-2xl shadow-lg max-w-md">
          <FileCheck className="w-16 h-16 text-[#DED7CE] mx-auto mb-4" />
          <p className="font-heading text-xl text-[#1C1C1C] mb-2">{t('errors.notFound')}</p>
          <p className="text-[#7A7A7A] mb-6">{t('certificate.notFoundDesc', 'This certificate could not be found')}</p>
          <Link to="/certificates">
            <Button className="bg-[#1C1C1C] hover:bg-[#4A4A4A] text-white rounded-full px-8">
              {t('common.back')}
            </Button>
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FAF7F2] to-[#F4EFE9]" data-testid="certificate-page">
      <div className="max-w-[1000px] mx-auto px-4 sm:px-6 py-8 md:py-12">
        {/* Top Bar */}
        <div className="flex items-center justify-between mb-8">
          <button
            onClick={handleBack}
            className="inline-flex items-center gap-2 font-body text-sm text-[#7A7A7A] hover:text-[#1C1C1C] transition-colors group"
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-[#DED7CE]">
          
          {/* Header with Product Image */}
          <div className="relative bg-gradient-to-br from-[#1C1C1C] to-[#4A4A4A] p-8 md:p-12">
            {/* Decorative pattern */}
          ...
          {/* Quick Stats Bar */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-[#FAF7F2] border-b border-[#E6DFD6]">
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-white mx-auto mb-2 flex items-center justify-center shadow-sm">
                <MapPin className="w-5 h-5 text-[#1C1C1C]" />
              </div>
              <p className="text-xs text-[#7A7A7A] uppercase tracking-wider">{t('productDetail.origin')}</p>
              <p className="font-semibold text-[#1C1C1C]">{product.country_origin}</p>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-white mx-auto mb-2 flex items-center justify-center shadow-sm">
                <Leaf className="w-5 h-5 text-green-600" />
              </div>
              <p className="text-xs text-[#7A7A7A] uppercase tracking-wider">{t('productDetail.ingredients')}</p>
              <p className="font-semibold text-[#1C1C1C]">{product.ingredients?.length || 0}</p>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-white mx-auto mb-2 flex items-center justify-center shadow-sm">
                <Award className="w-5 h-5 text-amber-500" />
              </div>
              <p className="text-xs text-[#7A7A7A] uppercase tracking-wider">{t('productDetail.certifications')}</p>
              <p className="font-semibold text-[#1C1C1C]">{product.certifications?.length || 0}</p>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-white mx-auto mb-2 flex items-center justify-center shadow-sm">
                <Star className="w-5 h-5 text-[#1C1C1C]" />
              </div>
              <p className="text-xs text-[#7A7A7A] uppercase tracking-wider">{t('certificate.status', 'Status')}</p>
              <p className="font-semibold text-green-600">{t('certificate.verifiedStatus', 'Verified')}</p>
            </div>
          </div>

          {/* Ingredients & Origin */}
          <section className="p-8 md:p-12 border-b border-[#E6DFD6]" data-testid="ingredient-origin-section">
            ...
            {hasIngredients ? (
              <div className="grid gap-3">
                {product.ingredients.map((ingredient, idx) => {
                  ...
                  return (
                    <div 
                      key={idx} 
                      className="flex items-center justify-between p-4 bg-[#FAF7F2] rounded-xl hover:bg-[#F4EFE9] transition-colors"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-body text-lg text-[#1C1C1C] font-medium">
                          {ingredient}
                        </span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-body text-base text-[#4A4A4A]">
                          {origin}
                        </span>
                        {countryCode ? (
              ...
              <div className="text-center py-8 bg-[#FAF7F2] rounded-xl">
                <p className="text-[#7A7A7A]">{t('common.noData')}</p>
              </div>
            )}
          </section>

          {/* Nutritional Information */}
          <section className="p-8 md:p-12 border-b border-[#E6DFD6]" data-testid="nutritional-table-section">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 rounded-full bg-[#1C1C1C] flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-sm text-[#7A7A7A]">{t('certificate.per100g', 'Per 100g')}</p>
              </div>
            </div>
            
            <div className="bg-[#FAF7F2] rounded-2xl overflow-hidden">
              {nutritionalInfo && typeof nutritionalInfo === 'object' ? (
                <div className="divide-y divide-[#E6DFD6]">
                  {Object.entries(nutritionalInfo).map(([key, value], idx) => (
                    <div 
                      key={key} 
                      className={`flex items-center justify-between p-4 ${idx % 2 === 0 ? 'bg-white' : 'bg-[#FAF7F2]'}`}
                    >
          ...
          {/* Certifications */}
          <section className="p-8 md:p-12 border-b border-[#E6DFD6]" data-testid="dietary-labels-section">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 rounded-full bg-[#1C1C1C] flex items-center justify-center">
            ...
            {hasCertifications ? (
              <div className="flex flex-wrap gap-3">
                {product.certifications.map((cert, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-[#1C1C1C] to-[#4A4A4A] text-white px-6 py-3 rounded-full text-base font-semibold shadow-md hover:shadow-lg transition-shadow"
                  >
                    <Shield className="w-4 h-4" />
                    {cert}
                  </span>
          ...
          {/* Country & Producer */}
          <section className="p-8 md:p-12 bg-gradient-to-br from-[#FAF7F2] to-white border-b border-[#E6DFD6]" data-testid="producer-story-section">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <h2 className="font-heading text-2xl font-bold text-[#1C1C1C] mb-4">
          ...
          <div className="p-8 md:p-12 text-center">
            <Link to={`/products/${productId}`}>
              <Button
                size="lg"
                className="bg-[#1C1C1C] hover:bg-[#4A4A4A] text-white rounded-full px-12 py-6 font-semibold text-lg transition-all shadow-lg hover:shadow-xl hover:scale-105"
          ...
          {/* Footer */}
          <div className="p-8 bg-[#1C1C1C] text-center">
