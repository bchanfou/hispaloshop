...
const COOKIE_TABLE = [{
  ...
  tipo: 'Esencial',
  duracion: 'Permanente',
  proposito: 'Multi-cuenta'
}, {
  cookie: 'locale',
  tipo: 'Esencial',
  duracion: '1 a\u00f1o',
  proposito: 'Idioma/pa\u00eds'
}, {
  cookie: 'cart_id',
  tipo: 'Esencial',
  duracion: '30 d\u00edas',
...
export default function LegalPage() {
  ...
  return <div className="min-h-screen bg-stone-50 pt-24" style={{
    fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif'
      ...
      <div className="max-w-[720px] mx-auto px-4 pb-20">
        ...
        duration: 0.5
          ...
          {TABS.map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 rounded-full text-sm font-semibold cursor-pointer transition-all duration-150 ${activeTab === tab.id ? 'bg-stone-950 text-white' : 'bg-transparent text-stone-500 hover:text-stone-700'}`}>
              {tab.label}
            </button>)}
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          duration: 0.4
            ...
            {content.sections.map((section, i) => <motion.div key={i} initial={{
            opacity: 0,
            y: 16
          }} whileInView={{
            opacity: 1,
            ...
            {activeTab === 'cookies' && <motion.div initial={{
            opacity: 0,
            y: 16
          }} whileInView={{
            opacity: 1,
            ...
            delay: 0.2
                ...
                <div className="overflow-x-auto rounded-xl border border-stone-200">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {COOKIE_TABLE.map((row, i) => <tr key={row.cookie} className={i % 2 === 0 ? 'bg-white' : 'bg-stone-50'}>
