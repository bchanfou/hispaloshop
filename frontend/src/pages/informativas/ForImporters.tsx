    ...
    <div>
        ...
        <div className="max-w-[1200px] mx-auto w-full">
          <div className="max-w-[820px]">
            ...
            <h1 className="info-h1 hero-animate-in-delay-1 text-white whitespace-pre-line mb-6">
              {t('for_importers.tuTiendaB2cYB2bEnUnSoloLugar', 'Tu tienda B2C y B2B en un solo lugar.')}
            </h1>
        ...
        <div className="reveal" id="ventajas-importador">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
            <div>
              ...
              <p className="text-stone-500 mb-4">{t('for_importers.vendeTusProductosImportadosDirectame', 'Vende tus productos importados directamente al consumidor en el país destino, sin intermediarios.')}</p>
              <h4 className="font-bold mb-2">Influencers locales</h4>
              <p className="text-stone-500 mb-4">{t('for_importers.conectaYColaboraConInfluencersLocal', 'Conecta y colabora con influencers locales para promocionar tus productos (Pro y Elite).')}</p>
              <h4 className="font-bold mb-2">Productores globales</h4>
              <p className="text-stone-500 mb-4">{t('for_importers.compraDirectamenteAProductoresDeCua', 'Compra directamente a productores de cualquier país, con contacto directo y sin intermediarios.')}</p>
              <h4 className="font-bold mb-2">{t('for_importers.panelDeGestionUnificado', 'Panel de gestión unificado')}</h4>
              <p className="text-stone-500 mb-4">{t('for_importers.gestionaPedidosPagosLogisticaCert', 'Gestiona pedidos, pagos, logística, certificados y comunicación desde un solo panel.')}</p>
              <h4 className="font-bold mb-2">Soporte real</h4>
              <p className="text-stone-500 mb-4">{t('for_importers.soportePorChatInternoConAdminLocal', 'Soporte por chat interno con admin local y por email en todos los planes.')}</p>
            </div>
            <div>
              <h4 className="font-bold mb-2">Diferencias por plan</h4>
              <ul className="text-stone-700 text-[15px] space-y-2">
                <li><b>Free:</b> {t('for_importers.todasLasFuncionesBasicasVenderB2c', 'Todas las funciones básicas, vender B2C, publicar productos, certificados QR, soporte estándar.')}</li>
                <li><b>Pro:</b> {t('for_importers.matchingConInfluencersDashboardAvan', 'Matching con influencers, dashboard avanzado, analítica de ventas, soporte prioritario.')}</li>
                <li><b>Elite:</b> {t('for_importers.todoLoAnteriorIaComercialAvanzada', 'Todo lo anterior + IA comercial avanzada, análisis de mercado internacional, generación automática de contratos, comisión más baja.')}</li>
              </ul>
            </div>
          </div>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3"><Check size={18} className="text-stone-950" strokeWidth={2.5} />{t('for_importers.colaboracionConInfluencersLocalesPr', 'Colaboración con influencers locales (Pro y Elite)')}</div>
            <div className="flex items-center gap-3"><Check size={18} className="text-stone-950" strokeWidth={2.5} />{t('for_importers.panelDeGestionPagosYLogisticaUnif', 'Panel de gestión, pagos y logística unificados')}</div>
            ...
            <div className="flex items-center gap-3"><Check size={18} className="text-stone-950" strokeWidth={2.5} />{t('for_importers.iaComercialAvanzadaYGeneracionDeCo', 'IA comercial avanzada y generación de contratos (Elite)')}</div>
          </div>
        </div>
      </Section>
      ...
      <Section dark={true}>
        <div className="reveal">
          ...
          <h2 className="info-h2 text-white mb-8">{t('for_importers.importarYVenderNuncaFueTanFacil', 'Importar y vender nunca fue tan fácil')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-10">
            <div>
              <ol className="list-decimal list-inside text-white/80 space-y-2">
                <li>{t('for_importers.creaTuCuentaDeImportador', 'Crea tu cuenta de importador')}</li>
                <li>Publica tus productos importados</li>
                <li>Vende al consumidor final (B2C)</li>
                <li>Colabora con influencers (Pro y Elite)</li>
                <li>{t('for_importers.accedeAHerramientasDeIaYAnalitica', 'Accede a herramientas de IA y analítica avanzada (Pro y Elite)')}</li>
                <li>{t('for_importers.gestionaPagosLogisticaYCertificados', 'Gestiona pagos, logística y certificados desde un solo panel')}</li>
              </ol>
            </div>
            <div>
              <h4 className="text-white mb-2">Preguntas frecuentes</h4>
              <ul className="text-white/70 text-[15px] space-y-2">
                <li><b>¿Qué necesito para empezar?</b> {t('for_importers.soloCrearTuCuentaYSubirTusProduct', 'Solo crear tu cuenta y subir tus productos.')}</li>
                <li><b>¿Puedo vender tanto B2C como B2B?</b> {t('for_importers.siPuedesVenderAlConsumidorFinalY', 'Sí, puedes vender al consumidor final y también acceder a oportunidades B2B.')}</li>
                <li><b>¿Qué comisiones se aplican?</b> Depende del plan: Free (20%), Pro (18%), Elite (15%).</li>
                <li><b>¿Cómo es el soporte?</b> {t('for_importers.siemprePorChatInternoYEmailConPr', 'Siempre por chat interno y email, con prioridad en Pro y Elite.')}</li>
