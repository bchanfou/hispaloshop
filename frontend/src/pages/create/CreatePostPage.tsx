...
function buildFilterCSS(activeFilter, filterIntensity, adjustments) {
  ...
  if (activeFilter.css !== 'none' && filterIntensity > 0) {
    ...
    const scaled = activeFilter.css.replace(/(\w+)\(([^)]+)\)/g, (_, fn, val) => {
...
export default function CreatePostPage() {
  ...
  const handlePublish = async () => {
    ...
    if (!selectedFiles.length) {
      toast.error(i18n.t('create_post.anadeAlMenosUnaImagenParaPublicar', 'Añade al menos una imagen para publicar'));
      return;
    }
  ...
  const renderStep1 = () => <div className="bg-black flex flex-col w-full h-full" style={{
    ...
    paddingBottom: 'env(safe-area-inset-bottom)'
        ...
        navigate(-1);
          ...
          <span className="text-white text-[15px] font-medium tracking-tight">{i18n.t('create_post.nuevaPublicacion', 'Nueva publicación')}</span>
          <button disabled={!hasFiles} onClick={() => goToStep(2)} className="bg-transparent border-none cursor-pointer text-[13px] font-semibold text-white transition-opacity" style={{
        ...
        {hasFiles ? <>
            ...
            <div className="relative flex-1 flex items-center justify-center bg-[#111] overflow-hidden shrink-0 max-h-[52vh]">
              ...
              {selectedFiles.length > 1 && <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-sm text-white text-[11px] font-semibold px-2.5 py-1 rounded-full">
                  {previewIndex + 1} / {selectedFiles.length}
                </div>}
            </div>

            {/* Reorder strip */}
            {selectedFiles.length > 1 && <div className="bg-black/80 px-4 py-2 flex gap-1.5 overflow-x-auto shrink-0">
                {previewUrls.map((url, i) => <div key={i} draggable onDragStart={e => {
          ...
          setPreviewIndex(i);
        }} onClick={() => setPreviewIndex(i)} className="relative shrink-0 cursor-grab overflow-hidden rounded-xl transition-transform w-11 h-11" style={{
          border: previewIndex === i ? '2px solid #fff' : '2px solid transparent'
            ...
            e.stopPropagation();
            const removeIdx = i;
            setSelectedFiles(prev => prev.filter((_, idx) => idx !== removeIdx));
            setPreviewIndex(prev => {
              const newLen = selectedFiles.length - 1;
              if (newLen <= 0) return 0;
              if (removeIdx < prev) return prev - 1;
              if (prev >= newLen) return newLen - 1;
              return prev;
            });
          }} aria-label={`Quitar imagen ${i + 1}`} className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-black/70 text-white text-[10px] border-none cursor-pointer flex items-center justify-center p-0">×</button>
                  </div>)}
              </div>}
            ...
            <div className="bg-black/80 px-4 py-3 flex items-center justify-between shrink-0">
              ...
              <div className="flex gap-2">
                <button onClick={() => cameraInputRef.current?.click()} className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white border-none text-[12px] font-medium py-2 px-3.5 rounded-full cursor-pointer transition-colors min-h-[40px]">
                  <Camera size={15} /> Foto
                </button>
                <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white border-none text-[12px] font-medium py-2 px-3.5 rounded-full cursor-pointer transition-colors min-h-[40px]">
            ...
            <div className="flex gap-2 justify-center px-4 pb-4 pt-2">
              {['foto', 'galeria'].map(tab => <button key={tab} onClick={() => setUploadTab(tab)} className={`border-none rounded-full px-5 py-2.5 text-[13px] font-semibold cursor-pointer transition-colors min-h-[44px] ${uploadTab === tab ? 'bg-white text-black' : 'bg-transparent text-white/60'}`}>
                  {tab === 'foto' ? 'Foto' : i18n.t('store.gallery', 'Galería')}
                </button>)}
            </div>

            {/* Center content */}
            <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
              {uploadTab === 'foto' ? <Camera size={48} className="text-white/25" /> : <Image size={48} className="text-white/25" />}
              <span className="text-base text-white font-medium">
                {uploadTab === 'foto' ? 'Toma una foto' : i18n.t('create_post.seleccionaImagenes', 'Selecciona imágenes')}
              </span>
              <span className="text-xs text-white/40 text-center">
                {uploadTab === 'foto' ? i18n.t('create_post.capturaDirectamenteDesdeTuCamara', 'Captura directamente desde tu cámara') : 'Hasta 10 imágenes · JPG, PNG o WebP'}
              </span>
              <button onClick={() => uploadTab === 'foto' ? cameraInputRef.current?.click() : fileInputRef.current?.click()} className="bg-white text-black border-none text-sm font-semibold py-3 px-7 rounded-full cursor-pointer mt-2 transition-all hover:bg-white/90 active:scale-95 min-h-[44px]" aria-label={uploadTab === 'foto' ? i18n.t('create_post.abrirCamaraParaTomarFoto', 'Abrir cámara para tomar foto') : 'Elegir imágenes de la galería'}>
                {uploadTab === 'foto' ? i18n.t('create_post.abrirCamara', 'Abrir cámara') : 'Elegir de la galería'}
              </button>
            </div>
          </>}
  ...
  const renderStep2 = () => {
    ...
    return <div className="w-full h-full bg-black flex flex-col">
        ...
        <div className="relative w-full flex justify-center items-center bg-[#111] shrink-0 max-h-[50%] overflow-hidden">
          ...
          filterSwipeRef.current.startX = e.touches[0].clientX;
        }} onTouchEnd={e => {
          ...
          if (dx < 0 && currentIdx < FILTERS.length - 1) {
            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeFilter.name} initial={{
              opacity: 0.7
            }} animate={{
              opacity: 1
            }} transition={{
              duration: 0.2
                ...
                <img src={previewUrls[previewIndex]} alt={`Vista previa con filtro ${activeFilter.name}`} className="w-full h-full object-cover" style={{
                filter: filterCSS
              }} />
              </motion.div>
            </AnimatePresence>
            {/* filter name overlay */}
            <AnimatePresence>
              {showFilterName && activeFilter.name !== 'Natural' && <motion.div initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
            ...
            {/* vignette overlay */}
            {adjustments.vignette > 0 && <div className="absolute inset-0" style={buildVignetteStyle(adjustments.vignette)} />}
            {/* text overlays — dynamic positions require inline style */}
            {textOverlays.map(o => <div key={o.id} onMouseDown={e => handleDragStart(e, o)} onTouchStart={e => handleDragStart(e, o)} className="absolute -translate-x-1/2 -translate-y-1/2 cursor-grab select-none font-semibold touch-none" style={{
            left: `${o.x}%`,
            top: `${o.y}%`,
            fontFamily: o.font,
            fontSize: o.size,
            ...
            <div className="absolute bottom-2.5 left-1/2 -translate-x-1/2 flex gap-1.5">
              {ASPECT_RATIOS.map(ar => <button key={ar.label} onClick={() => setAspectRatio(ar)} className={`border-none rounded-full px-2.5 py-1 text-[11px] font-semibold cursor-pointer ${aspectRatio.label === ar.label ? 'bg-white text-black' : 'bg-black/50 text-white'}`}>
                  {ar.label}
                </button>)}
            </div>
          </div>
        ...
        {selectedFiles.length > 1 && <div className="flex gap-1.5 px-3 py-2 overflow-x-auto bg-black/60 shrink-0">
            {previewUrls.map((url, i) => <div key={i} onClick={() => setPreviewIndex(i)} className="w-11 h-11 rounded-[10px] overflow-hidden shrink-0 cursor-pointer transition-all duration-150" style={{
          border: previewIndex === i ? '2px solid #fff' : '2px solid transparent',
        ...
        <div className="flex border-b border-white/15 shrink-0">
          {tabs.map(t => <button key={t.key} onClick={() => setActiveTab(t.key)} className={`flex-1 bg-transparent border-none py-2.5 text-xs font-medium cursor-pointer flex items-center justify-center gap-1 ${activeTab === t.key ? 'text-white border-b-2 border-b-white' : 'text-white/50 border-b-2 border-b-transparent'}`}>
              <t.icon size={14} />
              {t.label}
            </button>)}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          {activeTab === 'filtros' && <div>
              ...
              if (navigator.vibrate) navigator.vibrate(10);
                ...
                transform: activeFilter.name === f.name ? 'scale(1.05)' : 'scale(1)'
                      ...
                      {previewUrls[previewIndex] && <img src={previewUrls[previewIndex]} alt={f.name} className="w-full h-full object-cover" style={{
              ...
              <div className="flex items-center gap-2.5 py-1">
                ...
                <input type="range" min={0} max={100} value={filterIntensity} onChange={e => setFilterIntensity(Number(e.target.value))} className="flex-1 accent-stone-950" />
                <span className="text-xs text-white/50 min-w-[28px] text-right">{filterIntensity}</span>
              </div>
            </div>}

          {/* ─── Ajustes ─── */}
          {activeTab === 'ajustes' && <div className="flex flex-col gap-3.5">
              {[{
            key: 'brightness',
            label: 'Brillo',
            min: -100,
            ...
            key: 'saturation',
            label: i18n.t('create_post.saturacion', 'Saturación'),
            min: -100,
            max: 100
          }, {
            key: 'warmth',
            ...
            key: 'vignette',
            label: i18n.t('create_post.vineteado', 'Viñeteado'),
            min: 0,
            max: 100
          }].map(s => <div key={s.key} className="flex items-center gap-2.5">
                  <span className="text-xs text-white/70 min-w-[72px]">{s.label}</span>
                  <input type="range" min={s.min} max={s.max} value={adjustments[s.key]} onChange={e => setAdjustments(p => ({
              ...p,
              [s.key]: Number(e.target.value)
            }))} onDoubleClick={() => setAdjustments(p => ({
              ...p,
              [s.key]: 0
            }))} title="Doble clic para resetear" className="flex-1 accent-stone-950" />
                  <span className="text-xs text-white/50 min-w-[28px] text-right">{adjustments[s.key]}</span>
                </div>)}
            </div>}

          {/* ─── Texto ─── */}
          {activeTab === 'texto' && <div className="flex flex-col gap-3.5">
              {textOverlays.length < 10 && <button onClick={addTextOverlay} className="self-center bg-white/10 border border-dashed border-white/30 rounded-xl text-white px-5 py-2.5 text-[13px] cursor-pointer flex items-center gap-1.5">
                  <Type size={14} /> + Añadir texto
                </button>}

              {textOverlays.map(o => <div key={o.id} className="bg-white/[0.08] rounded-xl p-2.5 flex flex-col gap-2">
                  <div className="flex items-center gap-1.5">
                    <input value={o.text} onChange={e => updateOverlay(o.id, {
                text: e.target.value
                  ...
                  <div className="flex gap-1.5">
                    {FONT_OPTIONS.map(f => <button key={f.label} onClick={() => updateOverlay(o.id, {
                font: f.value
              })} className={`border-none rounded px-2 py-0.5 text-[11px] cursor-pointer ${o.font === f.value ? 'bg-white text-black' : 'bg-white/15 text-white'}`} style={{
                fontFamily: f.value
              }}>
                        {f.label}
                      </button>)}
                  ...
                  {/* colors */}
                  <div className="flex gap-2">
                    {COLOR_DOTS.map(c => <button key={c.value} onClick={() => updateOverlay(o.id, {
                color: c.value
              })} className="w-[22px] h-[22px] rounded-full cursor-pointer p-0" style={{
                background: c.value,
                ...
                key: 'outline',
                label: 'Contorno'
              }].map(s => <button key={s.key} onClick={() => updateOverlay(o.id, {
                style: s.key
              })} aria-pressed={o.style === s.key} className={`flex-1 border-none rounded-full py-1.5 text-[11px] font-semibold cursor-pointer transition-colors duration-150 ${o.style === s.key ? 'bg-white text-black' : 'bg-white/15 text-white'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-white/60">{i18n.t('create_post.tamano', 'Tamaño')}</span>
                    <input type="range" min={14} max={48} value={o.size} onChange={e => updateOverlay(o.id, {
          ...
          {/* ─── Recorte ─── */}
          {activeTab === 'recorte' && <div className="flex gap-2 justify-center flex-wrap">
              {CROP_RATIOS.map(cr => <button key={cr.label} onClick={() => {
            if (cr.value) setAspectRatio(cr);else setAspectRatio({
              label: 'Original',
              value: null
            });
          }} className={`border-none rounded-full px-[18px] py-2 text-[13px] font-medium cursor-pointer ${aspectRatio.label === cr.label ? 'bg-white text-black' : 'bg-white/[0.12] text-white'}`}>
                  {cr.label}
                </button>)}
            </div>}
        </div>
  ...
  const renderStep3 = () => <div className="w-full h-full bg-white flex flex-col">
      {/* top bar */}
      <div className="bg-white h-[52px] flex items-center px-4 border-b border-stone-200 shrink-0">
        ...
        <span className="flex-1 text-center text-stone-950 text-[15px] font-medium">{i18n.t('create_post.nuevaPublicacion', 'Nueva publicación')}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-[100px]">
        {/* thumbnail row */}
        {selectedFiles.length > 1 && <div className="flex gap-2 overflow-x-auto mb-4">
            {previewUrls.map((url, i) => <div key={i} onClick={() => setPreviewIndex(i)} className="w-[52px] h-[52px] rounded-xl overflow-hidden shrink-0 cursor-pointer" style={{
        ...
        {/* draft banner */}
        {draftBanner && <div className="flex items-center justify-between gap-2 px-3.5 py-2.5 mb-3 bg-stone-100 rounded-[14px] border border-stone-200">
        ...
        <div className="relative mb-3">
          ...
          <div aria-hidden="true" className="absolute top-0 left-0 right-0 p-3 text-sm leading-relaxed whitespace-pre-wrap break-words text-transparent pointer-events-none box-border border-[1.5px] border-transparent">
            {caption.split(/(#[\w\u00C0-\u024F\u1E00-\u1EFF]+|@[\w\u00C0-\u024F\u1E00-\u1EFF]+)/g).map((part, i) => part.startsWith('#') ? <span key={i} className="text-stone-500 font-semibold">{part}</span> : part.startsWith('@') ? <span key={i} className="text-stone-600 font-semibold">{part}</span> : <span key={i}>{part}</span>)}
          </div>
        ...
        try {
          ...
          if (suggestion) {
            ...
            toast.success(i18n.t('create_post.sugerenciaAnadida', 'Sugerencia añadida'));
          } else {
            toast.error(i18n.t('create_post.noSePudoGenerarUnaSugerencia', 'No se pudo generar una sugerencia'));
          }
        } catch {
          toast.error(i18n.t('create_post.errorAlConectarConDavidAi', 'Error al conectar con David AI'));
        } finally {
          setAiLoading(false);
        }
      }} disabled={aiLoading} className="flex items-center bg-stone-100 text-stone-950 text-[13px] font-medium rounded-full px-4 py-2 border-none mb-4 transition-opacity" style={{
        ...
        {/* tag products */}
        <div className="mb-4">
          <button onClick={() => setShowProductSearch(true)} className="bg-stone-100 text-stone-950 border border-stone-200 rounded-xl px-4 py-2.5 text-[13px] cursor-pointer w-full text-left">
          ...
          {taggedProducts.length > 0 && <div className="flex flex-wrap gap-1.5 mt-2">
              {taggedProducts.map(p => <span key={p.id} className="bg-stone-100 border border-stone-200 rounded-full px-2.5 py-1 text-xs flex items-center gap-1">
                  {p.name}
                  <button onClick={() => setTaggedProducts(prev => prev.filter(x => x.id !== p.id))} aria-label={`Quitar ${p.name}`} className="bg-transparent border-none cursor-pointer p-0 leading-none">
                    <X size={12} />
                  </button>
                </span>)}
            </div>}
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 bg-stone-100 border border-stone-200 rounded-xl px-3 py-2.5">
            ...
            <input value={location} onChange={e => setLocation(e.target.value)} placeholder={i18n.t('create_post.anadirUbicacionEjGaliciaCosta', 'Añadir ubicación... (ej. Galicia, Costa Brava)')} aria-label={i18n.t('store.location', 'Ubicación')} className="flex-1 border-none outline-none text-[13px] bg-transparent" />
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex gap-2">
            <button onClick={() => setAudience('public')} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[13px] font-medium cursor-pointer transition-all duration-150 ${audience === 'public' ? 'border-2 border-stone-950 bg-stone-950 text-white' : 'border-[1.5px] border-stone-200 bg-transparent text-stone-950'}`}>
              <Globe size={14} /> Público
            </button>
            <button onClick={() => setAudience('followers')} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[13px] font-medium cursor-pointer transition-all duration-150 ${audience === 'followers' ? 'border-2 border-stone-950 bg-stone-950 text-white' : 'border-[1.5px] border-stone-200 bg-transparent text-stone-950'}`}>
        ...
        <div className="mb-4 flex flex-col gap-3">
          <label className="flex items-center justify-between cursor-pointer">
            ...
            <input type="checkbox" checked={hideLikes} onChange={e => setHideLikes(e.target.checked)} className="accent-stone-950 w-[18px] h-[18px]" />
          </label>
          <label className="flex items-center justify-between cursor-pointer">
            <span className="text-[13px] text-stone-950">Desactivar comentarios</span>
            <input type="checkbox" checked={disableComments} onChange={e => setDisableComments(e.target.checked)} className="accent-stone-950 w-[18px] h-[18px]" />
          </label>
        </div>
      </div>

      {/* product search modal */}
      {showProductSearch && <div className="absolute inset-0 z-[60] bg-black/50 flex items-end justify-center">
          <div className="bg-white w-full max-h-[70vh] rounded-t-2xl flex flex-col overflow-hidden">
            <div className="flex items-center px-4 py-3 border-b border-stone-200 gap-2">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Buscar producto..." aria-label="Buscar producto para etiquetar" autoFocus className="flex-1 border-none outline-none text-sm bg-transparent" />
              <button onClick={() => {
            setShowProductSearch(false);
            setSearchQuery('');
            setSearchResults([]);
          }} aria-label={i18n.t('create_post.cerrarBusqueda', 'Cerrar búsqueda')} className="bg-transparent border-none cursor-pointer">
                <X size={18} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {searchLoading && <div className="flex justify-center py-4">
            ...
            return <button key={pid} onClick={() => {
              ...
              {searchQuery && searchResults.length === 0 && <p className="text-center text-stone-500 text-[13px] py-5">Sin resultados</p>}
            </div>
          </div>
        </div>}

      {/* Publish success overlay */}
      {publishSuccess && <div className="absolute inset-0 z-[70] bg-white flex flex-col items-center justify-center gap-4 animate-[fadeIn_0.3s_ease]">
          ...
          <span className="text-base font-semibold text-stone-950">{i18n.t('create_post.publicado', '¡Publicado!')}</span>
        </div>}

      {/* Top progress bar — visible during upload */}
      {publishing && uploadProgress > 0 && uploadProgress < 100 && <div className="absolute top-0 left-0 right-0 z-[80] h-1 bg-stone-200 rounded-full overflow-hidden">
          <div className="h-full bg-stone-950 rounded-full transition-[width] duration-300 ease-in-out" style={{
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-stone-200 pb-[max(1rem,env(safe-area-inset-bottom))]">
        {/* Error + Retry */}
        {publishError && !publishing && <div className="flex items-center justify-between mb-2.5 px-1">
            <span className="text-[13px] text-stone-950 font-medium">Error al publicar</span>
            <button onClick={handlePublish} className="text-[13px] font-semibold text-stone-950 bg-transparent border-none cursor-pointer underline">
              Reintentar
            </button>
          </div>}
        {publishing ? <div className="flex gap-2">
            <button onClick={handlePublish} disabled className="flex-1 bg-stone-950 text-white text-[15px] font-semibold py-3.5 rounded-full border-none flex items-center justify-center gap-2 min-h-[48px] opacity-80">
            ...
            <button onClick={handleCancelUpload} className="bg-stone-100 text-stone-950 text-[13px] font-semibold py-3.5 px-5 rounded-full border-none cursor-pointer hover:bg-stone-200 transition-colors min-h-[48px]">
              Cancelar
            </button>
          </div> : <button onClick={handlePublish} className="w-full bg-stone-950 text-white text-[15px] font-semibold py-3.5 rounded-full border-none flex items-center justify-center gap-2 transition-all duration-150 hover:bg-stone-800 cursor-pointer min-h-[48px]">
            {publishError ? i18n.t('create_post.reintentarPublicacion', 'Reintentar publicación') : 'Publicar ahora'}
          </button>}
      </div>
  ...
  return <AnimatePresence mode="wait" custom={stepDirection}>
      <motion.div key={step} custom={stepDirection} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={stepTransition} className="fixed inset-0 z-50 lg:max-w-[480px] lg:mx-auto">
        {steps[step]()}
      </motion.div>
    </AnimatePresence>;
}