...
function getPreviewText(draft, type) {
  if (type === 'Post' || type === 'Reel') return draft.caption || i18n.t('categories.sinDescripcion', 'Sin descripción');
  if (type === 'Story') {
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-stone-200">
        <div className="max-w-[600px] mx-auto flex items-center gap-3 px-4 py-3">
          <button onClick={() => navigate(-1)} className="bg-transparent border-none cursor-pointer p-1" aria-label="Volver">
      ...
      <div className="max-w-[600px] mx-auto px-4 py-5">
        {drafts.length === 0 ? <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          draft,
          savedAt
        }) => <div key={key} className="flex items-center gap-3 bg-stone-50 rounded-2xl p-4 border border-stone-100">
                {/* Type icon */}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => navigate(route)} className="text-xs font-semibold text-white bg-stone-950 rounded-full px-4 py-2 border-none cursor-pointer hover:bg-stone-800 transition-colors">
                    Continuar
                  </button>
                  <button onClick={() => handleDelete(key)} className="w-9 h-9 rounded-full bg-stone-100 hover:bg-stone-200 border-none cursor-pointer flex items-center justify-center transition-colors" aria-label={`Eliminar borrador de ${type}`}>
