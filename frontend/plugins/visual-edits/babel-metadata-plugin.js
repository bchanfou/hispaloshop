  ...
  return {
    ...
    visitor: {
      ...
      JSXElement(jsxPath, state) {
        // Skip processing for excluded files
        const currentFile = state.filename || state.file.opts.filename || '';
        const basename = path.basename(currentFile);
        
        // Debug logging
        if (basename.includes('Certificate')) {
          console.log('[Babel Plugin] Processing file:', basename, 'Excluded:', EXCLUDED_FILES.includes(basename));
        }
